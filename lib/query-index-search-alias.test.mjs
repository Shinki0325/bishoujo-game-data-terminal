...
const works = [
  ...
  { workId: '5', title: '瀬里奈', brandId: 'alice', median: 80, voteCount: 100, releaseDate: '1999-07-23', filterIds: [], genreFilterIds: [], platformFilterId: 'pc', isNukige: false }
];
const baseState = {
...
test('matches simplified Chinese input against a Japanese kanji title', () => {
  const simplified = queryIndexedCatalog(index(), { ...baseState, titleQuery: '濑里奈' });
  const original = queryIndexedCatalog(index(), { ...baseState, titleQuery: '瀬里奈' });
  assert.deepEqual(simplified.map(work => work.workId), ['5']);
  assert.deepEqual(original.map(work => work.workId), ['5']);
});

test('matches generated Mandarin pinyin against a Japanese kanji title', () => {
  const result = queryIndexedCatalog(index(), { ...baseState, titleQuery: 'lailinai' });
  assert.deepEqual(result.map(work => work.workId), ['5']);
});
