...
import { FormulaSyntaxError } from './formula.js';
import { DEFAULT_TIERS, TIER_COLOR_IDS } from './tier-palette.js';
import { MAX_TIERS, MIN_TIERS, TIER_NAME_MAX_LENGTH } from './tier-config.js';

export const STATE_SCHEMA_VERSION = 'egs-tier-state-v4';
...
const VALID_TIER_COLOR_IDS = new Set(TIER_COLOR_IDS);
const VALID_SORT_KEYS = new Set(['voteCount', 'median', 'title', 'brandName', 'releaseDate']);
...
const MAX_AUTHORITY_WORK_IDS = 3000;
const MAX_FILTER_IDS = 49;
...
function normalizeFilterState(source, authoritySnapshot) {
  const filterState = exactSnapshot(source, V4_FILTER_STATE_FIELDS, 'state.filterState');
  try {
    ...
    if (
      filterState.releaseYearStart < 1989
...
function normalizeV3FilterState(source, authoritySnapshot) {
  ...
  return normalizeFilterState(candidate, authoritySnapshot);
}
...
function normalizeTiers(source) {
  ...
  for (let index = 0; index < sourceTiers.length; index += 1) {
    ...
    if (typeof tier.colorId !== 'string' || !VALID_TIER_COLOR_IDS.has(tier.colorId)) {
      throw stateError(
        'INVALID_TIER',
...
function normalizeV3State(candidate, authoritySnapshot) {
  try {
    ...
    return normalizeV4State(ownObject([
      ['schemaVersion', STATE_SCHEMA_VERSION],
      ['sampleId', state.sampleId],
...
export function saveState(storage, state) {
  return runValidationInvocation(() => {
    const normalized = normalizeV4State(state, null);
