...
test('workbench defers tools and detail runtime; worker preloading stays works-only', async () => {
  ...
  const projectResources = await readFile(new URL('./project-resources.js', import.meta.url), 'utf8');
  assert.match(projectResources, /const ensureProjectEntityRuntime = createLazyResource/);
  assert.doesNotMatch(projectResources, /^import.*from.*project-entity-runtime/m);
  assert.match(source, /ensureProjectRuntime: projectResources\.ensureRuntime/);
