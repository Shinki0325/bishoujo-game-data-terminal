...
test('workbench defers tools and detail runtime; worker preloading stays works-only', async () => {
  ...
  assert.equal([...source.matchAll(/ensureProjectEntityRuntime\(\)/g)].length, 1);
  assert.match(source, /workDetailCreditsLoader\.load\(work\.workId\), ensureProjectEntityRuntime\(\)/);
