...
test('person detail loading deduplicates the same person and keeps displayed detail stable', () => {
  ...
  assert.match(source, /detailInFlight\?\.personId === personId[\s\S]*detailInFlight\.request\.isCurrent\(\)/);
...
test('Escape shares the user close path while programmatic close only invalidates', () => {
  ...
  assert.match(source, /lifetime\.listen\(dialog, 'cancel', event => \{\s*event\.preventDefault\(\);\s*closeDetailFromUser\(\);/);
  assert.match(source, /lifetime\.listen\(close, 'click', closeDetailFromUser\)/);
  const closeBlockStart = source.indexOf("lifetime.listen(dialog, 'close'");
