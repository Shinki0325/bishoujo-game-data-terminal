...
test('ranking controls preserve scale, lazy resize, subject-aware flags, mobile mirrors and cleanup',()=>{
  class Node extends EventTarget {
    value='100';checked=false;clicks=0;focuses=0;attributes=new Map();
  ...
  const exitButton=new Node();
  documentRef.getElementById=id=>id==='ranking-immersive-exit'?exitButton:null;
  ...
  view.setImmersive(true);assert.equal(exitButton.focuses,1);
  view.setImmersive(false);assert.equal(elements.rankingImmersive.focuses,1);
  assert.deepEqual(elements.rankingImmersive.focusOptions,{preventScroll:true});
  for(let i=0;i<2;i++){const pending=[...frames.values()];frames.clear();for(const fn of pending)fn();}
  assert.deepEqual(restored,[{top:450}]);
  assert.equal(elements.rankingImmersive.focuses,1,'delayed scroll restore must not refocus');
  windowRef.matchMedia=()=>({matches:true});
  view.prepareMode(true);view.setImmersive(true);view.prepareMode(false);view.setImmersive(false);
  assert.equal(elements.mobileRankingMore.focuses,1);
