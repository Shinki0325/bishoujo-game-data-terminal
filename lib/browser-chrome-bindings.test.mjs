...
test('ranking controls preserve scale, lazy resize, subject-aware flags, mobile mirrors and cleanup',()=>{
  ...
  const ranking={refreshLayout:()=>refreshes++,setShowCounts(){},setShowTitles(){}};
  ...
  elements.rankingImmersive.click();assert.equal(entered,1);windowRef.dispatchEvent(new Event('resize'));view.dispose();
