...
test('deferred file is not fetched during boot; concurrent reads share, validate and retry',async()=>{
 ...
 await assert.rejects(data.loadSearchText(),/503/);fail='hash';await assert.rejects(data.loadSearchText(),/校验/);fail=null;
 const [a,b]=await Promise.all([data.loadSearchText(),data.loadSearchText()]);assert.equal(a,b);assert.equal(calls,3);
