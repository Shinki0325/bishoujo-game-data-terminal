...
test('shard fetches dedupe, bound concurrency/cache, retry HTTP/hash failures and reject wrong order',async()=>{
 ...
 fail=1;await assert.rejects(store.get(['1']),/503/);now+=3001;fail=2;await assert.rejects(store.get(['1']),/校验/);now+=3001;fail=0;assert.equal((await store.get(['1'])).get('1').workId,'1');
