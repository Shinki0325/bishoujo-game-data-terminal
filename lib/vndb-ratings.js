...
export function prepareVndbRatingsSidecar(value, {
  ...
  if (!Array.isArray(value.works) || value.works.length !== allowedWorkIds.size) {
    throw new TypeError('ratings sidecar must cover every catalog work exactly once');
  }
  ...
  for (const status of RATING_STATUSES) {
    if (!Number.isInteger(value.statusCounts[status]) || value.statusCounts[status] < 0 || value.statusCounts[status] !== observedCounts[status]) {
