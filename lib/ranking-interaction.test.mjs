...
function fixture({preview=async()=>({rankedCount:2,width:800,height:600,columns:4}),storage,isBusy}={}) {
  const nodes=Object.fromEntries(['ranking-export-dialog','ranking-export-options','mobile-ranking-export-png','ranking-live-export',
    '[data-ranking-export-quality]','[data-ranking-export-canvas]','[data-export-summary]','[data-export-confirm]','[data-export-close]','[data-export-retry]','[data-export-json]'].map(k=>[k,new Node()]));
...
test('canvas choice defaults to base, refreshes geometry and persists without requiring storage',async()=>{
  const data=new Map(),storage={getItem:k=>data.get(k),setItem:(k,v)=>data.set(k,v)};
  const f=fixture({storage}),key='[data-ranking-export-canvas]';
  assert.equal(f.nodes[key].value,'base');f.nodes['ranking-export-options'].click();await settle();
  f.nodes[key].value='compact';f.nodes[key].dispatchEvent(new Event('change'));await settle();
  assert.equal(f.counts.preview,2);assert.equal(f.counts.png,0);f.api.dispose();
  const next=fixture({storage});assert.equal(next.nodes[key].value,'compact');next.api.dispose();
  const denied=fixture({storage:{getItem(){throw Error();},setItem(){throw Error();}}});
  assert.equal(denied.nodes[key].value,'base');denied.nodes['ranking-export-options'].click();
  denied.nodes[key].value='compact';denied.nodes[key].dispatchEvent(new Event('change'));await settle();
  assert.equal(denied.nodes['[data-export-confirm]'].disabled,false);denied.api.dispose();
});
