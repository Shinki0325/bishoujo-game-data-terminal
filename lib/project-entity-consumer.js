...
function normalizedMediaPolicy(value = {}) {
  ...
  return Object.freeze({
    ...
    requirePublicVisibility: candidate.requirePublicVisibility !== false,
    requireConfirmedAssertion: candidate.requireConfirmedAssertion !== false,
    requireClearedRights: candidate.requireClearedRights !== false,
    requireClearedReview: candidate.requireClearedReview !== false
  });
}
...
function uniqueMediaCandidates(candidates, comparator, keyOf = candidate => candidate.mediaId) {
  const seen = new Set();
...
function mediaCompatibility(candidates, usage, comparator) {
  const selected = uniqueMediaCandidates(candidates.filter(candidate => candidate.usage === usage && candidate.availability === 'available'), comparator);
  return Object.freeze({
...
export function buildMediaProjection(mediaRelations = [], { selectionPolicy = {} } = {}) {
  ...
  const records = [...groups.values()].map(group => {
    const candidates = uniqueMediaCandidates(group, comparator, candidate => `${candidate.mediaId}|${candidate.usage}`);
