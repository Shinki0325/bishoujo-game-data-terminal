...
test('multiple public versions render a layers icon badge', () => {
  const root = card({ presentationMemberCount: 3 });
  const badge = nodes(root, 'selection-card-version-badge')[0];
  assert.ok(badge);
  assert.equal(badge.getAttribute('aria-label'), '3 个公开版本');
  assert.equal(badge.children[0].tagName, 'svg');
  assert.equal(badge.children[0].children.length, 2);
  assert.equal(nodes(badge, 'selection-card-version-count')[0].textContent, '3');
});
