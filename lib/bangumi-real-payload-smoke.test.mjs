...
import {
  BANGUMI_CANONICAL_ALIAS_FALLBACK_PATH,
  BANGUMI_PUBLIC_BINDINGS_PATH,
  BANGUMI_RATINGS_PATH
} from './runtime-config.js';

const root = fileURLToPath(runtimePayloadTestRoot);
...
test('real Bangumi payload covers card, detail, sort and import smoke', () => {
  ...
  const carrierPath = `data/${BANGUMI_PUBLIC_BINDINGS_PATH}`;
  const ratingsPath = `data/${BANGUMI_RATINGS_PATH}`;
  const aliasPath = `data/${BANGUMI_CANONICAL_ALIAS_FALLBACK_PATH}`;
