...
test('full runtime navigation invalidates pending UI before preparing the next route',async()=>{
  const start=main.indexOf('    invalidate() {'),end=main.indexOf('    home()',start),block=main.slice(start,end);
  ...
  const navigation=await read('workbench-navigation-controller.js');
  assert.match(navigation,/const ticket = session\.begin\(key\);\s*applying = false;\s*invalidate\(\)/);
  assert.match(navigation,/const generation = begin\('location'\)/);
  assert.match(main,/function beginUiNavigation\(key\) \{ return navigation\.begin\(key\); \}/);
});
