...
test('full runtime navigation invalidates pending UI before preparing the next route',()=>{
  ...
  for(const lane of ['renderSession','companyRenderSession','detailHydrationSession'])assert.ok(block.includes(`${lane}.suspend()`));
...
test('workbench and detail late failures do not announce errors in the next workspace',()=>{
  assert.match(main,/if \(!generation\.isCurrent\(\)\) return false;\s*generation\.fail\(error\);\s*interactionMetrics\.cancel\(interaction, 'worker-error'\)/);
  assert.match(main,/if \(!sequence\.isCurrent\(\)\) return;\s*sequence\.fail\(error\);/);
  assert.match(main,/if \(elements\.detailsDialog\.open\) return;[^\n]*\n\s*detailViewsSession\.suspend/);
});
test('private statistics request is cancelable; shared catalog and people loaders stay reusable',async()=>{
  assert.match(main,/fetch\(endpoint, \{ cache: 'default', credentials: 'omit', signal: request\.signal \}\)/);
