...
const relations = [
  ...
  { mediaEntityId: 'med_multi', mediaRelationId: 'rel_multi_thumb', targetEntityId: 'wk_1', targetType: 'work', scope: { workEntityId: 'wk_1' }, usage: 'thumbnail', source: 'egs', sourceClaimIds: ['claim_multi_thumb'], rightsStatus: 'cleared-public', clearanceStatus: 'cleared', assertionStatus: 'confirmed', visibility: 'public' },
  { mediaEntityId: 'med_multi', mediaRelationId: 'rel_multi_preview', targetEntityId: 'wk_1', targetType: 'work', scope: { workEntityId: 'wk_1' }, usage: 'preview', source: 'egs', sourceClaimIds: ['claim_multi_preview'], rightsStatus: 'cleared-public', clearanceStatus: 'cleared', assertionStatus: 'confirmed', visibility: 'public' },
  { mediaEntityId: 'med_multi', mediaRelationId: 'rel_multi_gallery', targetEntityId: 'wk_1', targetType: 'work', scope: { workEntityId: 'wk_1' }, usage: 'gallery', source: 'egs', sourceClaimIds: ['claim_multi_gallery'], rightsStatus: 'cleared-public', clearanceStatus: 'cleared', assertionStatus: 'confirmed', visibility: 'public' },
  ...
  { mediaEntityId: 'med_private', mediaRelationId: 'rel_private', targetEntityId: 'wk_1', targetType: 'work', scope: { workEntityId: 'wk_1' }, usage: 'gallery', source: 'owner', sourceClaimIds: ['claim_private'], rightsStatus: 'owner-cleared', clearanceStatus: 'cleared', assertionStatus: 'confirmed', visibility: 'private-review', ...privateFields },
...
for (const flag of ['requirePublicVisibility', 'requireConfirmedAssertion', 'requireClearedRights', 'requireClearedReview']) {
  assert.throws(() => buildMediaProjection(relations, { selectionPolicy: { [flag]: false } }), /cannot disable/);
}

const work = forward.records.find(record => record.targetEntityId === 'wk_1');
assert.equal(work.primaryMediaId, 'med_owner_thumb');
assert.deepEqual(work.fallbackMediaIds, ['med_vndb_thumb', 'med_egs_thumb', 'med_multi', 'med_preview', 'med_gallery_ok']);
assert.deepEqual(work.compatibility.thumbnail, { availability: 'available', mediaId: 'med_owner_thumb', fallbackMediaIds: ['med_vndb_thumb', 'med_egs_thumb', 'med_multi'] });
assert.deepEqual(work.compatibility.preview, { availability: 'available', mediaId: 'med_preview', fallbackMediaIds: ['med_multi'] });
assert.equal(work.candidates.filter(item => item.mediaId === 'med_multi').length, 3);
assert.deepEqual(work.candidates.filter(item => item.mediaId === 'med_multi').map(item => item.usage), ['thumbnail', 'preview', 'gallery']);
assert.equal(work.gallery.length, 5);
...
assert.equal(work.gallery.find(item => item.mediaId === 'med_private').selectionEligibility, 'not-public');
...
console.log('M3 media policy focused checks: 29/29');
