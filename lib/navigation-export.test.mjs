...
function exportFixture(extra={}){
  ...
  const deps={isImportBusy:()=>f.busyImport,getSubject:()=>f.subject,getCompanyState:()=>({tiers:[{id:'s',name:'S',colorId:'red'}],selectedCompanyIds:['c'],tierOrder:{s:['c']}}),
...
test('JSON uses existing work serializer and preserves the exact company schema',()=>{
  ...
  assert.deepEqual(JSON.parse(f.calls.find(c=>c[0]==='json')[1].text),{schemaVersion:2,tiers:[{id:'s',name:'S',colorId:'red'}],selectedCompanyIds:['c'],tierOrder:{s:['c']}});
