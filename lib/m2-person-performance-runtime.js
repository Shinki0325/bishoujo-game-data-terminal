...
export function createM2PersonPerformanceRuntime({ manifestUrl, indexUrl, fetchImpl = globalThis.fetch, cryptoRef = globalThis.crypto, cacheMode = 'force-cache', requestPolicy = {}, cacheBudget = {} } = {}) {
  ...
  // Current snapshot has one 45 MB source shard. Budget must retain that hot
  // shard (3x accounting weight); shrinking the dataset shard is a content task.
  const shardPromises = createBudgetCache({ maxEntries: 24, maxBytes: 192 * 1024 * 1024, ...cacheBudget });
  const requestResource = createResourceRequest({ ...requestPolicy, fetchImpl });
  async function fetchJson(url, expectedSha, label, weigh = () => {}) {
    return requestResource(url, { cache: cacheMode, label, validationKey: expectedSha, validate: async bytes => {
      if (!SHA256_PATTERN.test(expectedSha) || await digest(bytes, cryptoRef) !== expectedSha) throw new Error(`${label} integrity failed`);
      weigh(bytes.byteLength * 3);
  ...
  async function loadDirectory() {
    ...
    })().catch(error => { if (directoryPromise === request) directoryPromise = null; throw error; });
  ...
  async function loadPerson(entityId) {
    ...
    if (!promise) {
      let weight = 0;
      promise = fetchJson(new URL(shardMeta.path, manifestUrl), shardMeta.sha256, `person shard ${shardKey}`, bytes => { weight = bytes; }).then(payload => {
      ...
      }).catch(error => { if (shardPromises.get(shardKey) === promise) shardPromises.delete(shardKey); throw error; });
      shardPromises.set(shardKey, promise);
      const current = promise;
      promise.then(() => shardPromises.settle(shardKey, current, weight), () => {});
    }
