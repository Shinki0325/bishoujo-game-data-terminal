import {
  CATALOG_WORK_LIMIT,
  countWithFilterPatch,
  countWithFilterPatches,
  queryCatalog
} from './catalog.js';
import {
  canRedo,
...
const JSON_EXPORT_FILENAME = 'egs-tier-100-state-v4.json';
...
export function createAppController({ sample, storage, confirm, announce, now, downloadJson }) {
  ...
  const knownFilterIds = sample.filters.map(filter => filter.filterId);
  const authority = {
  ...
  if (state.workspaceMode === 'ranking' && state.selectedWorkIds.length === 0) {
    state = validateState({ ...state, workspaceMode: 'selection' }, authority);
  }
  ...
  function commitEdit(
    ...
    const nextState = validateState({
      ...
      workspaceMode: nextSelectedWorkIds.length === 0
        ? 'selection'
        : workspaceMode,
      savedAt: timestamp(now)
    }, authority);
  ...
  function restoreHistory(nextHistory) {
    ...
    const nextState = validateState({
      ...
      workspaceMode: nextEdit.selectedWorkIds.length === 0 ? 'selection' : state.workspaceMode,
      savedAt: timestamp(now)
    }, authority);
    state = nextState;
    history = nextHistory;
  ...
  function visibleWorks(visibleWorkIds) {
    if (visibleWorkIds === undefined) {
      return queryCatalog(sample.works, state.filterState, knownFilterIds, state.selectedWorkIds);
    }
  ...
  return Object.freeze({
    ...
    setWorkspaceMode(workspaceMode) {
      if (workspaceMode === 'ranking' && state.selectedWorkIds.length === 0) {
        announce('请先选择至少一个候选作品再进入排榜', 'warning');
        return false;
      }
    ...
    clearCandidates() {
      ...
      const nextTierOrder = clearTierOrder(state.tiers, state.tierOrder, state.selectedWorkIds);
      return commitEdit([], nextTierOrder, 'selection');
    },
    ...
    importJson(jsonText) {
      ...
      const nextState = validateState({
        ...imported,
        workspaceMode: imported.workspaceMode === 'ranking' && imported.selectedWorkIds.length === 0
          ? 'selection'
          : imported.workspaceMode,
        savedAt: timestamp(now)
      }, authority);
