...
export function buildCompanyDirectory({
  ...
  for (const work of works) {
    ...
    if (!stat) continue;
    stat.count += 1;
    stat.score += Number(work.median) || 0;
  ...
  const companies = brands.map(brand => {
    ...
    const company = {
      ...
      averageScore: stat.count > 0 ? Math.round((stat.score / stat.count) * 10) / 10 : null,
...
export function worksForCompany(model, companyId, {
  ...
  return model.works
    ...
    .sort((left, right) => {
      const comparison = String(left.work[sortKey] ?? '').localeCompare(
        String(right.work[sortKey] ?? ''), 'ja', { numeric: true, sensitivity: 'base' }
      );
      return comparison === 0 ? left.index - right.index : comparison * sign;
    })
    .map(item => item.work);
