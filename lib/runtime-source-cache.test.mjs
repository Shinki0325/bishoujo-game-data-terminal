...
test('failed optional data can retry without invalidating successful workspace data', async () => {
  let failing = true, stableCalls = 0, now = 0;
  const load = createRuntimeSourceCache({ cryptoRef: webcrypto, requestPolicy: { now: () => now, sleep: async ms => { now += ms; } }, fetchImpl: async url => {
  ...
  failing = false; await assert.rejects(load('broken')); now += 3001;
  await load('broken'); await load('stable'); assert.equal(stableCalls, 1);
});
