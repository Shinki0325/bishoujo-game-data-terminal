...
export function prepareVndbAdmissionsSidecar(value, { catalogSnapshotId, catalogSha256, workIds }) {
  ...
  const admissions = sidecar.works.map((row, index) => {
    ...
    const keys = ['egsWorkId', 'title', 'furigana', 'releaseDate', 'companyId', 'platform', 'median', 'voteCount', 'vndbId', 'admissionSource', 'admissionThreshold', 'thumbnail', 'preview'];
    if (Object.keys(item).some(key => !keys.includes(key)) || Object.keys(item).length !== keys.length) throw new TypeError(`VNDB admissions works[${index}] fields are invalid`);
    if (!WORK_ID.test(item.egsWorkId) || known.has(item.egsWorkId) || seen.has(item.egsWorkId) || typeof item.title !== 'string' || !item.title || !VNDB_ID.test(item.vndbId) || item.admissionSource !== 'vndb-vote-threshold' || JSON.stringify(item.admissionThreshold) !== JSON.stringify({ field: 'vndbVoteCount', operator: '>=', value: 100 })) throw new TypeError(`VNDB admissions works[${index}] identity is invalid`);
