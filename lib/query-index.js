...
export function createQueryIndex({
  ...
  if (searchText !== null) {
    if (!['query-search-text-v1','query-search-text-v2'].includes(searchText.schema) || !Array.isArray(searchText.workIds)
      || searchText.workIds.length !== workSnapshots.length
      || searchText.workIds.some((id, i) => id !== workSnapshots[i].workId)) throw new TypeError('search text identity mismatch');
    if (searchText.schema === 'query-search-text-v2') {
      if (!Array.isArray(searchText.rows) || searchText.rows.length !== workSnapshots.length
      ...
      normalizedTitles=Object.freeze(searchText.rows.map(row=>row[0]));
      normalizedLooseTitles=Object.freeze(searchText.rows.map(row=>row[1]));
      pinyinTitles=Object.freeze(searchText.rows.map(row=>row[2]));
    } else {
      for (const key of ['normalizedTitles','normalizedLooseTitles','pinyinTitles']) {
        if (!Array.isArray(searchText[key]) || searchText[key].length !== workSnapshots.length
          || searchText[key].some(value => typeof value !== 'string')) throw new TypeError('search text format mismatch');
      }
      normalizedTitles = Object.freeze(searchText.normalizedTitles.slice());
      normalizedLooseTitles = Object.freeze(searchText.normalizedLooseTitles.slice());
      pinyinTitles = Object.freeze(searchText.pinyinTitles.slice());
    }
  }
  return Object.freeze({
    ...
    get normalizedTitles() { return normalizedTitles ??= Object.freeze(workSnapshots.map(work => {
      ...
      return [
        buildSearchHaystack(workValues),
        companyValues.map(normalizeTitle).filter(Boolean).join('\n')
      ].filter(Boolean).join('\n');
    })); },
    get normalizedLooseTitles() { return normalizedLooseTitles ??= Object.freeze(workSnapshots.map(work => {
      const aliases = workAliasesById?.get?.(work.workId);
      const pinyin = workPinyinById?.get?.(work.workId);
      const companyAliases = companyAliasesById?.get?.(work.brandId);
      const companyPinyin = companyPinyinById?.get?.(work.brandId);
      ...
      return [
        buildSearchHaystack(workValues, { loose: true }),
        companyValues.map(normalizeLooseTitle).filter(Boolean).join('\n')
      ].filter(Boolean).join('\n');
    })); },
    get pinyinTitles() { return pinyinTitles ??= Object.freeze(workSnapshots.map(work => {
      const aliases = workAliasesById?.get?.(work.workId);
      return buildPinyinSearchHaystack([
        work.title,
        ...(Array.isArray(aliases) ? aliases : [])
      ]);
    })); },
...
export function projectedCountsForIndex(
  ...
  for (const position of yearPositions) {
    const year = Number(String(index.works[position]?.releaseDate ?? '').slice(0, 4));
    if (Number.isInteger(year) && year >= 1987 && year <= 2026) {
  ...
  return { filters, brands, yearCounts };
}
