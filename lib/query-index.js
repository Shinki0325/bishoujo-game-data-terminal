...
function deriveIndexes(works, knownFilterIds) {
  ...
  const companies = {};
  const numeric = { median: [], voteCount: [], releaseDate: [] };
  for (let position = 0; position < works.length; position += 1) {
    const work = works[position];
    for (const filterId of [...work.filterIds, ...work.genreFilterIds]) {
      (facets[filterId] ??= []).push(position);
    }
    (companies[work.brandId] ??= []).push(position);
    numeric.median.push(work.median);
    numeric.voteCount.push(work.voteCount);
...
function formulaMask(index, filterState) {
  const source = filterState.mode === 'advanced'
...
function matchingPositions(index, filterState, selectedWorkIds) {
  ...
  for (let position = 0; position < matches.length; position += 1) {
    ...
    if (
      index.numeric.median[position] < filterState.minimumScore
      || index.numeric.voteCount[position] < filterState.minimumVoteCount
    ) continue;
    ...
    if (normalizedQuery && !index.normalizedTitles[position].includes(normalizedQuery)) continue;
...
function comparePositions(index, leftPosition, rightPosition, filterState) {
  ...
  let comparison;
  if (filterState.sortKey === 'median' || filterState.sortKey === 'voteCount') {
...
export function createQueryIndex({ works, knownFilterIds, brands = [], backendIndexes = null }) {
  ...
  const sourceIndexes = assertBackendIndexes(
  ...
  return Object.freeze({
    ...
    normalizedTitles: Object.freeze(workSnapshots.map(work => normalizeTitle(work.title))),
