...
test('classic tier width is remembered independently by normal and live modes',()=>{
  let saved=null;const io={read:()=>saved,write:(_,s)=>saved=s};const p=createRankingPresentation(io);
  p.setDisplayStyle('classic');p.setUiScale('tierWidth',140);p.setViewMode('live');
  assert.equal(p.inspect().uiScale.tierWidth,140);p.setUiScale('tierWidth',120);
  p.setViewMode('normal');assert.equal(p.inspect().uiScale.tierWidth,140);
  const restored=createRankingPresentation(io);assert.equal(restored.inspect().uiScale.tierWidth,140);
  restored.setViewMode('live');assert.equal(restored.inspect().uiScale.tierWidth,120);
});
test('immersive entry does not force fullscreen, browser fullscreen exit does not leave live, handled Esc is ignored',async()=>{
  ...
  const pressEsc=()=>{const event=new Event('keydown',{cancelable:true});Object.defineProperty(event,'key',{value:'Escape'});doc.dispatchEvent(event);};
  c.revealControls();pressEsc();assert.equal(nodes['ranking-immersive-controls'].hidden,true);assert.equal(classes.size,1);
  doc.querySelector=()=>({});pressEsc();assert.equal(classes.size,1);
  doc.querySelector=()=>null;pressEsc();assert.equal(classes.size,0);
