...
export function parseUiLocationHash(hash) {
  ...
  if (route === 'persons') {
    return { page: 'persons', query: queryValue(params, 'query'), pageNumber: positivePage(params.get('page')), personId: null };
  }
  const personMatch = /^persons\/person\/([^/]+)$/u.exec(route);
  if (personMatch) return { page: 'persons', query: '', pageNumber: 1, personId: safeId(personMatch[1]) };
  return null;
}
