...
function mergeExactCrossSourcePersons(persons) {
  ...
  // A VNDB staff alias can be the exact public credit name used by EGS. Merge
  // this alias path when it resolves to one VNDB staff; shared credited works
  // are evidence retained by the crosswalk, not a merge gate.
  for (const egs of [...persons.values()]) {
    ...
    const candidates = [...persons.values()].filter(person => person !== egs
      && person.sourceRefs.some(ref => ref?.source === 'vndb' && ref?.kind === 'staff')
      && (person.nameVariants ?? []).some(variant => normalizeName(variant?.name) === aliasKey));
