...
function assertEnvelope(value, projection, { base = false } = {}) {
  ...
  if ((!base && value.schemaVersion !== 'terminal-wiki-m2-person-delta-v1') || (base && value.projection !== 'entities') || value.projection !== projection || value.publicationStatus !== 'source-only') throw new TypeError(`M2 ${projection} payload contract mismatch`);
...
function buildState(entityRecords, relationRecords, catalogWorks = []) {
...
export function createM2PersonRuntime({ entitiesUrl, relationsUrl, baseEntitiesUrl = null, manifestUrl = null, catalogWorks = [], fetchImpl = globalThis.fetch, cryptoRef = globalThis.crypto, cacheMode = 'force-cache' } = {}) {
  ...
  async function load() {
    ...
    statePromise = (async () => {
      ...
      const [entities, relations, baseEntities] = await Promise.all([
      ...
      const state = buildState([...baseRecords, ...deltaRecords], assertEnvelope(relations.value, 'relations'), catalogWorks);
