...
function buildState(entityRecords, relationRecords, catalogWorks = [], variantRecords = []) {
  ...
  for (const record of entityRecords) {
    ...
    persons.set(record.entityId, {
      ...
      // M1's EGS creator entities are the established voice-person carrier;
      // retain this source hint even when the corresponding character
      // relation is not present in the M2 delta.
      roleHints: Array.isArray(record.sourceRefs) && record.sourceRefs.some(ref => ref?.source === 'egs' && ref?.kind === 'creator') ? ['voice-actor'] : [],
      credits: []
    });
  }
