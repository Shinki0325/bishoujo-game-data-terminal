...
function buildState(entityRecords, relationRecords, catalogWorks = [], variantRecords = [], basePersonIds = new Set()) {
  ...
  for (const record of entityRecords) {
    ...
    persons.set(record.entityId, {
      ...
      // The M1 person projection is itself the explicit character-voice
      // carrier (its entities were admitted from M1 character-voiced-by
      // relations).  M2 generic EGS creator records are deliberately not
      // treated as voice actors unless they have an explicit voice relation.
      roleHints: basePersonIds.has(record.entityId) ? ['voice-actor'] : [],
      credits: []
    });
  }
  for (const relation of relationRecords) {
    ...
    if (!person) continue;
    // Work credits carry an EGS numeric source id in evidence.  Admission
    // character-voice relations only expose a canonical work entity, so keep
    // that relation (for role filtering and representative-character display)
    // even when a numeric catalog id cannot be resolved locally.
    const workId = isWorkCredit ? workIdFromRelation(relation) : null;
...
export function createM2PersonRuntime({ entitiesUrl, relationsUrl, baseEntitiesUrl = null, baseEntitiesSha256 = M1_PERSON_ENTITIES_SHA256, variantsUrl = null, manifestUrl = null, catalogWorks = [], fetchImpl = globalThis.fetch, cryptoRef = globalThis.crypto, cacheMode = 'force-cache' } = {}) {
  ...
  async function load() {
    ...
    statePromise = (async () => {
      ...
      const basePersonIds = new Set(baseRecords.map(record => record.entityId));
      ...
      const state = buildState([...baseRecords, ...deltaRecords], assertEnvelope(relations.value, 'relations'), catalogWorks, variantRecords, basePersonIds);
