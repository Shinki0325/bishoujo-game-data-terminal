...
try {
  for (const viewport of [
    ...
    if (viewport.name === 'desktop') {
      const firstActivity = page.locator('.person-directory-activity').first();
      ...
      await page.waitForFunction(() => document.querySelector('.person-directory-activity')?.getBoundingClientRect().width >= 300);
      ...
      assert.ok(activityBox?.width >= 300, `desktop: wider activity timeline (${activityBox?.width ?? 0}px; grid ${activityGrid})`);
    }
