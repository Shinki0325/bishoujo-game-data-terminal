...
test('work JSON import is one undoable edit and invalid input is atomic', () => {
  const app = makeController({ resolveWork: id => ({ workId: id, title: id }) });
  app.selectWorks(['1']);
  app.moveToTier('1', 'tier-s', 0);
  const before = app.inspectState();
  const beforeHistory = app.inspect(['1', '2']);
  const imported = createDefaultState('id-controller');
  imported.filterState.titleQuery = 'imported filter';
  imported.tiers[0] = { ...imported.tiers[0], name: 'Imported S' };
  imported.selectedWorkIds = ['2'];
  imported.selectedWorkRefs = [{ workGroupId: 'family', editionWorkId: '2' }];
  imported.tierOrder['tier-s'] = ['2'];
  const importedJson = JSON.stringify(imported);

  assert.deepEqual(app.previewImportJson(importedJson), {
    selectedCount: 1,
    rankedCount: 1,
    unrankedCount: 0,
    tierCount: 5
  });
  assert.equal(app.importJson(importedJson), true);
  assert.deepEqual(app.inspectState().selectedWorkIds, ['2']);
  assert.deepEqual(app.inspectState().tierOrder['tier-s'], ['2']);
  assert.equal(app.inspectState().tiers[0].name, 'Imported S');
  assert.equal(app.inspectState().filterState.titleQuery, 'imported filter');
  assert.equal(app.inspect(['1', '2']).historyPastCount, beforeHistory.historyPastCount + 1);

  assert.equal(app.undo(), true);
  assert.deepEqual(app.inspectState().selectedWorkIds, before.selectedWorkIds);
  assert.deepEqual(app.inspectState().tierOrder, before.tierOrder);
  assert.deepEqual(app.inspectState().tiers, before.tiers);
  assert.equal(app.inspectState().filterState.titleQuery, 'imported filter');
  assert.equal(app.redo(), true);
  assert.deepEqual(app.inspectState().selectedWorkIds, ['2']);
  assert.deepEqual(app.inspectState().tierOrder['tier-s'], ['2']);

  const stateBeforeInvalid = app.inspectState();
  const historyBeforeInvalid = app.inspect(['1', '2']);
  const invalid = JSON.stringify({ ...imported, selectedWorkIds: ['unknown'], selectedWorkRefs: [{ workGroupId: 'unknown', editionWorkId: 'unknown' }] });
  assert.throws(() => app.previewImportJson(invalid));
  assert.throws(() => app.importJson(invalid));
  assert.deepEqual(app.inspectState(), stateBeforeInvalid);
  assert.equal(app.inspect(['1', '2']).historyPastCount, historyBeforeInvalid.historyPastCount);
  assert.equal(app.inspect(['1', '2']).historyFutureCount, historyBeforeInvalid.historyFutureCount);
});

test('work JSON import accepts already-registered custom works in ID-only authority', () => {
  const app = makeController();
  app.registerLocalWorks([{ workId: 'custom-local-import', workGroupId: 'custom-local-import', localMediaKind: 'custom', title: '导入自定义作品' }]);
  const imported = createDefaultState('id-controller');
  imported.selectedWorkIds = ['custom-local-import'];
  imported.selectedWorkRefs = [{ workGroupId: 'custom-local-import', editionWorkId: 'custom-local-import' }];
  imported.tierOrder['tier-a'] = ['custom-local-import'];
  app.importJson(JSON.stringify(imported));
  assert.deepEqual(app.inspectState().selectedWorkIds, ['custom-local-import']);
  assert.deepEqual(app.inspectState().selectedWorkRefs, [{ workGroupId: 'custom-local-import', editionWorkId: 'custom-local-import' }]);
  assert.deepEqual(app.inspectState().tierOrder['tier-a'], ['custom-local-import']);
  app.undo();
  assert.deepEqual(app.inspectState().selectedWorkIds, ['custom-local-import']);
});
