...
export function createFilterWorkerClient(input) {
  ...
  function onMessage(generation, event) {
    ...
    if ((entry.type === 'init' || entry.type === 'update' || entry.type === 'search-text' || entry.type === 'warm-search') && message.type === 'ready') {
      initialized = true;
  ...
  function request(type, payload) {
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      // Metadata may fetch verified media shards, just like initialization;
      // ordinary index queries retain their shorter responsiveness deadline.
