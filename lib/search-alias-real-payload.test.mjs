...
const root = runtimePayloadTestRoot;
...
test('indexes additional frozen Bangumi Chinese titles that were previously absent', () => {
  ...
  // The current reviewed canonical title is 白色相簿2; it takes display
  // precedence over historical source fallback titles.
  assert.equal(enrichmentSource.works.find(work => work.workId === '13255').displayTitle, '白色相簿2');
  assert.deepEqual(search('白色相簿2'), ['13255']);
});
