...
function snapshotWork(value, expectedWorkId, invocation) {
  ...
  return {
    workId,
    title,
    coverPath: coverDescriptor.value,
    coverWidth,
    coverHeight
  };
}
...
export function planTierPng(input) {
  return withSyncBoundary(invocation => {
    ...
    const allowed = new Set(['tiers', 'tierOrder', 'worksById', 'logicalMaxWidth', 'pixelRatio']);
    ...
    for (const geometry of tierGeometry) {
      ...
      for (let index = 0; index < tierOrder[tierId].length; index += 1) {
        ...
        items.push({
          ...
          titleStrip: {
            x,
            y: y + LAYOUT.itemSize - LAYOUT.titleStripHeight,
            width: LAYOUT.itemSize,
            height: LAYOUT.titleStripHeight
          },
  ...
  try {
    ...
    for (const tier of internalPlan.tiers) {
      ...
      try {
        ...
        context.fillStyle = tier.foreground;
      ...
      for (const item of tier.items) {
        ...
        if (cover.missing) {
          ...
          callContext(contextState, 'fillText', [
            'NO COVER',
            item.x + (item.width / 2),
            item.y + ((item.height - LAYOUT.titleStripHeight) / 2)
          ]);
        } else {
