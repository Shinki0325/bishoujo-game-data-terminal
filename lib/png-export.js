...
function snapshotPresentation(value, invocation) {
  ...
  if (hasUiScale) {
    ...
    const allowedScaleKeys = new Set(['overall', 'card', 'rail', 'annotation', 'tierName']);
    if (
      scaleKeys.length !== allowedScaleKeys.size
  ...
  return { showTitles, annotations, display, cardScale: scaleValue('card'), tierScale: scaleValue('tierName'),
...
export function planTierPng(input) {
  return withSyncBoundary(invocation => {
    ...
    const LAYOUT = classic ? { ...DEFAULT_LAYOUT, outerPadding: 0, tierLabelWidth: 100, gap: 0,
