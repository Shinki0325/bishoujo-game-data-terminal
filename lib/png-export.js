...
export function planTierPng(input) {
  return withSyncBoundary(invocation => {
    ...
    const minimumRowHeight = Math.max(0, Math.ceil(
      (logicalMinHeight - 2 * LAYOUT.outerPadding - (tierDefinitions.length - 1) * LAYOUT.gap)
      / tierDefinitions.length
    ));
    ...
    for (const tier of tierDefinitions) {
      ...
      const height = Math.max(minimumRowHeight, (rowCount * itemHeight) + ((rowCount - 1) * LAYOUT.gap));
