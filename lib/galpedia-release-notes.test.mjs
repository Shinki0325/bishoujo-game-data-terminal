...
test('GALPEDIA release notes are a frozen, unique local public-beta record', () => {
  ...
  assert.equal(GALPEDIA_RELEASE_NOTES.length, 2);
  assert.equal(CURRENT_GALPEDIA_RELEASE, GALPEDIA_RELEASE_NOTES[0]);
  assert.equal(CURRENT_GALPEDIA_RELEASE.version, 'v1.0.4-beta');
  assert.equal(CURRENT_GALPEDIA_RELEASE.label, '公测版');
  assert.equal(CURRENT_GALPEDIA_RELEASE.date, '2026-09-07');
  assert.equal(CURRENT_GALPEDIA_RELEASE.releaseId, '20260907-galpedia-v1.0.4-beta');
  assert.equal(GALPEDIA_RELEASE_NOTES[1].version, 'v1.0.3-beta');
  ...
  assert.equal(CURRENT_GALPEDIA_RELEASE.log[0].title, '栏目按需加载');
