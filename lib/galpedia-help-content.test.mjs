...
test('release notes are one local source for the about landing and changelog', () => {
  assert.ok(GALPEDIA_RELEASE_NOTES.length >= 1);
  assert.equal(CURRENT_GALPEDIA_RELEASE, GALPEDIA_RELEASE_NOTES[0]);
  assert.match(CURRENT_GALPEDIA_RELEASE.version, /^v\d+\.\d+\.\d+(?:-[\w.]+)?$/u);
  assert.equal(CURRENT_GALPEDIA_RELEASE.label, '公测版');
  assert.match(CURRENT_GALPEDIA_RELEASE.date, /^\d{4}-\d{2}-\d{2}$/u);
  assert.equal(CURRENT_GALPEDIA_RELEASE.releaseId, `${CURRENT_GALPEDIA_RELEASE.date.replaceAll('-', '')}-galpedia-${CURRENT_GALPEDIA_RELEASE.version}`);
  assert.equal(new Set(GALPEDIA_RELEASE_NOTES.map(item => item.releaseId)).size, GALPEDIA_RELEASE_NOTES.length);
  ...
  assert.ok(searchHelpArticles(CURRENT_GALPEDIA_RELEASE.version).some(item => item.id === 'about.changelog'));
  assert.ok(searchHelpArticles(CURRENT_GALPEDIA_RELEASE.date).some(item => item.id === 'about.changelog'));
});

test('image editing can be discovered by task and is distinct from work annotations', () => {
  for (const query of ['画笔', '打码', '文本框', '自定义贴纸']) assert.ok(searchHelpArticles(query).some(item => item.id === 'tier.stickers'));
  const article = getHelpArticle('tier.stickers');
  assert.match(article.steps.join(''), /⋯.*编辑图片/u);
  assert.match(article.notes.join(''), /作品标注不改封面底图/u);
  assert.doesNotMatch(getHelpArticle('tier.live').notes.join(''), /手机端不提供/u);
});
...
test('records verified live filter commits and sticker gestures', () => {
  ...
  assert.ok(filters.notes.some(note => note.includes('点击“查看结果”即可收起筛选面板')));
