...
test('release notes are one local source for the about landing and changelog', () => {
  ...
  assert.equal(CURRENT_GALPEDIA_RELEASE.version, 'v1.0.1-beta');
  assert.equal(CURRENT_GALPEDIA_RELEASE.label, '公测版');
  assert.equal(CURRENT_GALPEDIA_RELEASE.date, '2026-09-06');
  assert.equal(CURRENT_GALPEDIA_RELEASE.releaseId, '20260906-galpedia-v1.0.1-beta');
  ...
  assert.equal(CURRENT_GALPEDIA_RELEASE.log.length, 5);
  assert.equal(getHelpArticle('about.changelog').sourceRefs[0], CURRENT_GALPEDIA_RELEASE.releaseId);
  assert.ok(searchHelpArticles('v1.0.1-beta').some(item => item.id === 'about.changelog'));
