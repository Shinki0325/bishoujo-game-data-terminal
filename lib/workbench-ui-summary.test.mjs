...
import {createWorkbenchUISummary,validateWorkbenchUISummary,createOwnedWorkbenchUI,projectPersonWorkMetadata} from './workbench-ui-summary.js';
...
test('owned startup only exposes explicit metadata, never full works, aliases or compact bytes',async()=>{
  const ui=structuredClone(createOwnedWorkbenchUI(data));
  assert.equal(ui.schema,'galpedia-owned-ui-v1');
  assert.deepEqual(ui.sample.works,[]);assert.deepEqual(ui.ratedDisplayWorks,[]);
  assert.equal(ui.sampleSource.works,undefined);
  for(const key of ['workAliasesById','workPinyinById','workDisplayTitlesById','searchText'])assert.equal(ui[key],null);
  assert.deepEqual(Object.keys(ui.enrichment).sort(),['companyAliasesById','companyPinyinById']);
  for(const key of ['presentationFamiliesSource','catalogSource','populationContract','brands','confirmedBangumiImportBindings'])assert.deepEqual(ui[key],data[key]);
  const original=data.ratedDisplayWorks.length;
  assert.ok(original>0);assert.equal(data.sampleSource.works.length,original);
  let startup;
  const handle=createWorkbenchWorkerHandler({runtime:createFilterWorkerRuntime(),loadSource:async()=>({data,bytes:new ArrayBuffer(100)}),projectWork:workbenchQueryWork,onData:message=>{startup=structuredClone(message);}});
  await handle({id:1,type:'init',payload:{workbenchSource:source}});
  assert.equal(startup.workbenchBytes,undefined);assert.deepEqual(startup.uiData,ui);
});

test('on-demand metadata exactly preserves aliases, titles and person joining fields',async()=>{
  const handle=makeHandler();await handle({id:1,type:'init',payload:{workbenchSource:source}});
  const selected=data.ratedDisplayWorks.filter((_,i)=>i%29===0);
  const workIds=selected.map(w=>w.workId);
  for(const kind of ['aliases','titles','person']) {
    const reply=await handle({id:2,type:'work-metadata',payload:{workIds:[...workIds,'source-only-unknown'],kind}});
    assert.equal(reply.type,'work-metadata');
    assert.deepEqual(reply.rows,selected.map(work=>kind==='aliases'?{workId:work.workId,aliases:data.workAliasesById?.get(work.workId)??[]}:kind==='titles'?{workId:work.workId,title:work.title}:projectPersonWorkMetadata(work)));
  }
  for(const payload of [{kind:'bad',workIds:[]},{kind:'titles',workIds:[null]},{kind:'titles',workIds:Array(50001).fill('1')}])assert.equal((await handle({id:3,type:'work-metadata',payload})).type,'error');
  assert.deepEqual((await handle({id:4,type:'work-metadata',payload:{workIds:[],kind:'titles'}})).rows,[]);
});
