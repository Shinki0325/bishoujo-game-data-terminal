...
import { createM2PersonRuntime, M1_PERSON_ONLY_ENTITIES_SHA256 } from './m2-person-runtime.js';
import { runtimePayloadTestRoot } from './runtime-payload-test-root.mjs';
import { fileURLToPath } from 'node:url';

const root = fileURLToPath(new URL('data/terminal-wiki-m2-person-source-only-v1/', runtimePayloadTestRoot));
const m1Root = fileURLToPath(new URL('data/terminal-wiki-m1-person-source-only-v1/', runtimePayloadTestRoot));
const bytes = file => file === 'm1-entities.json'
  ? fs.readFileSync(path.join(m1Root, 'entities.json'))
  : file === 'm1-voice-relations.json'
    ? fs.readFileSync(path.join(m1Root, 'voice-relations.json'))
...
assert.equal(stats.personCount, 10168);
...
const naruseMia = state.records.find(item => item.entityId === 'per_000000000c6c');
assert.equal(naruseMia?.canonicalName, '成瀬 未亜');
assert.deepEqual(state.search('成濑未亚').map(item => item.entityId), ['per_000000000c6c']);
assert.equal(state.search('成瀬 未亜').some(item => item.entityId === naruseMia.entityId), true);
assert.equal(state.search('chenglai').some(item => item.entityId === naruseMia.entityId), true);
...
assert.equal(hinano?.sourceRefs.some(ref => ref.source === 'egs' && ref.id === '3360'), true);
assert.equal(state.records.some(item => item.sourceRefs.some(ref => ref.source === 'egs' && ref.id === '3360') && item.entityId !== hinano?.entityId), false);
const aya = state.records.find(item => item.sourceRefs.some(ref => ref.source === 'vndb' && ref.id === 's662'));
assert.equal(aya?.sourceRefs.some(ref => ref.source === 'egs' && ref.id === '5941'), true);
assert.equal(aya?.sourceRefs.some(ref => ref.source === 'vndb' && ref.id === 's13883'), false);
assert.equal(state.records.some(item => item.sourceRefs.some(ref => ref.source === 'egs' && ['11556', '18577'].includes(ref.id))), true);
...
console.log('M2 person runtime checks: 13/13');
