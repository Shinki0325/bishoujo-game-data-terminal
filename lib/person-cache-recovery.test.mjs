...
test('oversized person shards remain complete for callers and can be read again after eviction',async()=>{
  const f=setup({maxEntries:1,maxBytes:1});
  const first=await f.runtime.loadPerson('per_0000000009a6');
  const second=await f.runtime.loadPerson('per_0000000009a6');
  assert.ok(first.credits.length>0);assert.deepEqual(second,first);
  assert.equal(f.requests.length,4); // manifest + index + two complete shard reads
});
