...
test('UI consumer does not permanently disable the person service after one failed shard',async()=>{
  ...
  const start=source.indexOf('async onLoadPerson'),end=source.indexOf('onSelect(personId)',start),block=source.slice(start,end);
  assert.doesNotMatch(block,/personPerformanceRuntime = null/);assert.match(block,/throw error/);
  const directory=source.slice(source.indexOf('async function ensurePersonRuntime'),source.indexOf('async function ensurePersonFilterOptions'));
  assert.doesNotMatch(directory,/personPerformanceRuntime = null/);
  assert.match(directory,/retry the configured source', error\);\s*throw error;/);
});
