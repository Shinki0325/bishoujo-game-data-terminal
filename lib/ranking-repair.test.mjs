...
test('standard PNG has a base canvas, compact is explicit and high quality doubles resolution',()=>{
 ...
 assert.equal(projectPngExport(snapshot).logicalMaxWidth,960);assert.equal(projectPngExport(snapshot).logicalMinHeight,540);
 assert.equal(projectPngExport({...snapshot,exportCanvas:'compact'}).logicalMaxWidth,296);
 assert.equal(projectPngExport({...snapshot,exportCanvas:'compact'}).logicalMinHeight,0);
 assert.equal(projectPngExport(snapshot).pixelRatio,1);assert.equal(projectPngExport({...snapshot,exportQuality:'high'}).pixelRatio,2);
});
