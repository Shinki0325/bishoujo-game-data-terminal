...
test('owned source queries equal old object initialization for aliases, sorting, filters and histograms',async()=>{
 ...
 assert.equal(reply.type,'ready');assert.equal(reply.workbenchBytes,undefined);assert.equal(reply.uiData.schema,'galpedia-owned-ui-v1');assert.deepEqual(reply.uiData.ratedDisplayWorks,[]);
...
test('metadata arrives before index construction without transferring full catalog bytes',async()=>{
 ...
 const handle=createWorkbenchWorkerHandler({runtime,loadSource:async()=>({data,bytes}),projectWork:workbenchQueryWork,onData:message=>{
   events.push('display');const clone=structuredClone(message);assert.equal(clone.workbenchBytes,undefined);assert.equal(bytes.byteLength,8);assert.deepEqual(clone.uiData.sample.works,[]);assert.equal(clone.uiData.workAliasesById,null);
 }});
...
test('early UI metadata cannot bypass family validation or make a failed query window ready',async()=>{
 const events=[];
 const invalid={...data,presentationFamiliesSource:{...data.presentationFamiliesSource,sha256:'0'.repeat(64)}};
 const handle=createWorkbenchWorkerHandler({runtime:{handle(){events.push('index');return {type:'ready'};}},
   loadSource:async()=>({data:invalid}),projectWork:workbenchQueryWork,onData:()=>events.push('metadata')});
 const reply=await handle({id:1,type:'init',payload:{workbenchSource:source}});
 assert.equal(reply.type,'error');assert.match(reply.error.message,/hash/u);
 assert.deepEqual(events,['metadata']);
 const query=await handle({id:2,type:'query',payload:{paged:true,filterState:state}});
 assert.equal(query.type,'error');assert.match(query.error.message,/not been initialized/u);
});
test('ownership defaults on for public works, with legacy and direct-ranking boundaries',()=>{
 const original=globalThis.Worker;globalThis.Worker=function(){};
 try{
   const location={hostname:'127.0.0.1',hash:'#works',search:''};
   assert.equal(ownedWorkbenchEnabled(location),true);
   assert.equal(ownedWorkbenchEnabled({...location,search:'?workerWorkbench=1'}),true);
   assert.equal(ownedWorkbenchEnabled({...location,hostname:'favorite.bishojo.date',search:''}),true);
