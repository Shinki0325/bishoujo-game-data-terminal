  ...
  return async message=>{
    if (message?.type === 'work-metadata') {
      try {
        ...
        if(!['person','aliases','titles'].includes(kind)||!Array.isArray(workIds)||workIds.length>50000||workIds.some(id=>typeof id!=='string'))throw new TypeError('invalid work metadata request');
        const ids=new Set(workIds);
        const rows=owned.data.ratedDisplayWorks.filter(work=>ids.has(work.workId)).map(work=>kind==='person'
          ? projectPersonWorkMetadata(work)
    ...
    try {
      ...
      const uiSummary=replacing?createWorkbenchUISummary(data):null;
      ...
      // window. Query readiness still requires all validation below to succeed.
