...
test('selected working set follows latest selection and is released on disposal',async()=>{
  const f=fixture(), retained=[];f.deps.workData.retainWorkIds=ids=>retained.push([...ids]);
  await f.controller.run(request());
  await f.controller.run(request({state:{workspaceMode:'ranking',selectedWorkIds:['new'],filterState:{}}}));
  f.controller.dispose();assert.deepEqual(retained,[['s'],['new'],[]]);
});
