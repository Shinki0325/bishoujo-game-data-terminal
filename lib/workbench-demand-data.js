  ...
  if (config===WORKBENCH_DEMAND && locationRef===globalThis.location && fetchImpl===globalThis.fetch && cryptoRef===globalThis.crypto) {
    const load = typeof document==='object' && ownedWorkbenchEnabled(locationRef)
      ? async()=>{
        ...
        const {validateWorkbenchUISummary}=await import('./workbench-ui-summary.js');
        validateWorkbenchUISummary(reply.uiSummary,header.manifest.count);
        const payload=attachWorkbenchStore({...decodeWorkbenchPayload(body,header.manifest,{includePersonCatalog:false}),workerOwned:true,uiSummary:reply.uiSummary},header,{fetchImpl,cryptoRef});
...
export function decodeWorkbenchPayload(body,manifest,{includePersonCatalog=true}={}) {
