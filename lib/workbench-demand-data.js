...
export function createWorkbenchStore(manifest,works,{baseUrl,fetchImpl=fetch,cryptoRef=crypto,maxCached=64,concurrency=6,firstPagePromise=null,requestPolicy={}}={}) {
  ...
  // Keep only the selected board's validated records, not their entire shards.
  // This store is scoped to one pinned manifest; another revision gets a new store.
  const retained=new Map();
  let retainedIds=new Set();
  function remember(rows){for(const row of rows)if(retainedIds.has(row.workId))retained.set(row.workId,row);}
  ...
  function bucket(n){
    ...
    const task=run(async()=>{const rows=check(await read(manifest.shards[n]),workIds.slice(n*manifest.blockSize,(n+1)*manifest.blockSize));remember(rows);cache.set(n,rows);while(cache.size>maxCached)cache.delete(cache.keys().next().value);return rows;}).finally(()=>pending.delete(n));
  ...
  return Object.freeze({
    retainWorkIds(ids){
      ...
      for(const id of retainedIds){const row=this.peek(id);if(row)retained.set(id,row);}
    },
    async get(ids){
      ...
      const missing=publicIds.filter(id=>!resolved.has(id));
      if(!missing.length)return resolved;
      if(missing.every(id=>manifest.firstPage.ids.includes(id))){const rows=await first();remember(rows.values());return new Map(publicIds.map(id=>[id,resolved.get(id)??rows.get(id)]));}
      ...
      const rows=new Map(publicIds.map(id=>[id,resolved.get(id)??loaded.get(Math.floor(position.get(id)/manifest.blockSize))[position.get(id)%manifest.blockSize]]));
      remember(rows.values());
      return rows;
    },
    peek(id){return retained.get(id)??firstRows?.get(id)??cache.get(Math.floor(position.get(id)/manifest.blockSize))?.[position.get(id)%manifest.blockSize]??null;},
    ...
    stats(){return {cached:cache.size,pending:pending.size,running,firstPageLoaded:firstRows!==null,retained:retained.size};}
  });
}
...
export async function loadWorkerWorkbenchBundle(source,{config=WORKBENCH_DEMAND,fetchImpl=fetch,cryptoRef=crypto}={}) {
  ...
  const descriptor=deferred?manifest.workerBootstrap:manifest.bootstrap;
  ...
  const data=decodeWorkbenchPayload(body,manifest,{allowDeferredMedia:deferred});
