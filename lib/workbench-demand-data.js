...
export async function readWorkbenchFile(url,hash,{fetchImpl=fetch,cryptoRef=crypto}={}) {
  return JSON.parse(new TextDecoder().decode(await readWorkbenchBytes(url,hash,{fetchImpl,cryptoRef})));
}
async function readWorkbenchBytes(url,hash,{fetchImpl=fetch,cryptoRef=crypto}={}) {
...
export function validateWorkbenchManifest(m,{sourcePins=workbenchSourcePins()}={}) {
  ...
  for(const descriptor of [m.bootstrap,...m.shards,m.firstPage,...(m.searchText===undefined?[]:[m.searchText])]) if(!descriptor||!shaPattern.test(descriptor.sha256)||!/^[a-z0-9-]+\.[a-f0-9]{16}\.json$/u.test(descriptor.path)) throw new TypeError('作品分片清单不完整');
  return m;
}
export function createWorkbenchStore(manifest,works,{baseUrl,fetchImpl=fetch,cryptoRef=crypto,maxCached=64,concurrency=6,firstPagePromise=null}={}) {
  ...
  works=null;
  const position=new Map(workIds.map((id,i)=>[id,i])),cache=new Map(),pending=new Map(),queue=[];
  ...
  function bucket(n){
    ...
    const task=run(async()=>{const rows=check(await read(manifest.shards[n]),workIds.slice(n*manifest.blockSize,(n+1)*manifest.blockSize));cache.set(n,rows);while(cache.size>maxCached)cache.delete(cache.keys().next().value);return rows;}).finally(()=>pending.delete(n));
...
export async function loadWorkbenchData(options={}) {
  ...
  if (config===WORKBENCH_DEMAND && locationRef===globalThis.location && fetchImpl===globalThis.fetch && cryptoRef===globalThis.crypto) {
    const load = typeof document==='object' && ownedWorkbenchEnabled(locationRef)
      ? async()=>{
        const [{loadOwnedWorkbench},header]=await Promise.all([import('./workbench-worker-session.js'),loadWorkbenchHeader({config,fetchImpl,cryptoRef})]);
        const reply=await loadOwnedWorkbench();
        if(reply.manifestSha256!==config.sha256)throw new TypeError('查询线程数据版本不一致');
        // Transitional UI consumers still need the full display model. Transfer
        // its original compact bytes, never clone the expanded object graph.
        const body=JSON.parse(new TextDecoder().decode(reply.workbenchBytes));
        const payload=attachWorkbenchStore({...decodeWorkbenchPayload(body,header.manifest),workerOwned:true},header,{fetchImpl,cryptoRef});
        reply.workbenchBytes=null;
        return payload;
      }
...
export async function loadWorkerWorkbenchBundle(source,{config=WORKBENCH_DEMAND,fetchImpl=fetch,cryptoRef=crypto}={}) {
  ...
  const url=new URL(config.manifestPath,import.meta.url);
  const manifest=validateWorkbenchManifest(await readWorkbenchFile(url,config.sha256,{fetchImpl,cryptoRef}));
  const bytes=await readWorkbenchBytes(new URL(manifest.bootstrap.path,url),manifest.bootstrap.sha256,{fetchImpl,cryptoRef});
  const body=JSON.parse(new TextDecoder().decode(bytes));
  return {data:decodeWorkbenchPayload(body,manifest),bytes};
}
export function decodeWorkbenchPayload(body,manifest) {
  ...
  return {...body.context,searchText:body.searchText??null,confirmedBangumiImportBindings:body.context.bangumiPublicBindings?.bindings??null,sample,ratedDisplayWorks:works,sampleSource:{...body.context.sampleSource,works:works.map(w=>({...w,companyId:w.brandId}))}};
}
function attachWorkbenchStore(payload,{url,manifest,firstPagePromise},{fetchImpl,cryptoRef}) {
  const works=payload.ratedDisplayWorks;
  const workData=createWorkbenchStore(manifest,works,{baseUrl:url,fetchImpl,cryptoRef,firstPagePromise});
  let searchPromise=null;
  const loadSearchText=manifest.searchText ? ()=>searchPromise??=readWorkbenchFile(new URL(manifest.searchText.path,url),manifest.searchText.sha256,{fetchImpl,cryptoRef}).then(carrier=>{snapshotSearchText(carrier,works.map(w=>w.workId));return carrier;}).catch(error=>{searchPromise=null;throw error;}) : null;
