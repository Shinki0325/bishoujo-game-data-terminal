...
async function loadDemandPayload({config,fetchImpl,cryptoRef}) {
  ...
  return {...body.context,searchText:body.searchText??null,loadSearchText,confirmedBangumiImportBindings:body.context.bangumiPublicBindings?.bindings??null,sample,ratedDisplayWorks:works,sampleSource:{...body.context.sampleSource,works:works.map(w=>({...w,companyId:w.brandId}))},workData};
}
