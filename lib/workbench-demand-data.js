...
export function createWorkbenchStore(manifest,works,{baseUrl,fetchImpl=fetch,cryptoRef=crypto,maxCached=64,concurrency=6}={}) {
  ...
  function first(){
    return firstPromise??=(async()=>{const rows=check(await read(manifest.firstPage),manifest.firstPage.ids);firstRows=new Map(rows.map(w=>[w.workId,w]));return firstRows;})().catch(e=>{firstPromise=null;throw e;});
  }
...
export async function loadWorkbenchData({legacyLoader,config=WORKBENCH_DEMAND,locationRef=globalThis.location,fetchImpl=fetch,cryptoRef=crypto}={}) {
  ...
  if(body.schema!==WORKBENCH_SCHEMA||JSON.stringify(body.columns)!==JSON.stringify(WORKBENCH_COLUMNS)||body.rows.length!==manifest.count)throw new TypeError('作品轻量索引格式错误');
  body.context = restoreWorkbenchContext(body.context);
  const works=body.rows.map(row=>{if(row.length!==WORKBENCH_COLUMNS.length)throw new TypeError('作品字段数错误');return Object.fromEntries(WORKBENCH_COLUMNS.map((key,i)=>[key,row[i]]));});
  ...
  const workData=createWorkbenchStore(manifest,works,{baseUrl:url,fetchImpl,cryptoRef});
  return {...body.context,confirmedBangumiImportBindings:body.context.bangumiPublicBindings?.bindings??null,sample,ratedDisplayWorks:works,sampleSource:{...body.context.sampleSource,works:works.map(w=>({...w,companyId:w.brandId}))},workData};
}
