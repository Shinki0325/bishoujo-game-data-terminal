...
const MAX_KNOWN_FILTER_IDS = 49;
const MAX_RAW_GENRE_LENGTH = 512;
const MAX_WORKS = 3000;
...
function snapshotState(state) {
  try {
    ...
    const entries = STATE_FIELDS.map(field => [
      field,
      immutableStateValue(ownDataValue(state, field, 'state'), field)
    ]);
...
function assertWork(work) {
  ...
  if (typeof median !== 'number') {
    throw new TypeError('work.median must be a finite number');
  }
  if (!Number.isFinite(median)) {
    throw new RangeError('work.median must be finite');
  }

  if (typeof voteCount !== 'number') {
    throw new TypeError('work.voteCount must be an integer');
  }
  if (!Number.isInteger(voteCount) || voteCount < 0) {
    throw new RangeError('work.voteCount must be a non-negative integer');
  }
  ...
  return {
    ...
    voteCount,
    brandId,
    filterIds: new Set([...filterIds, ...genreFilterIds])
  };
}
