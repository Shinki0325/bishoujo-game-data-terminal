...
export function createWorkbenchLanding({documentRef=document,locationRef=location,storage=localStorage,isReady,navigate,onVisible}) {
  ...
  let preview=null,style=null,held=[],pending=null;
  function remove() {
    session.suspend();
  ...
  async function show() {
    if(lifetime.disposed||isReady()||preview||!canShowWorkbenchLanding(locationRef,storage))return;
    if(pending)return pending;
    const request=session.begin('first-page');
    pending=(async()=>{
      const payload=await loadWorkbenchLanding();
      if(!payload||!request.isCurrent()||isReady()||!canShowWorkbenchLanding(locationRef,storage))return;
  ...
  return {show,remove,dispose(){remove();session.dispose();lifetime.dispose();}};
}
