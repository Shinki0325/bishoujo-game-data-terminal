...
export function createResourceRequest({
  ...
  return function request(url, { cache = 'force-cache', label = '资料', validationKey = '', validate = bytes => bytes } = {}) {
    ...
    if (pending.has(key)) { diagnostics.event('request-shared'); return pending.get(key); }
    const failure = failures.get(key);
    if (failure && now() < failure.retryAt) { diagnostics.event('request-cooldown', failure); return Promise.reject(failure); }
    diagnostics.event('request-start'); diagnostics.gauge('activeRequests', 1);
    const task = Promise.resolve().then(async () => {
      ...
      for (let i = 0; i < maxAttempts; i++) {
        try {
          ...
          failures.delete(key);
          diagnostics.event('request-ready', { attempt: i + 1 });
          return value;
        } catch (error) {
          last = error;
          ...
          diagnostics.event('request-retry', { attempt: i + 1, kind: error.kind, status: error.status });
          await sleep(delay);
        }
      }
      ...
      diagnostics.event('request-error', last);
      throw last;
    });
