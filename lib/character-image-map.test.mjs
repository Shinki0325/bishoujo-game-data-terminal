...
const projectedWithFallback = applyCharacterImageMapToCharacters([
  { vndbCharacterId: sourceCharacterId, characterName: 'mapped with fallback' }
], prepared, {
  assetBase: 'https://assets.example.test/terminal-wiki/v1/',
  fallbackAssetBase: 'https://raw.example.test/terminal-wiki/v1/'
});
assert.match(projectedWithFallback[0].image.url, /^https:\/\/assets\.example\.test\//u);
assert.match(projectedWithFallback[0].image.fallbackUrl, /^https:\/\/raw\.example\.test\//u);
...
const detailSource = fs.readFileSync(new URL('./work-detail-resources-controller.js', import.meta.url), 'utf8');
...
assert.match(detailSource, /const initialProjection = getProjectRuntime\(\)\?\.projectCredits\?\.\(scopedCredits\);/u);
assert.match(detailSource, /loader\.loadCharacterImages\(work\.workId\)/u);
assert.match(mainSource, /characterAssetFallbackBase: CHARACTER_IMAGE_ASSET_FALLBACK_BASE/u);

console.log('character image map focused checks: 30/30');
