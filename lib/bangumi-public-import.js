...
export function planBangumiPublicImport({
  ...
  for (const [index, rawCollection] of sourceCollections.entries()) {
    ...
    const alreadySelectedWorkIds = workIds.filter(workId => selectedSet.has(workId));
    const selectableWorkIds = workIds.filter(workId => !selectedSet.has(workId));
    for (const workId of selectableWorkIds) {
    ...
    matched.push(Object.freeze({
      ...
      alreadySelectedWorkIds: Object.freeze(alreadySelectedWorkIds),
      selectableWorkIds: Object.freeze(selectableWorkIds)
    }));
  }
  return Object.freeze({
    ...
    mappedWorkCount: plannedWorkIds.length + matched.reduce((count, item) => count + item.alreadySelectedWorkIds.length, 0),
    alreadySelectedWorkCount: matched.reduce((count, item) => count + item.alreadySelectedWorkIds.length, 0),
