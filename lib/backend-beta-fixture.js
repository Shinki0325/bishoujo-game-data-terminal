...
const BACKEND_SCHEMA_VERSIONS = new Set(['egs-tier-beta-v1', 'egs-tier-full-v1']);
...
function assertManifestAsset(work, thumbnail, preview, manifestByWorkId) {
  ...
  const manifestThumbnail = asset.thumbnail ?? asset;
  if (
    manifestThumbnail.url !== thumbnail.url
    || manifestThumbnail.width !== thumbnail.width
    || manifestThumbnail.height !== thumbnail.height
    || manifestThumbnail.sha256 !== thumbnail.sha256
  ) {
    throw new TypeError(`asset manifest mismatch for work ${work.workId}`);
  }
  if (preview !== null) {
    const manifestPreview = assertObject(asset.preview, 'assetsManifest.assets[].preview');
    if (
      manifestPreview.url !== preview.url
      || manifestPreview.width !== preview.width
      || manifestPreview.height !== preview.height
      || manifestPreview.sha256 !== preview.sha256
    ) {
      throw new TypeError(`preview manifest mismatch for work ${work.workId}`);
    }
  }
}
...
function adaptWork(
  ...
  const preview = work.preview === undefined ? null : assertObject(work.preview, 'work.preview');
  assertManifestAsset(work, thumbnail, preview, manifestByWorkId);
  ...
  const previewPath = preview === null ? null : validateRelativeAssetPath(
    requiredString(preview.url, 'work.preview.url'),
    'work.preview.url'
  );
  if (preview !== null && (
    ...
    throw new TypeError('work.preview dimensions must be positive integers');
  }
  if (work.isNukige !== undefined && typeof work.isNukige !== 'boolean') {
    throw new TypeError('work.isNukige must be boolean when present');
  }
  return {
    ...
    coverHeight: height,
    ...(isFullExport ? {
      coverFallback: Boolean(thumbnail.fallback),
      previewPath,
      previewWidth: preview?.width ?? null,
      previewHeight: preview?.height ?? null,
      isNukige: work.isNukige ?? false
    } : {})
  };
}

export function isBackendBetaFixture(candidate) {
  return BACKEND_SCHEMA_VERSIONS.has(candidate?.schemaVersion);
}
...
export function prepareBackendBetaFixture(candidate, {
  ...
  if (!BACKEND_SCHEMA_VERSIONS.has(candidate.schemaVersion)) {
    throw new TypeError('fixture.schemaVersion must be a supported Backend export');
  }
  ...
  return {
    ...
    works: works.map(work => (
      adaptWork(
        ...
        manifestByWorkId,
        workGroupAuthority !== null,
        candidate.schemaVersion === 'egs-tier-full-v1'
      )
    ))
  };
}
