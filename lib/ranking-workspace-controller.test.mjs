...
function fixture() {
  const calls=[],state={tiers:[...DEFAULT_TIERS],tierOrder:{'tier-s':['w']}},company={selectedCompanyIds:['c'],tierOrder:{'tier-s':['c','d']}};
  ...
  const companies={inspect:()=>company,...Object.fromEntries(['moveToTier','moveToCandidates','toggle','undo','redo'].map(k=>[k,record('company:'+k)])),setTiers:tiers=>{calls.push(['company:tiers',tiers]);}};
...
test('bulk company moves preserve order and work bulk moves remain one state operation',()=>{
  ...
  assert.deepEqual(f.calls,[['company:moveToTier','a','tier-s',3],['company:moveToTier','b','tier-s',4]]);
...
test('tier delete observes minimum count, cancellation and active-board occupancy before synchronizing both configs',()=>{
  ...
  assert.match(f.calls[0][1],/2 部/);assert.deepEqual(f.calls.slice(1).map(v=>v[0]),['company:tiers','work:tiers']);assert.equal(f.state.tiers.length,4);
  f.calls.length=0;f.state.tiers=f.state.tiers.slice(0,3);assert.equal(f.actions.deleteTier('tier-a'),false);assert.equal(f.actions.deleteTier('missing'),false);assert.deepEqual(f.calls,[]);
});
test('tier creation preserves palette validation and focus handoff; busy blocks both state owners',()=>{
  const f=fixture();f.actions.addTier();assert.equal(f.state.tiers.length,6);assert.deepEqual(f.calls.map(v=>v[0]),['company:tiers','focus','work:tiers']);assert.equal(f.calls[1][1],f.state.tiers.at(-1).id);
...
test('company model adapts company selection to the original ranking builder without duplicating state',()=>{
  ...
  assert.equal(model,'model');assert.deepEqual(received,[{selectedWorkIds:['c'],tiers:f.state.tiers,tierOrder:f.company.tierOrder},items,'']);assert.deepEqual(f.calls,[]);
});
