...
test('editCrop resolves one document for asset loading and editor opening, and reuses it on re-edit', async () => {
  const f = fixture();
  let loadedDocument;
  let openedDocument;
  f.environment.loadStickerImages = async value => {
    loadedDocument = value;
    return { images: new Map(), release: () => f.log.push(['release-assets']) };
  };
  f.environment.loadEditor = async () => ({ open: async options => {
    openedDocument = options.document;
    return { compositeBlob: new Blob(['composite']), document: options.document };
  }});
  const first = await f.controller.editCrop({ baseBlob: base.blob, width: 64, height: 64 });
  assert.strictEqual(loadedDocument, openedDocument);
  const suppliedDocument = first.stickerDocument;
  const second = await f.controller.editCrop({ baseBlob: first.baseBlob, stickerDocument: suppliedDocument });
  assert.strictEqual(second.stickerDocument, suppliedDocument, 'the supplied document is preserved on re-edit');
  assert.strictEqual(loadedDocument, suppliedDocument);
  assert.strictEqual(openedDocument, suppliedDocument);
});
