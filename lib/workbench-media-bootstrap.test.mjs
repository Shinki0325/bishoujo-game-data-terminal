...
test('owned media is lazy, verified, equivalent to cards and retries a failed media shard',async()=>{
 const calls=[];let fail=false;
 const fetchImpl=async url=>{calls.push(String(url));if(fail&&/cards-|first-page\./u.test(String(url))){fail=false;return new Response('',{status:503});}return fetchSource(url);};
 ...
 fail=true;
 assert.equal((await handler({id:3,type:'work-metadata',payload:{kind:'person',workIds}})).type,'error');
...
test('first-page hint overlaps manifest loading but never bypasses hash or identity validation',async()=>{
 ...
 const loading=loadWorkbenchLanding({config:{...config,firstPage},fetchImpl:async url=>{
   calls.push(String(url));if(String(url).endsWith('/manifest.json'))await gate;return fetchSource(url);
 }});
 assert.ok(calls[0].endsWith(firstPage.path));assert.ok(calls[1].endsWith('/manifest.json'));
 release();assert.deepEqual((await loading).works.map(work=>work.workId),manifest.firstPage.ids);
