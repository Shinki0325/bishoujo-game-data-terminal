...
import {
  ...
  DATA_REVISION
} from './lib/runtime-config.20260823-backend-image-selection-sync-v1.js?v=703b8741f1ee70bf395955d54a1628d4ee01adbdc1fb2a4a8084c591dc77cbea';
...
async function initialize() {
  ...
  if (RUNTIME_FEATURES.authorityFanoutV1.enabled) {
    if (authorityFanoutSource === null) {
      throw new Error('图片选择权威数据未加载；为避免显示过期图片，已停止初始化。');
    }
    try {
      ...
      throw new Error('图片选择权威数据校验失败；为避免显示过期图片，已停止初始化。', { cause: error });
    }
  }
