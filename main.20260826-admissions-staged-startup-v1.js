...
import {
  ...
  DATA_REVISION
} from './lib/runtime-config.20260826-admissions-staged-startup-v1.js?v=99cb3e1e1b99112016499251a9e9bbe89068ac605d12ff3606da31cd67ce7682';
...
export async function fetchStagedRuntimeCoreSources({
  ...
  fetchOptional = fetchOptionalJsonWithSha256,
  ...
  const [catalogSource, admissionsSource] = await Promise.all([
    ...
    fetchOptional(admissionsUrl, 'VNDB admissions sidecar')
  ]);
  let admissions = null;
  if (admissionsSource !== null) {
    try {
      admissions = prepareAdmissions(admissionsSource.value, {
      ...
      warn('VNDB admissions sidecar rejected; loading core fallback carriers', error);
    }
  }
  if (admissions !== null) {
  ...
  const [backendIndexesSource, assetsManifestSource] = await Promise.all([
    fetchRequired(indexesUrl, 'Backend indexes'),
    fetchRequired(assetsManifestUrl, 'assets manifest')
  ]);
  return Object.freeze({
    catalogSource,
    admissions: null,
...
async function initialize() {
  ...
  const [
    ...
    Promise.all([
      ...
      RUNTIME_FEATURES.bangumiPublicBindingsV1.enabled
        ? fetchOptionalJsonWithSha256(DATA_URLS.bangumiPublicBindings, 'Bangumi public bindings carrier')
      ...
      RUNTIME_FEATURES.bangumiRatingsV1.enabled
        ? fetchOptionalJsonWithSha256(DATA_URLS.bangumiRatings, 'Bangumi ratings sidecar')
        : Promise.resolve(null),
      RUNTIME_FEATURES.bangumiCanonicalAliasFallbackV1.enabled
        ? fetchOptionalJsonWithSha256(DATA_URLS.bangumiCanonicalAliasFallback, 'Bangumi canonical alias fallback')
        : Promise.resolve(null),
  ...
  if (bangumiPublicBindingsSource !== null) {
    try {
      ...
      console.warn('Bangumi public bindings carrier rejected; continuing without Bangumi identity', error);
    }
  }
  ...
  if (bangumiRatingsSource !== null && bangumiPublicBindingsSource !== null && bangumiPublicBindings !== null) {
    try {
      ...
      console.warn('Bangumi ratings sidecar rejected; continuing without Bangumi ratings', error);
    }
  }
  ...
  if (bangumiCanonicalAliasFallbackSource !== null && bangumiPublicBindings !== null) {
    try {
      ...
      console.warn('Bangumi canonical alias fallback rejected; retaining existing aliases', error);
    }
  }
  ...
  if (authorityFanoutSource !== null) {
    try {
      ...
      if (authorityFanout.pageBindingIssues.length > 0) {
        console.warn('authority fanout page bindings rejected; media selection remains applied', {
          issueCount: authorityFanout.pageBindingIssues.length,
          issues: authorityFanout.pageBindingIssues
        });
      }
