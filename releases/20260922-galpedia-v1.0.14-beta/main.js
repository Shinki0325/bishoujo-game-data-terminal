import {createAnalysisSiteBridge} from './lib/analysis-site-bridge.js';
import {getStaticCompanyClient as analysisCompanies} from './lib/company-static-client.js';
...
async function initialize() {
  ...
  const staticCompanyPreparation = STATIC_SITE_MODE ? import('./lib/company-static-client.js').then(async ({getStaticCompanyClient})=>{
    const client=getStaticCompanyClient();return {client,directory:await client.loadDirectory()};
  }) : null;
  staticCompanyPreparation?.catch(()=>{});
  ...
  const workbenchQuery = createWorkbenchQueryController({
    ...
    query: (request, options) => filterWorkerClient.query(analysisSite?.request(request)??request, options),
  ...
  const ensureFilterWorker = createLazyResource(() => startupMetrics.measureAsync('filter-worker-init', async () => {
    ...
    // Static pages and admitted remote queries can render independently. The
    // shared local engine initializes only when the query path actually needs it.
  }));
  if (/^#works(?:[/?]|$)/u.test(window.location.hash)) {
    // Prepare the shared query engine alongside the UI when bandwidth permits,
    // but static/remote rendering never waits for it. No text rebuild is run.
    if(!navigator.connection?.saveData&&!/^(?:slow-)?2g$/u.test(navigator.connection?.effectiveType??''))
      void ensureFilterWorker().then(()=>filterWorkerClient.start?.()).catch(() => {});
  ...
  let workspaceScrollRestore = null;
  ...
  const preparedCompanies = await staticCompanyPreparation;
  const staticCompanies = preparedCompanies?.client ?? null;
  ...
  // summary fast path.
  companyDirectory = staticCompanies ? preparedCompanies.directory : fullWikiDirectories ? await companyDirectoryPromise : preparedWorkbench.uiSummary ? restoreCompanySummary(preparedWorkbench.uiSummary.companies) : buildCompanyDirectory({
    brands,
  ...
  function captureWorkspaceScroll() {
    // Selection scroll is recorded by its scroll event. The outer hash router
    // can hide the grid before this callback, clamping scrollTop to its new height.
    if (workspaceHost.activeKey === 'ranking') {
  ...
  function renderWorkspace(model) {
    ...
    if(workspaceHost.activeKey!==key)workspaceScrollRestore=['selection','ranking'].includes(key)?key:null;
    workspaceHost.render({ workspaceMode: model.state.workspaceMode, personDirectoryOpen, companyDirectoryOpen });
  }
  window.addEventListener('scroll',()=>{
    if(workspaceHost.activeKey==='selection'&&workspaceScrollRestore===null
      &&!personDirectoryOpen&&!companyDirectoryOpen&&/^#works(?:[/?]|$)/u.test(window.location.hash)){
      selectionScrollPosition={top:window.scrollY,left:window.scrollX};
    }
  },{passive:true});
  ...
  async function render(visibleBrands = [], interaction = null) {
    ...
    if(workspaceHost.activeKey!==(personDirectoryOpen?'persons':companyDirectoryOpen?'companies':state.workspaceMode))captureWorkspaceScroll();
    ...
    if (personDirectoryOpen) renderPersonDirectory();
    else if (companyDirectoryOpen) await renderCompanyDirectory();
    ...
    if (companyDirectoryOpen || personDirectoryOpen) {
      // The directory owns its own scroll surface and is intentionally not persisted.
    } else if(workspaceScrollRestore===model.state.workspaceMode){
      if(workspaceScrollRestore==='ranking')rankingView?.restoreScroll(rankingScrollPosition);
      else selectionView.restoreScroll(selectionScrollPosition);
      workspaceScrollRestore=null;
    }
  ...
  filterDraft=createFilterDraftSession({
    ...
    async preview(filterState,{visibleBrands=[]},isCurrent){
      ...
      const request={filterState,selectedWorkIds:controller.inspectState().selectedWorkIds,
        visibleBrands,visibleFilterIds:filterView.visibleFilterIds(),companyLimit:6,
        ...(previewPeople?{personWorkIndex:previewPeople}:{})};
      const result=await filterWorkerClient.counts(analysisSite?.request(request)??request);
  ...
  function replaceUiLocation() { if(navigation.update('replaceState'))analysisSite?.preserveScopeLocation(); }
  function pushUiLocation() { if(navigation.update('pushState'))analysisSite?.preserveScopeLocation(); }
  ...
  const navigation = createWorkbenchNavigationController({
    ...
    invalidate() {
      ...
      workbenchQuery.suspend(); filterWorkerClient.cancelQueries?.(); companyWorkspace.suspend(); detailOpening.suspend();
    ...
    companies: {
      enter(route) {
        ...
        openCompanyDirectory(route.companyId, { push: false });analysisSite?.restore(route);
      },
    ...
    persons: {
      enter(route) {
        ...
        personQuery = route.query ?? ''; personRole = route.role ?? 'all'; analysisSite?.restore(route); elements.personSearch.value = personQuery;
      },
    ...
    works: {
      enter(route) {
        ...
        currentWorkDetailId = null;analysisSite?.restore(route);
      },
      setPage: page => {selectionView.setPageNumber(page, { scroll: false, notify: false });analysisSite?.restoreScroll();},
      find: workReference,
      open: work => openWorkDetails(work, { push: false })
    },
    render: () => render(),
    onError: error => console.warn('页面资料暂时不可用，可在当前页面重试。', error)
  });
  analysisSite=createAnalysisSiteBridge({
    resultIds:()=>workbenchQuery.currentResultIds(), refresh:()=>render(),
    snapshot:()=>({url:'/'+window.location.hash,scroll:window.scrollY,filterState:controller.inspectState().filterState,workPage:selectionView.getPageNumber(),personPage:STATIC_SITE_MODE?staticPersonPage:(personDirectoryView.getPageNumber?.()??1),personQuery,personRole,companyQuery,companySort,companyHasImage,companyPage:(companyDirectoryView.getPageNumber?.()??1),companyDetailSortKey,companyDetailSortDirection,companyRole:document.getElementById('company-reviewed-role')?.value,companyScope:document.getElementById('company-reviewed-scope')?.value}),
    restore(back,route){
      if(route.page==='works'){route.pageNumber=back.workPage;controller.setFilterState(back.filterState);selectionView.setPageNumber(back.workPage,{scroll:false,notify:false});}
      if(route.page==='persons'){route.pageNumber=back.personPage;personQuery=back.personQuery;personRole=back.personRole;staticPersonPage=back.personPage;}
      if(route.page==='companies'){route.pageNumber=back.companyPage;companyQuery=back.companyQuery;companySort=back.companySort;companyHasImage=back.companyHasImage;companyDetailSortKey=back.companyDetailSortKey;companyDetailSortDirection=back.companyDetailSortDirection;for(const [id,value]of [['company-reviewed-role',back.companyRole],['company-reviewed-scope',back.companyScope]])if(document.getElementById(id)&&value)document.getElementById(id).value=value;}
    },
    async entity(kind){
      if(kind==='company'){
        const id=selectedCompanyId;if(!id)throw Error('请先打开一家会社。');
        const role=document.getElementById('company-reviewed-role')?.value??'all',scope=document.getElementById('company-reviewed-scope')?.value??'work';
        const label=document.getElementById('company-detail-title').textContent.trim();
        const workIds=await analysisCompanies().workIds(id,{role,scope,sortKey:companyDetailSortKey,direction:companyDetailSortDirection});
        return {kind,id:id+':'+role+':'+scope,entityId:id,label:label+(role==='all'?'':' · '+document.querySelector('#company-reviewed-role option:checked').textContent),workIds:[...new Set(workIds)]};
      }
      const id=selectedPersonId;if(!id)throw Error('请先打开一位人物。');const role=personRole,label=document.getElementById('person-detail-title').textContent.trim();
      const person=STATIC_SITE_MODE?await(await staticPersonServices()).detail.loadPerson(id):await personWorkspace.loadPerson(id);
      const roleOf=c=>c.creditType==='character-voiced-by'?'voice-actor':c.roleCode;
