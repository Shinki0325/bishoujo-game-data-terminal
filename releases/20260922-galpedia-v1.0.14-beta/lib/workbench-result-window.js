...
export function createWorkbenchResultWindow(data) {
  ...
  return Object.freeze({
    project(result, payload) {
      ...
      if(payload.analysisWorkIds !== undefined){
        const subset=payload.analysisWorkIds;
        if(!Array.isArray(subset)||subset.length>50000||subset.some(id=>typeof id!=='string'||!workById.has(id)))throw new TypeError('invalid analysis work scope');
        const ids=new Set(subset),familyIds=new Set(subset.map(id=>families?.familyForWork(id)?.presentationWorkId??id));
        projected=projected.filter(work=>ids.has(work.workId)||familyIds.has(families?.familyForWork(work.workId)?.presentationWorkId??work.workId));
        result={...result,counts:null};
      }
