...
export function densityColor(value,max){
  ...
  const stops=[[247,245,251],[168,143,208],[60,29,112]],n=t<=.5?0:1,u=t<=.5?t*2:(t-.5)*2;
...
export class Chart {
  ...
  density(){
    ...
    groups.forEach((g,i)=>{
      ...
      ctx.globalAlpha=1;ctx.fillStyle='#f7f8fa';ctx.fillRect(left,t,width,b-t);ctx.strokeStyle='#e9e9ef';ctx.lineWidth=.8;ctx.strokeRect(left,t,width,b-t);
      ...
      for(const bin of g.density){
        ...
        if(selected){ctx.strokeStyle='#241236';ctx.lineWidth=1.5;ctx.strokeRect(left,top,width,Math.max(.25,bottom-top));}
