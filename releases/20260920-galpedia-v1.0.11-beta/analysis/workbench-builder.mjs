...
export class WorkbenchBuilder{
  constructor(api,dashboard){
    ...
    const css=node('link');css.rel='stylesheet';css.href=new URL('./workbench-builder.css',import.meta.url).href;document.head.append(css);document.body.classList.add('guided-workbench');
    const skin=node('link');skin.rel='stylesheet';skin.href=new URL('./site-theme.css',import.meta.url).href;document.head.append(skin);
    ...
    for(const [label,hash] of [['作品库','works'],['会社库','companies'],['人物','persons'],['排榜','ranking'],['工作台',null]]){
      const link=node('a',label);link.href=hash?home+'#'+hash:location.pathname;if(!hash)link.setAttribute('aria-current','page');nav.append(link);
    }
  ...
  companyKeys(){const all=[...new Set(this.api.data().rows.map(r=>r.company))];return ['Key','AUGUST',...all].filter((k,i,a)=>all.includes(k)&&a.indexOf(k)===i).slice(0,2);}
  launch(mode='new'){
