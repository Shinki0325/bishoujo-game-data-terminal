...
import {FIELDS,field,DEFAULT,sanitize,analyze,csvCell,assignField,restoreState,chartRows,inScoreView,groupField} from './model.mjs';
...
const INITIAL={...DEFAULT,brushEnabled:innerWidth>720,top:innerWidth<=720?6:12};
...
let returnMatrix=null,dashboard,experience,draftTimer,searchTimer,sessionReady=false,engine,refreshId=0;
...
function selectRows(ids,mode='replace'){
  ...
  selection=next;page=0;updateSelection(true);renderTable();plot.set(result,selection);syncHistory();queueDraft();
}
...
function apply(patch,{record=true,preserveSelection=false}={}){
  ...
  state=sanitize({...state,...patch});if(!preserveSelection)selection.clear();page=0;$('dirty-label').textContent='未保存';
  document.querySelectorAll('.preset').forEach(b=>b.classList.remove('active'));return refresh();
}
function syncControls(){
  selectOptions($('x-field'),['scatter','histogram','ecdf','line'].includes(state.chart)?FIELDS.filter(f=>f.type==='number'):FIELDS,state.x);selectOptions($('y-field'),FIELDS.filter(f=>f.type==='number'),state.y);
  ...
  $('log-control').hidden=state.chart!=='scatter';$('top-control').hidden=!['bar','box','density'].includes(state.chart)||field(state.x).type==='number';$('aggregation-control').hidden=!['bar','line'].includes(state.chart)||state.chart==='line'&&state.lineUnit==='edition';$('aggregation').querySelector('[value=count]').disabled=state.chart==='line';$('bin-control').hidden=state.chart!=='histogram';
  const grouped=['bar','box','density'].includes(state.chart)||!!result.series,categorical=grouped&&field(groupField(state)).type==='category';
  ...
  document.querySelector('[data-shelf=x] label').textContent=['bar','box','density'].includes(state.chart)?'分组依据':'横轴';
...
async function refresh(){
  if(!data)return;
  const ticket=++refreshId;document.body.dataset.computing='true';$('result-summary').textContent='正在更新图表…';
  let next;try{next=await engine.run(state);}catch(error){if(ticket===refreshId){document.body.dataset.computing='false';if(result){state=result.state;syncControls();syncFullUI(result,data);}$('result-summary').textContent=error.message;toast(error.message);}return;}
  if(ticket!==refreshId||!next)return;
  result=next;state=result.state;document.body.dataset.computing='false';syncControls();
  ...
  const groups=['box','bar','density'].includes(state.chart)||!!result.series;
  ...
  dashboard?.render();plot.set(result,selection);$('chart').setAttribute('aria-label',$('chart-title').textContent+'；'+$('chart-subtitle').textContent+'。下方提供对应作品明细。');
  ...
  const legends=plot.colorLegend();$('legend').replaceChildren(...legends.map(v=>{const e=document.createElement(v.groupKey?'button':'span'),dot=document.createElement('i');dot.style.background=v.color;e.append(dot,document.createTextNode(v.label));if(v.groupKey){e.type='button';e.setAttribute('aria-pressed',String(state.brushGroup===v.groupKey));e.title='只在此组内框选';e.addEventListener('click',()=>apply({brushGroup:state.brushGroup===v.groupKey?'':v.groupKey},{preserveSelection:true}));}return e;}));
...
function tableRows(){const rows=representedRows().filter(r=>!selection.size||selection.has(r.id));return rows.sort((a,b)=>sort.direction*((a[sort.key]??-Infinity)-(b[sort.key]??-Infinity))||Number(a.id)-Number(b.id));}
function updateSelection(reveal=false){
  ...
  const rows=representedRows().filter(r=>!selection.size||selection.has(r.id));renderOverview(result,rows,selection.size>0);renderGroupStatistics(result,selection,ids=>{selectRows(ids);$('table-panel').scrollIntoView({behavior:'smooth',block:'start'});});
  renderViewStatus(rows);
  $('table-scope').textContent=selection.size?'当前图形选中的版本；导出明细也会使用这个范围。':'当前图中涉及的版本；点击图形可以缩小到对应作品。';
}
...
function renderTable(){
  const metrics=[...new Set([state.x,state.y])];
  document.querySelectorAll('.active-field-heading').forEach(e=>e.remove());
  const heading=$('table-body').closest('table').querySelector('thead tr');
  for(const key of metrics){const th=document.createElement('th');th.className='active-field-heading';th.textContent=field(key).label;heading.append(th);}
  ...
  $('table-body').replaceChildren(...rows.slice(page*10,page*10+10).map(r=>{
    ...
    a.textContent=r.title;a.title=r.originalTitle;a.href='../../../#works/work/'+encodeURIComponent(r.id);a.target='_blank';a.rel='noopener';small.textContent='EGS '+r.id;title.append(a,small);tr.append(title);
    for(const [value,cls] of [[r.company,''],[String(r.year??'—'),''],[r.median,'score'],[r.votes,'']]){const td=document.createElement('td');td.textContent=typeof value==='number'?format(value):value??'—';td.title=String(value??'');td.className=cls;tr.append(td);}
    for(const key of metrics){const td=document.createElement('td');const value=r[key];td.textContent=Array.isArray(value)?value.join(' / '):typeof value==='number'?format(value):value??'—';if(field(key).character)td.title='本作有效角色数：'+(r.characterValid?.[key]??0);tr.append(td);}return tr;
  }));
...
async function restorePlan(p){
  ...
  if(p.releaseId!==data.releaseId||p.catalogSha256!==data.catalogSha256){if(!await migratePlan(p,data))return;p={...p,state:{...p.state,grain:p.state.grain??'edition'}};}
  await apply(restoreState(p.state));$('plan-name').value=typeof p.name==='string'?p.name.slice(0,80):'';const ids=new Set(representedRows().map(r=>r.id));selection=new Set(Array.isArray(p.selection)?p.selection.filter(id=>typeof id==='string'&&ids.has(id)):[]);
  updateSelection(true);renderTable();plot.set(result,selection);queueDraft();$('dirty-label').textContent='已恢复';toast('已恢复方案与图中选择。');
}
...
function renderFilterChips(){
  const chips=[...(state.minVotes>0?[{text:field(state.voteSource??'votes').label+' ≥ '+state.minVotes,patch:{minVotes:0}}]:[]),
...
function resetFilters(){apply({minVotes:30,yearFrom:1980,yearTo:2030,search:'',tag:''});}
function bind(){
  ...
  for(const f of FIELDS){const b=document.createElement('button');b.className='field-item '+(f.type==='category'?'category':'');b.draggable=true;b.dataset.field=f.id;b.title=f.label+' · '+f.source+(f.multi?' · 一个版本可有多个值':'');b.setAttribute('aria-label','添加字段 '+f.label);
    ...
    b.addEventListener('dragstart',e=>{e.dataTransfer.setData('text/plain',f.id);e.dataTransfer.effectAllowed='copy';});b.addEventListener('click',()=>assign(activeShelf,f.id));$(f.type==='number'?'number-fields':'category-fields').append(b);
  }
  ...
  document.querySelectorAll('[data-chart]').forEach(b=>b.addEventListener('click',()=>{const chart=b.dataset.chart;let x=state.x;
    if(['scatter','histogram','ecdf','line'].includes(chart)&&field(x).type==='category')x=chart==='scatter'?'year':'median';if(chart==='ecdf'&&!['histogram','ecdf'].includes(state.chart))x=state.y;apply({chart,x,brushGroup:'',...(chart==='line'&&state.seriesField==='none'?{seriesField:'company',x:'year',groupMode:'manual',groupSelections:{...state.groupSelections,company:defaultCompanies()}}:{})});}));
  ...
  for(const [id,key] of [['min-votes','minVotes'],['year-from','yearFrom'],['year-to','yearTo'],['tag-filter','tag'],['top-groups','top'],['aggregation','aggregation'],['bin-width','binWidth'],['group-sort','groupSort'],['min-group','minGroup']])$(id).addEventListener('change',e=>apply({[key]:e.target.value}));
  ...
  $('log-x').addEventListener('change',e=>apply({logX:e.target.checked}));$('reset-button').addEventListener('click',resetFilters);$('reset-empty').addEventListener('click',()=>apply({minVotes:30,yearFrom:1980,yearTo:2030,search:'',tag:'',minGroup:1}));
  ...
  for(const key of ['year','median','votes'])$('sort-'+key).addEventListener('click',()=>{sort={key,direction:sort.key===key?-sort.direction:-1};page=0;renderTable();});
  ...
  $('import-file').addEventListener('change',async e=>{const f=e.target.files[0];if(!f)return;if(f.size>1000000){toast('方案文件过大。');e.target.value='';return;}try{restorePlan(JSON.parse(await f.text()));}catch{toast('文件不是有效的 JSON 方案。');}e.target.value='';});
  $('export-button').addEventListener('click',()=>{const rows=tableRows();const exportFields=FIELDS.filter(f=>result.engine.packs.includes(f.pack)&&(!f.character||usedFields(state).includes(f.id)));const columns=['代表版本ID','主作品ID','作品','统计单位','角色数值汇总','角色范围',...exportFields.map(f=>f.label+(f.unit?'（'+f.unit+'）':'')),...exportFields.filter(f=>f.character).map(f=>f.label+'有效角色数'),'数据版本'];
    const content=[columns,...rows.map(r=>[r.id,r.presentationId,r.title,state.grain,state.characterAggregation,state.characterScope,...exportFields.map(f=>Array.isArray(r[f.id])?r[f.id].join(' / '):r[f.id]??''),...exportFields.filter(f=>f.character).map(f=>r.characterValid?.[f.id]??0),data.releaseId])].map(row=>row.map(csvCell).join(',')).join('\r\n');
    download('\uFEFF'+content,'galpedia-analysis-'+rows.length+'-editions.csv','text/csv;charset=utf-8');toast(`已导出 ${rows.length.toLocaleString()} 个版本，范围与明细表一致。`);});
