...
export const DEFAULT = Object.freeze({chart:'scatter',x:'year',y:'median',color:'decade',minVotes:30,
  yearFrom:1980,yearTo:2030,tag:'',search:'',top:12,aggregation:'median',binWidth:5,logX:false,
...
export function sanitize(input={}) {
  ...
  for(const f of FIELDS.filter(f=>f.type==='category')){
    ...
    if(Array.isArray(keys))s.groupSelections[f.id]=[...new Set(keys.filter(k=>typeof k==='string'&&k.length>0&&k.length<=200))].slice(0,12);
  }
  ...
  if(['scatter','histogram','ecdf','line'].includes(s.chart)&&field(s.x).type!=='number')s.x='year';
  return s;
}
// Migrate saved v1 plans at the persistence boundary, never during field assignment.
export function restoreState(input){
  return sanitize({...input,...(input?.chart==='box'&&input?.distribution==='density'?{chart:'density',densityWidth:5}:{})});
}
...
export function assignField(state,channel,id){
  ...
  if(channel==='x'&&['scatter','histogram','ecdf','line'].includes(state.chart)&&f.type!=='number')return {error:'当前图表横轴需要数值。请先选择“密度图”“箱线图”或“分组统计”，再添加分类字段。'};
  return {state:sanitize({...state,[channel]:id})};
}
export function filterRows(rows,s){
  const query=s.search.toLocaleLowerCase();
  return rows.filter(r=>(s.minVotes===0||Number.isFinite(r[s.voteSource??'votes'])&&r[s.voteSource??'votes']>=s.minVotes)&&
    (Number.isFinite(r.year)?r.year>=s.yearFrom&&r.year<=s.yearTo:s.yearFrom===1900&&s.yearTo===2100)&&(!s.tag||r.tags.includes(s.tag))&&
    (!query||[r.title,r.originalTitle,r.company,r.id].some(v=>String(v??'').toLocaleLowerCase().includes(query))));
}
...
export function histogram(rows,key,width){
  ...
  if(!map.size)return [];
  const starts=[...map.keys()],min=Math.min(...starts),max=Math.max(...starts);
...
function analyzeBase(rows,input){
  ...
  if(s.chart==='histogram'){
    ...
    if(numbers.length){const span=Math.max(...numbers)-Math.min(...numbers);width*=Math.max(1,Math.ceil(span/(width*100)-1e-10));}
    ...
    return {state:s,rows:filtered,filteredCount:filtered.length,plottedCount:numbers.length,
      missingCount:filtered.length-numbers.length,bins,actualBinWidth:width};
  }
  const numeric=field(s.x).type==='number';
  const xvalues=filtered.map(r=>r[s.x]).filter(Number.isFinite);
  const xGroupWidth=s.x==='median'?5:s.x==='votes'?niceWidth(xvalues.length?Math.max(...xvalues)-Math.min(...xvalues):1):null;
  ...
  if(s.x==='month')for(let month=1;month<=12;month++)if(!groups.some(g=>g.key===month))groups.push({key:month,ids:[],count:0,...describe([])});
  const byName=(a,b)=>s.x==='birthdayMonth'?parseInt(a.key)-parseInt(b.key):String(a.key).localeCompare(String(b.key),'zh-CN');
  groups.sort(numeric?(a,b)=>a.key-b.key:s.groupSort==='name'?byName:s.groupSort==='median'?(a,b)=>(b.median??-Infinity)-(a.median??-Infinity)||b.count-a.count||byName(a,b):(a,b)=>b.count-a.count||byName(a,b));
  ...
  const eligible=numeric?groups:groups.filter(g=>g.n>=s.minGroup||(s.chart==='bar'&&s.aggregation==='count'&&g.count>=s.minGroup));
  const manual=!numeric&&s.groupMode==='manual';
  ...
  const visible=manual?manualKeys.map(key=>({...rawByKey.get(key)??{key,ids:[],count:0,...describe([])},label:key})):
    numeric?eligible:eligible.slice(0,s.top),ids=new Set(visible.flatMap(g=>g.ids));
  ...
  return {state:s,rows:filtered,filteredCount:filtered.length,plottedCount:ids.size,
    ...
    membershipCount:visible.reduce((n,g)=>n+g.count,0),density,xGroupWidth,groupOrder:manual?'manual':numeric?'numeric':s.groupSort};
}
...
export function analyze(rows,input){
  ...
  return {...result,view,densityCurves,overlay:curveView?null:buildOverlays(result,field(s.chart==='histogram'?s.x:s.y)?.range)};
}
