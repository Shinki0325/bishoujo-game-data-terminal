...
export function mountFullUI(data,apply,dashboard){
  ...
  for(const name of ['作品','评分','标签','会社与制作人员','角色']){
    ...
    for(const f of FIELDS.filter(f=>f.group===name)){
      const coverage=f.character?data.characterEntityCoverage[f.id]:f.coverage;const b=document.querySelector(`[data-field="${f.id}"]`);const badge=document.createElement('small');badge.textContent=coverage.toLocaleString()+(f.character?' 角色':' 版本');b.append(badge);
