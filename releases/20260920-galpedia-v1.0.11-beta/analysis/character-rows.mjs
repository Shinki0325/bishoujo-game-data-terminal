import {analysisGrain,filterRows,plottedFields} from './model.mjs';
...
export function characterRows(core,packs,s,manifest){
  ...
  const works=new Map(filterRows(primary,{...s,search:'',recordFilter:null}).map(r=>[r.presentationId,r]));
  ...
  return {rows,summary:{grain,characterCount:new Set(rows.map(r=>r.characterId)).size,relationCount:qualified.length,workCount:works.size,charFields:fields.map(f=>f.id).filter(id=>plottedFields(s).includes(id))}};
}
