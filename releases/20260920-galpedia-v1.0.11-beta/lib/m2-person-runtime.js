...
function crosswalkMaps(crosswalk) {
  ...
  for (const row of crosswalk?.characters ?? []) {
    if (!row?.workId || !row?.canonicalCharacterEntityId) continue;
    const members = Array.isArray(row.memberCharacterEntityIds) ? row.memberCharacterEntityIds : [row.egsCharacterEntityId];
    for (const member of members) if (member) character.set(`${row.workId}:${member}`, row);
  }
...
function buildState(entityRecords, relationRecords, catalogWorks = [], variantRecords = [], characterRoleRecords = [], namePreferenceRecords = [], crosswalk = null) {
  ...
  for (const relation of relationRecords) {
    ...
    person.credits.push({
      ...
      workEntityId: WORK_ENTITY_ID.test(String(isVoiceCredit ? relation.scope?.workEntityId : relation.subject ?? ''))
        ...
        : null,
      characterId: isVoiceCredit ? String(mappedCharacter?.canonicalCharacterEntityId ?? mappedCharacter?.vndbCharacterEntityId ?? relation.subject ?? '') : null,
      sourceCharacterId,
      characterRole: roleRecord?.role ?? null,
      characterName: isVoiceCredit ? (mappedCharacter?.canonicalName ?? mappedCharacter?.vndbName ?? relation.characterName ?? null) : null,
  ...
  const search = (query = '') => {
    return filterPersonsBySearch(records, query);
  };
...
export function createM2PersonRuntime({ entitiesUrl, relationsUrl, baseEntitiesUrl = null, baseEntitiesSha256 = M1_PERSON_ENTITIES_SHA256, baseRelationsUrl = null, baseRelationsSha256 = M1_PERSON_VOICE_RELATIONS_SHA256, variantsUrl = null, characterRolesUrl = null, namePreferencesUrl = null, crossSourceCrosswalkUrl = null, manifestUrl = null, catalogWorks = [], loadCatalogWorks = null, fetchImpl = globalThis.fetch, cryptoRef = globalThis.crypto, cacheMode = 'force-cache' } = {}) {
  ...
  async function load() {
    ...
    statePromise = (async () => {
      ...
      const resolvedCatalog=loadCatalogWorks?await loadCatalogWorks():catalogWorks;
      if(!Array.isArray(resolvedCatalog))throw new TypeError('person catalog must be an array');
      const state = buildState([...baseRecords, ...deltaRecords], [...baseVoiceRelations, ...assertEnvelope(relations.value, 'relations')], resolvedCatalog, variantRecords, characterRoleRecords, namePreferenceRecords, crosswalk);
