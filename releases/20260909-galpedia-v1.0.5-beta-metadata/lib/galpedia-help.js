...
export function createHelpDrawer() {
  ...
  function articleLink(article) {
    ...
    // Keep the drawer's article identity separate from the boot shell's
    // trigger attribute. The shell owns data-help-article for buttons outside
    // the drawer; sharing it here would intercept the click and lose the
    // drawer's back stack.
    link.dataset.helpLink = article.id;
  ...
  function renderReleaseSummary(release) {
    const meta = node('div', 'help-release-meta');
    meta.dataset.releaseId = release.releaseId;
    const version = node('strong', 'help-release-version', `${release.version} · ${release.label}`);
    const date = node('time', 'help-release-date', `发布日期 ${release.date}`);
    date.dateTime = release.date;
    meta.append(version, date);
    const list = node('ul', 'help-release-summary');
    for (const text of release.summary.slice(0, 3)) list.append(node('li', '', text));
    return [meta, node('h4', '', '本次更新'), list];
  }
  function renderAboutOverview() {
    content.append(node('p', 'help-breadcrumb', '庭守手册 / 关于'));
    const heading = node('h3', '', '关于 GALPEDIA'); heading.tabIndex = -1; content.append(heading);
    const brand = node('section', 'help-about-intro'); brand.dataset.helpAbout = 'overview';
    brand.append(node('p', 'help-about-brand', '少女箱庭 GALPEDIA'));
    brand.append(node('p', 'help-about-copy', getHelpArticle('about.galpedia').summary));
    if (CURRENT_GALPEDIA_RELEASE?.notice) brand.append(node('p', 'help-about-note', CURRENT_GALPEDIA_RELEASE.notice));
    content.append(brand);

    const release = CURRENT_GALPEDIA_RELEASE;
    if (release) {
      const card = node('section', 'help-release-card'); card.dataset.releaseId = release.releaseId;
      card.append(node('p', 'help-release-kicker', '当前版本'));
      const [meta, updateHeading, list] = renderReleaseSummary(release);
      card.append(meta, updateHeading, list);
      const changelog = getHelpArticle('about.changelog');
      if (changelog) {
        const link = button('查看完整更新日志', '查看完整更新日志', () => pushArticle(changelog.id));
        link.className = 'help-release-log-link';
        link.dataset.helpLink = changelog.id;
        link.dataset.helpAction = 'open-changelog';
        card.append(link);
      }
      content.append(card);
    }

    content.append(node('h4', '', '继续阅读'));
    for (const id of ['about.galpedia', 'about.shiori', 'data.sources']) {
      const article = getHelpArticle(id);
      if (article) content.append(articleLink(article));
    }
    renderKeeperPreferences();
  }
  function renderReleaseChangelog() {
    const heading = node('h4', '', '完整更新日志'); content.append(heading);
    const list = node('div', 'help-release-list');
    for (const [index, release] of GALPEDIA_RELEASE_NOTES.entries()) {
      const entry = node('details', 'help-release-entry');
      entry.dataset.releaseId = release.releaseId;
      entry.open = index === 0;
      const summary = node('summary', '', `${release.version} · ${release.label} · ${release.date}`);
      entry.append(summary);
      entry.append(node('h4', '', '更新内容'));
      const logList = node('ul', 'help-release-log');
      for (const item of release.log) {
        const row = node('li');
        row.append(node('strong', '', item.title), node('span', '', item.text));
        logList.append(row);
      }
      entry.append(logList);
      list.append(entry);
    }
    content.append(list);
  }
  function render() {
    ...
    if (!renderKeeperOverviewGuide(article) && article.id !== 'about.changelog') content.append(node('p', 'help-summary', article.summary));
    if (article.id === 'about.changelog') renderReleaseChangelog();
