...
export function createAppController({ sample, catalogAuthority = null, resolveWork = null, localWorks = [], storage, confirm, announce, now, downloadJson }) {
  ...
  function importSummary(nextState) {
    const rankedCount = nextState.tiers.reduce(
      (total, tier) => total + nextState.tierOrder[tier.id].length,
      0
    );
    return Object.freeze({
      selectedCount: nextState.selectedWorkIds.length,
      rankedCount,
      unrankedCount: nextState.selectedWorkIds.length - rankedCount,
      tierCount: nextState.tiers.length
    });
  }

  function prepareImportedState(jsonText) {
  ...
  return Object.freeze({
    ...
    clearCandidates() {
      const ranked = new Set(Object.values(state.tierOrder).flat());
      const remaining = state.selectedWorkIds.filter(id => ranked.has(id));
      const count = state.selectedWorkIds.length - remaining.length;
      if (count === 0) return false;
      if (!confirm(`将移除 ${count} 部未排榜候选作品，已排榜内容保持不变。是否继续？`)) {
        return false;
      }
      return commitEdit(remaining, state.tierOrder);
    },
    ...
    previewImportJson(jsonText) {
      const imported = importState(jsonText, stateAuthority);
      return importSummary(imported);
    },

    importJson(jsonText) {
      // Import replaces the persisted state, but is one edit transaction: the
      // existing work edit history remains available for undo/redo. Non-edit
      // fields (for example filters) are intentionally not part of history.
      const nextState = prepareImportedState(jsonText);
      const nextHistory = commitHistory(history, createEditSnapshot(nextState));
