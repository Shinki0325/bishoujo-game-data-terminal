...
export function createPersonDirectoryView({ root, onSearch, onRoleChange, onSelect, onLoadPerson, onOpenWork, onOpenPerson, onOpenCompany, imageUrlForWork } = {}) {
  ...
  function renderDetail(person) {
    ...
    const activityYears = Array.isArray(person.activityYears) ? person.activityYears : [];
    ...
    frequencyNote.append(node(documentRef, 'span', 'person-detail-muted', person.spanLabel ?? '日期未知'), node(documentRef, 'span', 'person-detail-muted', yearsActive ? `年均 ${(person.workCount / yearsActive).toFixed(1)} 部` : '年均未知'));
  ...
  function render() {
    ...
    for (const person of visible) {
      ...
      row.append(faces, cell, metrics, node(documentRef, 'span', 'person-directory-role', roleLabel(primaryRole)), activity, node(documentRef, 'span', 'person-directory-span', person.spanLabel ?? '日期未知'));
  ...
  return Object.freeze({
    ...
    filter(query = '') {
      ...
      return model.filter(person => (person.searchKey ?? [person.canonicalName, ...(person.aliases ?? []), ...(person.nameVariants ?? []).map(item => item.name), ...(person.nameVariants ?? []).map(item => item.latin)].map(normalizePersonSearch).join(' ')).includes(needle));
    },
