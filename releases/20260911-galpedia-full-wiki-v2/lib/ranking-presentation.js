...
export const UI_SCALE_KEYS = Object.freeze(['overall', 'card', 'rail', 'annotation', 'tierName', 'tierWidth']);
...
export function createRankingPresentation({ read, write, storageKey = RANKING_UI_KEY }) {
  ...
  const modeProfiles = {};
  let viewMode = 'normal';
  const snapshotMode = () => ({ showCounts, showTitles, uiScale: cloneUiScale(uiScale) });
  function restoreMode(profile) {
    if (!profile) return;
    showCounts = profile.showCounts === true; showTitles = profile.showTitles === true;
    for (const key of UI_SCALE_KEYS) uiScale[key] = normalizeUiScale(profile.uiScale?.[key] ?? 100);
  }
  try {
    const parsed = JSON.parse(read(storageKey) ?? 'null');
    if (parsed !== null && typeof parsed === 'object' && !Array.isArray(parsed)) {
      if (parsed.showCounts === true) showCounts = true;
      for (const style of ['garden', 'classic']) for (const mode of ['normal', 'live']) {
        const profile = parsed.modeProfiles?.[`${style}:${mode}`];
        if (profile && typeof profile === 'object') modeProfiles[`${style}:${mode}`] = {
          showCounts: profile.showCounts === true, showTitles: profile.showTitles === true,
          uiScale: Object.fromEntries(UI_SCALE_KEYS.map(key => [key, normalizeUiScale(profile.uiScale?.[key] ?? 100)]))
        };
      }
      if (typeof parsed.showTitles === 'boolean') showTitles = parsed.showTitles;
      if (parsed.display?.style === 'classic') display.style = 'classic';
      if (['square', 'portrait', 'landscape', 'contain'].includes(parsed.display?.shape)) display.shape = parsed.display.shape;
      for (const style of ['garden', 'classic']) {
        const profile = parsed.displayProfiles?.[style];
        if (profile && typeof profile === 'object') displayProfiles[style] = {
          showCounts: profile.showCounts === true, showTitles: profile.showTitles === true,
          shape: ['square', 'portrait', 'landscape', 'contain'].includes(profile.shape) ? profile.shape : 'square',
          uiScale: Object.fromEntries(UI_SCALE_KEYS.map(key => [key, normalizeUiScale(profile.uiScale?.[key] ?? 100)]))
        };
      }
  ...
  restoreMode(modeProfiles[`${display.style}:normal`]);

  let immersive = false;

  function persist() {
    modeProfiles[`${display.style}:${viewMode}`] = snapshotMode();
    try {
      write(storageKey, JSON.stringify({
        showCounts,
        showTitles,
        annotations: cloneAnnotations(annotations),
        display, displayProfiles, modeProfiles,
  ...
  return Object.freeze({
    inspect,
    setViewMode(mode) {
      if (!['normal', 'live'].includes(mode)) throw new TypeError('unknown view mode');
      if (mode === viewMode) return inspect();
      modeProfiles[`${display.style}:${viewMode}`] = snapshotMode();
      viewMode = mode;
      // First visit inherits the current view. Afterwards the modes are independent.
      restoreMode(modeProfiles[`${display.style}:${viewMode}`]);
      persist(); return inspect();
    },
    syncViewMode() {
      modeProfiles[`${display.style}:${viewMode === 'normal' ? 'live' : 'normal'}`] = snapshotMode();
      persist(); return inspect();
    },
    setDisplayStyle(style) {
      if (!['garden', 'classic'].includes(style)) throw new TypeError('unknown display style');
      if (style === display.style) return inspect();
      modeProfiles[`${display.style}:${viewMode}`] = snapshotMode();
      displayProfiles[display.style] = { showCounts, showTitles, uiScale: cloneUiScale(uiScale), shape: display.shape };
      const saved = displayProfiles[style];
      showCounts = saved?.showCounts === true;
      showTitles = saved?.showTitles === true;
      for (const key of UI_SCALE_KEYS) uiScale[key] = normalizeUiScale(saved?.uiScale?.[key] ?? 100);
      display = { style, shape: ['square', 'portrait', 'landscape', 'contain'].includes(saved?.shape) ? saved.shape : 'square' };
      restoreMode(modeProfiles[`${style}:${viewMode}`] ?? modeProfiles[`${style}:normal`]);
      modeProfiles[`${style}:normal`] ??= snapshotMode();
      persist(); return inspect();
    },
    setDisplayShape(shape) {
      if (!['square', 'portrait', 'landscape', 'contain'].includes(shape)) throw new TypeError('unknown display shape');
      display.shape = shape; persist(); return inspect();
    },
    setDensity(density) {
      const size = { compact: 80, standard: 100, spacious: 130 }[density];
      if (!size) throw new TypeError('unknown density');
      // A preset has a single size scale; old custom compound scales remain restorable per style.
      uiScale.overall = 100; uiScale.rail = 100; uiScale.card = size;
      persist(); return inspect();
    },
    saveDisplay: persist,
    setShowCounts(value) {
    ...
    setUiScale(key, value, { persistChange = true } = {}) {
...
export function createImmersiveController({ root, documentRef, idleMs = 3000, onBeforeChange = () => {}, onChange = () => {} }) {
  ...
  const toggle = documentRef.getElementById('ranking-live-toggle');
  const fullscreenButton = documentRef.getElementById('ranking-live-fullscreen');
  ...
  function deactivate() {
    ...
    if (wasImmersive) onBeforeChange(false);
  ...
  async function enter() {
    if (root.classList.contains('is-ranking-immersive')) return;
    onBeforeChange(true);
  ...
  function syncFullscreen() {
    const full = documentRef.fullscreenElement === root;
    if (fullscreenButton) {
      fullscreenButton.textContent = full ? '退出全屏' : '全屏';
      fullscreenButton.setAttribute('aria-pressed', String(full));
    }
  }
  fullscreenButton?.addEventListener('click', async () => {
    try {
      if (documentRef.fullscreenElement === root) await documentRef.exitFullscreen?.();
      else await root.requestFullscreen?.();
    } catch { if (fullscreenButton) fullscreenButton.textContent = '全屏不可用'; }
  });

  documentRef.addEventListener('keydown', event => {
    if (event.key !== 'Escape' || event.defaultPrevented || !root.classList.contains('is-ranking-immersive')) return;
    if (typeof documentRef.querySelector === 'function'
      && documentRef.querySelector('dialog[open], #display-menu:not([hidden])')) return;
    event.preventDefault?.();
    if (controls.hidden === false) { hideControls(true); toggle?.focus?.(); return; }
    void exit();
  });
  ...
  toggle?.addEventListener('click', () => controls.hidden ? revealControls() : hideControls(true));
