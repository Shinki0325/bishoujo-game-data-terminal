...
export function createCommandSearch({ ensureRuntime, navigate, beforeOpen = () => {} }) {
  ...
  const queries = createWorkspaceSession();
  ...
  async function run() {
    if (lifetime.disposed || composing || !dialog.open) return;
    ...
    try {
      ...
      if (!token.isCurrent() || !dialog.open || input.value.trim() !== query) return;
      let count = 0;
      function appendOption(section, name, href, item = null) {
        ...
        token.scope.listen(link, 'click', event => {
          if (event.ctrlKey || event.metaKey || event.shiftKey || event.altKey || event.button !== 0) return;
          event.preventDefault(); enter(link);
        });
        token.scope.listen(link, 'pointermove', () => { if (active !== options.indexOf(link)) select(options.indexOf(link)); });
  ...
  lifetime.listen(document.getElementById('global-search-open'), 'click', () => open());
  lifetime.listen(document.getElementById('home-search-form'), 'submit', event => { event.preventDefault(); open(document.getElementById('home-search-input').value); });
  lifetime.listen(document.getElementById('global-search-form'), 'submit', event => {
