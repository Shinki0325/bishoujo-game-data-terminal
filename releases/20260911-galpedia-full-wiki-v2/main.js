...
import { loadPersonDisplayNames } from './lib/full-wiki-person-names.js?v=20260911v2';
...
async function initialize() {
  ...
  const fullWikiEnabled = FULL_WIKI_RUNTIME.enabled && Boolean(preparedWorkbench.fullWiki);
  ...
  if (fullWikiEnabled) {
    const {createFullWikiDirectories} = await import('./lib/full-wiki-directories-v2.js?v=20260911v2');
    const {createPersonCastLoader} = await import('./lib/full-wiki-person-cast-v2.js?v=20260911v2');
  ...
  } else if (RUNTIME_FEATURES.personDirectoryV1?.enabled === true) {
  ...
  const workDetailResources = createWorkCreditsController({
    ...
    ensureProjectRuntime: fullWikiEnabled ? async () => null : projectResources.ensureRuntime,
    getProjectRuntime: () => fullWikiEnabled ? null : projectResources.current,
    loadIdentityCrosswalk: fullWikiEnabled ? async () => null : projectResources.loadIdentityCrosswalk,
