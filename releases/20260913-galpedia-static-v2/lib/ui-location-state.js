...
const PERSON_ROLES = new Set(['all', 'voice-actor', 'scenario', 'artwork', 'music']);
...
export function parseUiLocationHash(hash) {
  ...
  const workMatch = /^(?:work|works\/work)\/([^/]+)$/u.exec(route);
  if (workMatch) {
  ...
  if (companyMatch) {
    return { page: 'companies', query: '', sort: 'totalVoteCount-desc', hasImage: true, pageNumber: 1, companyId: safeCompanyId(companyMatch[1]) };
  }
  if (route === 'ranking') {
    return { page: 'ranking', subject: params.get('subject') === 'company' ? 'company' : 'work' };
  }
  if (route === 'persons') {
    const role = queryValue(params, 'role');
...
export function formatUiLocationHash(state) {
  ...
  if (safeId(state.workId)) return `#work/${state.workId}`;
  const params = new URLSearchParams();
  const query = limitedText(state.query);
