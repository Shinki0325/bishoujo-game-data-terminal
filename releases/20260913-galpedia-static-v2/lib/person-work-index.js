 ...
 */
function validatePersonWorkIndexHeader(value) {
...
function normalizePersonRoles(personId, sourceRoles) {
    ...
    for (const [role, sourceWorkIds] of Object.entries(sourceRoles)) {
      // Source credit labels are extensible; the filter's public role choices
      // are a separate, bounded taxonomy. Keep every source relationship for
      // the "all" query instead of rejecting an entire valid source index.
      if (!role.trim() || role.length > 128 || ['__proto__', 'constructor', 'prototype'].includes(role)) {
        throw new TypeError(`personWorkIndex role ${role} is invalid`);
      }
...
function finishPersonWorkIndex(value, persons) {
return Object.freeze({
    format: PERSON_WORK_INDEX_FORMAT,
...
export function normalizePersonWorkIndex(value) {
  validatePersonWorkIndexHeader(value);
  const persons = {};
  for (const [personId, sourceRoles] of Object.entries(value.persons)) {
    const roles = normalizePersonRoles(personId, sourceRoles);
    if (roles) persons[personId] = roles;
  }
  return finishPersonWorkIndex(value, persons);
}

export async function normalizePersonWorkIndexAsync(value) {
  // Keep the worker's synchronous API; the UI validates the same contract in
  // bounded batches before structured-cloning the acknowledged projection.
  ...
  for (const [personId, sourceRoles] of Object.entries(value.persons)) {
    const roles = normalizePersonRoles(personId, sourceRoles);
    if (roles) persons[personId] = roles;
    if ((++count & 127) === 0 && performance.now() - started >= 8) {
      await yieldMainThread(); started = performance.now();
    }
  }
  const result = finishPersonWorkIndex(value, persons);
  await yieldMainThread();
  return result;
}
