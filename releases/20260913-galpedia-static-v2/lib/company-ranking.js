...
export function createCompanyRanking({ companies, tiers, storage = null }) {
  ...
  const companyAliases = new Map(companies.flatMap(company => (company.companyIdAliases??[]).map(id=>[id,company.companyId])));
  ...
  try {
    ...
    let parsed = typeof stored === 'string' ? JSON.parse(stored) : null;
    if(parsed && Array.isArray(parsed.selectedCompanyIds) && parsed.tierOrder && typeof parsed.tierOrder==='object') {
      const canonical=id=>companyAliases.get(id)??id;
      parsed={...parsed,selectedCompanyIds:parsed.selectedCompanyIds.map(canonical),
        tierOrder:Object.fromEntries(Object.entries(parsed.tierOrder).map(([tier,ids])=>[tier,Array.isArray(ids)?ids.map(canonical):ids]))};
    }
