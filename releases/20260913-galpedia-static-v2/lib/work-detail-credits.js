...
const CHARACTER_IMAGE_INDEX_SCHEMA_VERSION = 'terminal-wiki-character-image-shards-v1';
const CHARACTER_IMAGE_SHARD_SCHEMA_VERSION = 'terminal-wiki-character-image-shard-v1';
const ID_PATTERN = /^[1-9][0-9]*$/u;
const SHA256_PATTERN = /^[a-f0-9]{64}$/u;
const SHARD_PATH_PATTERN = /^shards\/[0-9]{3,}\.json$/u;
const CHARACTER_IMAGE_SHARD_PATH_PATTERN = /^character-images\/[0-9]{3,}\.json$/u;
...
function prepareIndex(value, binding) {
  ...
  const descriptors = index.buckets.map((candidate, bucketIndex) => {
    ...
    let characterImageShard = null;
    if (descriptor.characterImageShard !== undefined && descriptor.characterImageShard !== null) {
      const imageDescriptor = assertObject(
        descriptor.characterImageShard,
        `work-detail credits buckets[${bucketIndex}].characterImageShard`
      );
      if (
        typeof imageDescriptor.path !== 'string'
        || !CHARACTER_IMAGE_SHARD_PATH_PATTERN.test(imageDescriptor.path)
        || imageDescriptor.path !== `character-images/${descriptor.bucketId}.json`
      ) {
        throw new TypeError(`work-detail credits buckets[${bucketIndex}].characterImageShard.path is unsafe`);
      }
      characterImageShard = Object.freeze({
        bucketId: descriptor.bucketId,
        path: imageDescriptor.path,
        bytes: assertPositiveInteger(imageDescriptor.bytes, `work-detail credits buckets[${bucketIndex}].characterImageShard.bytes`),
        sha256: assertSha256(imageDescriptor.sha256, `work-detail credits buckets[${bucketIndex}].characterImageShard.sha256`)
      });
    }
  ...
  const imageBinding = index.characterImages === undefined
    ? null
    : assertObject(index.characterImages, 'work-detail credits characterImages');
  if (binding.characterImageMapSnapshotId !== undefined) {
    if (imageBinding === null || imageBinding.schemaVersion !== CHARACTER_IMAGE_INDEX_SCHEMA_VERSION) {
      throw new TypeError('work-detail character image shard binding is unavailable');
    }
    if (imageBinding.snapshotId !== binding.characterImageMapSnapshotId) {
      throw new TypeError('work-detail character image snapshot does not match the runtime pin');
    }
    if (imageBinding.sourceMapSha256 !== binding.characterImageMapSha256) {
      throw new TypeError('work-detail character image map hash does not match the runtime pin');
    }
    if (imageBinding.sourceAliasMapSha256 !== binding.characterImageAliasMapSha256) {
      throw new TypeError('work-detail character image alias hash does not match the runtime pin');
    }
    const imageShardCount = descriptors.filter(descriptor => descriptor.characterImageShard !== null).length;
    if (imageBinding.shardCount !== imageShardCount) {
      throw new TypeError('work-detail character image shard count does not match the index');
    }
  }
  return Object.freeze({
    ...
    descriptorsByWorkId,
    characterImages: imageBinding === null ? null : Object.freeze({ ...imageBinding })
  });
}
...
export function validateWorkDetailCreditsShard(value, descriptor) {
  if (!descriptor || typeof descriptor.bucketId !== 'string') throw new TypeError('work-detail shard descriptor is required');
...
async function fetchJsonBytes(url, label, { requestResource, cacheMode, prepare = value => value }) {
  return requestResource(url, { cache: cacheMode, label, validate: async bytes => {
    let value;
    try {
...
export function createWorkDetailCreditsLoader({
  ...
  requestPolicy = {},
  characterImageMapSnapshotId,
  characterImageMapSha256,
  characterImageAliasMapSha256
}) {
  ...
  const characterImageShardPromises = new Map();

  function loadIndex() {
    if (indexPromise !== null) return indexPromise;
    indexPromise = fetchJsonBytes(indexUrl, '作品制作资料目录', { requestResource, cacheMode, prepare: ({ value }) => prepareIndex(value, binding) })
      .catch(error => {
        indexPromise = null;
        throw error;
      });
  ...
  function loadShard(descriptor) {
    ...
    const pending = fetchJsonBytes(
      ...
      { requestResource, cacheMode, prepare: async ({ bytes, value }) => {
  ...
  function loadCharacterImageShard(descriptor) {
    const imageDescriptor = descriptor.characterImageShard;
    if (imageDescriptor === null) return Promise.resolve(null);
    const cached = characterImageShardPromises.get(descriptor.bucketId);
    if (cached !== undefined) return cached;
    const shardUrl = new URL(imageDescriptor.path, indexUrl);
    shardUrl.search = indexUrl.search;
    const pending = fetchJsonBytes(
      shardUrl,
      `角色图片分片 ${descriptor.bucketId}`,
      { requestResource, cacheMode, prepare: async ({ bytes, value }) => {
      if (bytes.byteLength !== imageDescriptor.bytes) {
        throw new Error(`角色图片分片 ${descriptor.bucketId} 大小校验失败`);
      }
      if (await sha256Hex(bytes, cryptoRef) !== imageDescriptor.sha256) {
        throw new Error(`角色图片分片 ${descriptor.bucketId} 完整性校验失败`);
      }
      const shard = assertObject(value, `character image shard ${descriptor.bucketId}`);
      if (shard.schemaVersion !== CHARACTER_IMAGE_SHARD_SCHEMA_VERSION || shard.bucketId !== descriptor.bucketId) {
        throw new TypeError(`角色图片分片 ${descriptor.bucketId} 契约不匹配`);
      }
      if (
        !Array.isArray(shard.workIds)
        || JSON.stringify(shard.workIds) !== JSON.stringify(descriptor.workIds)
      ) {
        throw new TypeError(`角色图片分片 ${descriptor.bucketId} 作品范围不匹配`);
      }
      if (
        shard.sourceMapSha256 !== characterImageMapSha256
        || shard.sourceAliasMapSha256 !== characterImageAliasMapSha256
        || shard.snapshotId !== characterImageMapSnapshotId
      ) {
        throw new TypeError(`角色图片分片 ${descriptor.bucketId} 来源绑定不匹配`);
      }
      return prepareCharacterImageMap(shard.imageMap, {
        snapshotId: characterImageMapSnapshotId,
        aliases: shard.imageAliases,
        sourceMapSha256: characterImageMapSha256
      });
    } }).catch(error => {
      characterImageShardPromises.delete(descriptor.bucketId);
      throw error;
    });
    characterImageShardPromises.set(descriptor.bucketId, pending);
    return pending;
  }
  ...
  return Object.freeze({
    ...
    async loadCharacterImages(workId) {
      if (typeof workId !== 'string' || !ID_PATTERN.test(workId)) return null;
      const index = await loadIndex();
      const descriptor = index.descriptorsByWorkId.get(workId);
      if (descriptor === undefined) return null;
      return loadCharacterImageShard(descriptor);
    }
  });
}
