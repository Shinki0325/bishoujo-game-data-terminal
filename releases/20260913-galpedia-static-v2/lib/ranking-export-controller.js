...
export function projectPngExport(snapshot) {
  ...
  const labelWidth=100 * (snapshot.presentation?.uiScale?.tierWidth ?? 100) / 100;
  const compactWidth=classic ? Math.max(320, Math.min(2048, Math.round(labelWidth + Math.min(high ? 11 : 8, largestRow) * cardWidth)))
    : 128 + Math.min(high ? 11 : 8, largestRow) * 168;
  const baseCanvas=snapshot.exportCanvas!=='compact';
  return { tiers: snapshot.state.tiers, tierOrder: snapshot.tierOrder, worksById, presentation: snapshot.presentation,
    logicalMaxWidth: baseCanvas ? Math.max(960,compactWidth) : compactWidth,
    logicalMinHeight: baseCanvas ? 540 : 0, pixelRatio: high ? 2 : 1 };
}
