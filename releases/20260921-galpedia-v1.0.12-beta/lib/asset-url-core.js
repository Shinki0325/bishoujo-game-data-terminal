...
export function resolveAssetUrl(relativePath, assetBase = DEFAULT_ASSET_BASE, lookupFullWikiPublicMediaUrl = null) {
  ...
  if (lookupFullWikiPublicMediaUrl && path.startsWith('data/terminal-wiki-media-v1/') && !(isLocalPreviewOrigin(globalThis.location?.origin) && new URLSearchParams(globalThis.location?.search ?? '').get('localMedia') === '1')) {
...
// Static person records carry approved public cover object URLs. Use the same
// delivery endpoint as resolveAssetUrl's approved public object path branch.
export function resolvePublicCoverUrl(urlValue) {
  const path = externalV2ObjectPath(urlValue, PRIMARY_COVER_ASSET_BASE);
  // The unavailable-cover placeholder was not included in the mirror delivery.
  return path === null || path === V2_FALLBACK_PATH ? urlValue : new URL(path, PUBLIC_COVER_MIRROR.base).href;
}
...
export function resolveWorkImageUrl(work, assetBase = DEFAULT_ASSET_BASE, lookupPublicMediaUrl = null) {
  return resolveAssetUrl(assetPathFromWork(work), assetBase, lookupPublicMediaUrl);
}

export function applyImageAsset(image, work, assetBase = DEFAULT_ASSET_BASE, lookupPublicMediaUrl = null) {
  if (image === null || typeof image !== 'object') {
    throw new AssetUrlError('image must be an object', { path: 'image' });
  }
  image.crossOrigin = 'anonymous';
  image.referrerPolicy = 'no-referrer';
  image.src = resolveWorkImageUrl(work, assetBase, lookupPublicMediaUrl);
  return image;
}
