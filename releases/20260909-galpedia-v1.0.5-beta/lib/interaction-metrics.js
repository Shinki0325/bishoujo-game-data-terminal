...
export function createInteractionMetrics({
  ...
  function releaseMarks(record) {
    for (const name of record.marks.keys()) performanceRef?.clearMarks?.(`egs-interaction:${record.id}:${record.kind}:${name}`);
  }
  ...
  function begin(kind) {
    ...
    records.push(record);
    if (records.length > MAX_RECORDS) releaseMarks(records.shift());
  ...
  function clear() {
    for (const record of records) releaseMarks(record);
    records.length = 0;
    active.clear();
  }
