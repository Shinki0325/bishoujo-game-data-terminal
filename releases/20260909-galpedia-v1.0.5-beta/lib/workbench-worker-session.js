...
export function loadOwnedWorkbench() {
  return loaded??=new Promise((resolve,reject)=>{
    diagnostics.event('worker-loading');
    receiveData=value=>{diagnostics.event('worker-ready');resolve(value);};
    getOwnedWorkbenchClient().init(workbenchSourceDescriptor()).catch(reject);
  }).catch(error=>{diagnostics.event('worker-error',{kind:error?.kind??'unknown'});loaded=null;receiveData=null;throw error;});
}
