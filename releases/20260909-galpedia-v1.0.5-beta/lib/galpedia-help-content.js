...
const DEFINITIONS = [
  ...
  makeArticle({
    ...
    notes: ['调整条件后，结果会自动更新；点击“查看结果”即可收起筛选面板。', '找不到预期作品时，可先清除筛选条件再试。'],
  ...
  makeArticle({
    ...
    subtitle: 'Browse and rank on every screen',
    summary: '移动端支持浏览、搜索、筛选、查看详情、选择与排榜；需要更大画布时也可在电脑继续。',
    keywords: ['移动端', '手机', '触控', '选择', '分享链接', '电脑', '筛选'],
    steps: ['在移动作品库搜索标题、别名或拼音，并打开筛选抽屉。', '点击“选择”进入选择模式，勾选多部作品后打开“查看已选”。', '点击“生成分享链接”复制候选选择。', '打开分享链接后选择追加或替换候选池；移动端与电脑端都可以继续排榜。'],
    notes: ['分享链接只带作品选择，不带本地图片或完整排榜状态。', '导入前会显示可以加入和暂时找不到的作品；追加和替换是两个不同操作。', '手机端可直接继续排榜；电脑端提供更大的操作空间。'],
    related: ['works.selection', 'tier.backup', 'tier.overview'],
    contexts: [CONTEXTS.CURRENT, CONTEXTS.COMMON],
    sections: [{ title: '分享链接带来什么？', paragraphs: ['分享链接用于把选片带到另一个设备继续排榜，不是完整排榜备份；本地图片、贴纸和等级顺序仍需在原浏览器或备份文件中保留。'] }],
  ...
  makeArticle({
    ...
    notes: ['批量选中的候选会作为一组移动；放错位置时可点击撤销。', '页面撤销按顺序恢复分级、等级编辑和标注；图片编辑器里的撤销独立处理画笔和图层。', '在“更多操作”或直播的“⋯”中打开“全榜查找与定位”，可查找已排和候选条目，点击结果跳到所在位置，不改变排序。'],
  ...
  makeArticle({
    id: 'tier.stickers', category: CATEGORIES.TIER, title: '编辑图片：贴纸、画笔与文字',
    subtitle: 'Annotate a local image without changing its source',
    summary: '上传图片贴纸，用画笔标注或局部打码，添加文字，并调整图层位置与顺序。',
    keywords: ['贴纸', '图片编辑', '自定义贴纸', '画笔', '文字', '文本框', '打码', '马赛克', '模糊', '遮挡条', '旋转', '图层', '撤销', '重做'],
    steps: ['在普通排榜中点卡片的“⋯”，选择“编辑图片”；也可从图片预览或上传裁切页进入。桌面悬停或键盘聚焦卡片即可看到入口，手机可直接点原有整理按钮。', '选择画笔、打码笔、上传图片贴纸或添加文本框；每一笔、每张贴纸、每个文本框都独立成层。', '切回“选择”后拖动图层，用双指缩放、旋转，或用尺寸与图层按钮精确调整；从图层列表可选中被遮住的内容。', '选择“保存编辑”保留结果。上传流程会先返回裁切确认页，最后确认才加入候选；取消编辑保留进入前的版本。'],
    notes: ['每张图片最多 12 个图层；文字最多 200 字符、6 行。', '素材仅保存在当前浏览器；清除本站数据会丢失本机编辑。', '本机保留原图以便重编辑；打码合成到导出图，不应视为敏感信息销毁。', '作品标注不改封面底图，在直播展示和排榜图片导出中呈现；图片编辑中的文字则合成到封面。'],
  ...
  makeArticle({
    ...
    summary: '直播模式突出榜单，保留拖动、预览和作品标注；窗口直播与全屏可分别切换。',
    keywords: ['直播模式', '沉浸', '全屏', '图片标注', 'Escape', '退出', '桌面', '移动端', '全榜查找', '定位'],
    steps: ['在排榜工具栏点击“直播模式”，按需单独切换全屏。', '拖动候选或已分级图片调整排榜，点击图片查看大图。', '点卡片“⋯”选择“编辑标注”；桌面也可用右键菜单，Shift＋右键直接编辑。', '点击“返回编辑”回到普通排榜；关闭全屏不会同时退出直播。'],
    notes: ['作品标注在直播模式显示，普通模式也可编辑；不需要知道快捷键才能使用。', '图片中的贴纸、画笔和文本请回到普通排榜的“编辑图片”中修改。', 'Esc优先关闭当前弹窗或编辑，不作为退出直播的快捷键。'],
    related: ['tier.overview', 'tier.stickers', 'tier.export'],
    contexts: [CONTEXTS.CURRENT, CONTEXTS.COMMON],
    sections: [{ title: '标注与查看资料', paragraphs: ['每条标注最多 16 个字符，并按两行显示；手机端也可通过卡片菜单编辑。', '查看会社资料会暂时离开直播布局，使用“返回会社排榜”可回到原榜与直播模式。'] }],
  ...
  makeArticle({
    ...
    steps: ['在排榜页的“更多操作”中选择“导出 JSON”，保存当前排榜。', '需要恢复时选择“导入 JSON”，确认替换范围后应用；作品选择、等级和顺序可以撤销恢复，导入的筛选及卡片设置不在撤销范围内。', '想分享结果图时，选择“导出图片”，生成 PNG。'],
