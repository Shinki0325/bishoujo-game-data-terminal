...
function assertDisplayTitle(displayTitle) {
  if (
    ...
    throw new TypeError('displayTitle must be non-empty, trimmed, and free of control characters');
  }
}

function encodeDisplayLabel(displayTitle) {
  if (
  ...
  return `"${displayTitle.replace(/\\/gu, '\\\\').replace(/"/gu, '\\"')}"`;
}
...
function displayAuthority(filters) {
  ...
  for (const filter of filters) {
    ...
    assertDisplayTitle(displayTitle);
    ...
    idToLabel.set(filterId, encodeDisplayLabel(displayTitle));
  }
...
function readQuotedLabel(source, start) {
  let offset = start + 1;
  let value = '';
  while (offset < source.length) {
    ...
    if (character === '\\') {
      ...
      if (escaped !== '"' && escaped !== '\\') {
        throw new FormulaSyntaxError(
          `Invalid quoted-label escape at offset ${offset}`,
          'INVALID_QUOTED_LABEL',
          offset
        );
      }
      value += escaped;
  ...
  throw new FormulaSyntaxError(
    `Unterminated quoted filter label at offset ${start}`,
    'UNTERMINATED_QUOTED_LABEL',
    start
  );
}

function translateTokens(source, resolveToken, unknownNoun, { quotedLabels = false } = {}) {
  const output = [];
  const offsetMap = [];
  let offset = 0;
  ...
  while (offset < source.length) {
    ...
    const start = offset;
    if (quotedLabels && character === '"') {
      const quoted = readQuotedLabel(source, start);
      const replacement = resolveToken(quoted.value);
      if (replacement === undefined) {
        throw new FormulaSyntaxError(
          `Unknown ${unknownNoun} "${quoted.value}" at offset ${start}`,
          'UNKNOWN_FILTER',
          start
        );
      }
      append(replacement, start, quoted.end - start);
      offset = quoted.end;
      continue;
    }
    while (
