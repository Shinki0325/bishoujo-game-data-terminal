...
import { releaseDateSortCompare, releaseStatusMatches, releaseYearMatches, RELEASE_STATUS } from './work-release-date.js';
import { cloneAttributeSelections } from './attribute-filters.js';
import { USER_WORK_LIMIT } from './work-limit.js';
import { CATALOG_RECORD_LIMIT } from './catalog-capacity.js';
import { PERSON_WORK_ROLES } from './person-work-index.js';
import {
...
const FILTER_STATE_FIELDS_WITHOUT_PERSON = Object.freeze(
  FILTER_STATE_FIELDS.filter(field => field !== 'personIds' && field !== 'personRole')
);
const LEGACY_FILTER_STATE_FIELDS = Object.freeze(
  FILTER_STATE_FIELDS_WITHOUT_PERSON.filter(field => field !== 'excludeNukige' && field !== 'releaseStatus')
);
...
export const CATALOG_WORK_LIMIT = CATALOG_RECORD_LIMIT;
...
function snapshotWork(work, index) {
  ...
  const isNukige = Object.hasOwn(work, 'isNukige')
    ...
    : false;
  const optionalRatingSortValue = field => {
    if (!Object.hasOwn(work, field)) return null;
    const value = ownDataValue(work, field, path, 'INVALID_WORK');
    if (value === null) return null;
    if (typeof value !== 'number' || !Number.isFinite(value) || value < 0) {
      throw catalogError('INVALID_WORK', `${path}.${field} must be null or a finite non-negative number`, `${path}.${field}`);
    }
    if (field.endsWith('VoteCount') && !Number.isInteger(value)) {
      throw catalogError('INVALID_WORK', `${path}.${field} must be null or a non-negative integer`, `${path}.${field}`);
    }
    return value;
  };
  if (!isSafeId(workId)) {
  ...
  if (median !== null && (typeof median !== 'number' || !Number.isFinite(median) || median < 0)) {
    throw catalogError(
      'INVALID_WORK',
      `${path}.median must be null or a finite non-negative number`,
      `${path}.median`
    );
  }
  if (voteCount !== null && (typeof voteCount !== 'number' || !Number.isInteger(voteCount) || voteCount < 0)) {
    throw catalogError(
      'INVALID_WORK',
      `${path}.voteCount must be null or a non-negative integer`,
      `${path}.voteCount`
    );
  }
  if (!isSafeId(brandId)) {
  ...
  return Object.fromEntries([
    ...
    ['median', median],
    ['voteCount', voteCount],
    ['egsScore', optionalRatingSortValue('egsScore')],
    ['vndbScore', optionalRatingSortValue('vndbScore')],
    ['vndbVoteCount', optionalRatingSortValue('vndbVoteCount')],
    ['bangumiScore', optionalRatingSortValue('bangumiScore')],
    ['bangumiVoteCount', optionalRatingSortValue('bangumiVoteCount')],
    ['brandId', brandId],
    ['isNukige', isNukige],
    [
      'rawFilterIds',
...
function isValidReleaseDate(value) {
  ...
  const partial = /^(\d{4})(?:-(\d{2}))?$/u.exec(value);
  if (partial) return Number(partial[1]) >= MIN_RELEASE_YEAR && Number(partial[1]) <= MAX_RELEASE_YEAR
    && (partial[2] === undefined || Number(partial[2]) >= 1 && Number(partial[2]) <= 12);
...
function optionalWorkGroupId(work, fallbackWorkId) {
  ...
  if (!Object.hasOwn(descriptor, 'value') || (!isSafeId(descriptor.value) && !/^(?:vndb:v|egs:)[1-9]\d*$/u.test(descriptor.value))) {
...
function snapshotFilterState(value) {
  const hasPersonFields = Object.hasOwn(value, 'personIds') || Object.hasOwn(value, 'personRole');
  const hasReleaseStatus = Object.hasOwn(value, 'releaseStatus');
  const fields = hasPersonFields
    ? (hasReleaseStatus ? FILTER_STATE_FIELDS : FILTER_STATE_FIELDS.filter(field => field !== 'releaseStatus'))
    : Object.hasOwn(value, 'excludeNukige')
      ? (hasReleaseStatus ? FILTER_STATE_FIELDS_WITHOUT_PERSON : FILTER_STATE_FIELDS_WITHOUT_PERSON.filter(field => field !== 'releaseStatus'))
  ...
  for (const [field, source] of entries) {
    ...
    if (FILTER_STATE_ARRAY_FIELDS.has(field)) {
      const limit = field === 'brandIds'
        ? MAX_BRAND_IDS
        : field === 'personIds' ? MAX_PERSON_IDS : MAX_STATE_FILTER_IDS;
  ...
  if (!Object.hasOwn(value, 'releaseStatus')) {
    appendDataProperty(snapshotEntries, ['releaseStatus', RELEASE_STATUS.ALL]);
  }
  if (!hasPersonFields) {
    appendDataProperty(snapshotEntries, ['personIds', []]);
    appendDataProperty(snapshotEntries, ['personRole', 'all']);
  }
  ...
  if (![RELEASE_STATUS.ALL, RELEASE_STATUS.RELEASED, RELEASE_STATUS.UNRELEASED, RELEASE_STATUS.TBD].includes(snapshot.releaseStatus)) {
    throw catalogError('INVALID_FILTER_STATE', 'filterState.releaseStatus is invalid', 'filterState.releaseStatus');
  }
  for (const personId of snapshot.personIds) {
    if (typeof personId !== 'string' || !SAFE_ID_PATTERN.test(personId) || UNSAFE_IDS.has(personId)) {
      throw catalogError('INVALID_FILTER_STATE', 'filterState.personIds contains an invalid ID', 'filterState.personIds');
    }
  }
  if (typeof snapshot.personRole !== 'string' || (snapshot.personRole !== 'all' && !PERSON_WORK_ROLES.includes(snapshot.personRole))) {
    throw catalogError('INVALID_FILTER_STATE', 'filterState.personRole is invalid', 'filterState.personRole');
  }
  if (
...
function compareRecords(left, right, sortKey, sortDirection) {
  ...
  if (NUMERIC_SORT_KEYS.has(sortKey)) {
    if (left.snapshot[sortKey] === null || right.snapshot[sortKey] === null) {
      if (left.snapshot[sortKey] === right.snapshot[sortKey]) return left.index - right.index;
      return left.snapshot[sortKey] === null ? 1 : -1;
    }
    ...
    if (comparison === 0) {
      comparison = releaseDateSortCompare(
        left.snapshot.releaseDate,
        right.snapshot.releaseDate,
        { direction: sortDirection }
      );
    }
...
// Runtime projections receive catalog objects that have already crossed the
// validation boundary. Re-snapshotting thousands of works and insertion-
// sorting them after a Worker query is unnecessarily expensive, so this
// internal-facing path preserves the exact comparator with native O(n log n)
// sorting. Callers must not use it for untrusted imported data.
export function sortTrustedCatalog(works, sortKey, sortDirection) {
  if (!Array.isArray(works)) throw new TypeError('works must be an array');
  assertSort(sortKey, sortDirection);
  const earliestReleaseByGroup = new Map();
  for (const work of works) {
    const workGroupId = work.workGroupId ?? work.workId;
    const current = earliestReleaseByGroup.get(workGroupId);
    if (current === undefined || work.releaseDate < current) {
      earliestReleaseByGroup.set(workGroupId, work.releaseDate);
    }
  }
  const records = works.map((work, index) => {
    ...
    return {
      ...
      snapshot: {
        ...work,
        egsScore: work.egsScore ?? null,
        vndbScore: work.vndbScore ?? null,
        vndbVoteCount: work.vndbVoteCount ?? null,
        bangumiScore: work.bangumiScore ?? null,
        bangumiVoteCount: work.bangumiVoteCount ?? null,
        groupEarliestReleaseDate: earliestReleaseByGroup.get(workGroupId) ?? work.releaseDate
      }
    };
  });
  records.sort((left, right) => compareRecords(left, right, sortKey, sortDirection));
  return records.map(record => record.original);
}
...
function queryCatalogInternal(works, filterState, knownFilterIds, selectedWorkIds) {
  ...
  // The synchronous catalogue path has no person reverse index.  Never
  // silently ignore a person condition and return the whole catalogue.
  if (state.personIds.length > 0) return [];
  ...
  const normalizedPinyinQuery = normalizeGeneratedPinyinQuery(state.titleQuery);
  ...
  for (const workSnapshot of filteredWorks) {
    ...
    if (
      ...
      && !buildSearchHaystack([record.snapshot.title]).includes(normalizedQuery)
      && (!normalizedPinyinQuery
        || !buildPinyinSearchHaystack([record.snapshot.title]).includes(normalizedPinyinQuery))
    ) {
      continue;
    }
    if (!releaseYearMatches(record.snapshot.releaseDate, state.releaseYearStart, state.releaseYearEnd)) {
      continue;
    }
    if (!releaseStatusMatches(record.snapshot.releaseDate, state.releaseStatus)) {
      continue;
    }
...
export function countWithFilterPatches(input) {
  return withCatalogBoundary(() => {
    ...
    if (current.personIds.length > 0 || states.some(state => state.personIds.length > 0)) {
      return states.map(() => 0);
    }

    const normalizedQuery = normalizeTitleQuery(current.titleQuery);
    const normalizedPinyinQuery = normalizeGeneratedPinyinQuery(current.titleQuery);
    const eligibleWorks = [];
    for (const record of authority.records) {
      if (
        normalizedQuery.length > 0
      ...
      if (!releaseYearMatches(record.snapshot.releaseDate, current.releaseYearStart, current.releaseYearEnd)) {
        continue;
      }
      if (!releaseStatusMatches(record.snapshot.releaseDate, current.releaseStatus)) {
        continue;
      }
