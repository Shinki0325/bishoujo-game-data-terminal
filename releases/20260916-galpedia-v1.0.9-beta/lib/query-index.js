import {compileCompanyWorkPositions} from "./company-work-filter.js";
...
function companyMask(index, brandIds) {
  ...
  for (const brandId of brandIds) {
    const source = index.companyPositions.get(brandId);
    if (!source) continue;
    for (const position of source) mask[position] = 1;
  }
  return mask;
}
...
function matchingPositions(index, filterState, selectedWorkIds) {
  ...
  const namedCompanies=companyQuery?companyMask(index,[...(exactCompanyIds??[]),...(looseCompanyIds??[])]):null;
  ...
  for (let position = 0; position < matches.length; position += 1) {
    const work = index.works[position];
    const companyQueryMatch = Boolean(namedCompanies?.[position]);
    if (
      !matches[position]
...
export function createQueryIndex({
  ...
  if(companyWorkIndex)brands=companyWorkIndex.brands;
  const companyPositions=companyWorkIndex?compileCompanyWorkPositions(workSnapshots,companyWorkIndex):new Map(Object.entries(sourceIndexes.companies));
