...
const DEFINITIONS = [
  ...
  makeArticle({
    ...
    steps: ['打开“筛选”，设置最低评分、评分人数或发行年份范围。', '按需选择会社、基础属性和标签；也可以使用筛选公式。', '面板中的预计数量会随条件更新；调整完成后点击底部按钮，应用条件并查看结果。', '回到作品库查看结果数量、筛选摘要和可清除的条件。'],
    notes: ['修改面板里的条件时，原来的作品列表会先保留。关闭面板与应用条件请以按钮提示为准。', '找不到预期作品时，可以清除筛选条件并应用，扩大到全库再查找。'],
