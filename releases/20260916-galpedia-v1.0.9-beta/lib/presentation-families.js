import { sortCatalog, sortTrustedCatalog } from './catalog.js';
import { PRESENTATION_FAMILY_LIMIT } from './catalog-capacity.js';

const SIDECAR_SCHEMA_VERSION = 'egs-tier-full-presentation-families-v1';
const MAX_FAMILIES = PRESENTATION_FAMILY_LIMIT;
...
export function preparePresentationFamiliesSidecar(value, {
  ...
  const candidateFamilies = assertArray(value.families, 'families', MAX_FAMILIES)
    .filter(family => {
      if (bangumiSubjectByWorkId === null) return true;
      const subjects = new Set(family.catalogMemberWorkIds
        .map(workId => bangumiSubjectByWorkId.get?.(workId))
        .filter(subjectId => typeof subjectId === 'string' && subjectId.length > 0));
      // A VNDB version family that maps to multiple independent Bangumi
      // subjects must remain visible as separate works.  Unknown/missing
      // subjects are retained rather than guessed into a split.
  ...
  const activeMappedEntries = mappedEntries.filter(([workId]) => familyByWorkId.has(workId));
  if (activeMappedEntries.length !== familyByWorkId.size) throw new TypeError('workToPresentationWorkId is missing an active family member');
  for (const [workId, presentationWorkId] of mappedEntries) {
    const family = familyByWorkId.get(workId);
    if (!allowedWorkIds.has(workId) || typeof presentationWorkId !== 'string') {
      throw new TypeError('workToPresentationWorkId contains an unknown member');
    }
    if (family !== undefined && presentationWorkId !== family.presentationWorkId) {
      throw new TypeError('workToPresentationWorkId does not match the public family members');
    }
  }
  function decorateWork(work) {
    const family = familyByWorkId.get(work.workId);
    return family === undefined ? work : Object.freeze({
      ...work, presentationFamily: family, presentationMemberCount: family.members.length
    });
  }
  return Object.freeze({
    familyCount: families.length,
    excludedFamilyCount: candidateFamilies.length === value.families.length
      ? 0
      : value.families.length - candidateFamilies.length,
    ...
    decorateWorks(works) {
      return works.map(decorateWork);
    },
    projectVisibleWorks(visibleWorks, { sortKey, sortDirection, workById = null, presorted = false, decorate = true } = {}) {
      if (!Array.isArray(visibleWorks)) throw new TypeError('visibleWorks must be an array');
      // Pick a matching edition using the requested order; an excluded
      // family default must never bypass a date, platform or title filter.
      if (!presorted && sortKey !== undefined && sortDirection !== undefined) {
        visibleWorks = sortCatalog(visibleWorks, sortKey, sortDirection);
      }
      const visibleWorkById = new Map(visibleWorks.map(work => [work.workId, work]));
      const output = [];
      const canPreserveSortedOrder = presorted && visibleWorks.every(work => {
        const family = familyByWorkId.get(work.workId);
        return family === undefined || visibleWorkById.has(family.defaultWorkId);
      });
      if (canPreserveSortedOrder) {
        for (const work of visibleWorks) {
          const family = familyByWorkId.get(work.workId);
          if (family === undefined) {
            output.push(workById?.get?.(work.workId) ?? work);
            continue;
          }
          if (work.workId !== family.defaultWorkId) continue;
          const defaultWork = workById?.get?.(family.defaultWorkId) ?? work;
          output.push(decorate ? decorateWork(defaultWork) : defaultWork);
        }
        return output;
      }
      ...
      for (const work of visibleWorks) {
        ...
        const matchingWork = visibleWorkById.get(family.defaultWorkId) ?? work;
        const displayWork = workById?.get?.(matchingWork.workId) ?? matchingWork;
        output.push(decorate ? decorateWork(displayWork) : displayWork);
      }
      if (sortKey === undefined || sortDirection === undefined) return output;
      return presorted
        ? sortTrustedCatalog(output, sortKey, sortDirection)
        : sortCatalog(output, sortKey, sortDirection);
    },
