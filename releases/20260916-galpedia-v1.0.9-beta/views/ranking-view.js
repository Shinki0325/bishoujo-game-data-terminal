...
function rankingMediaKey(work, media) {
  return JSON.stringify([
    media.coverUrl ?? null, media.previewUrl ?? null, media.assetBase ?? null,
    work.coverPath ?? null, work.coverWidth ?? null, work.coverHeight ?? null
  ]);
}
...
export function createRankingCard(documentRef, work, callbacks) {
  ...
  // Card nodes can move between a tier and the candidate pool. Keep the
  // listener closures mutable so a reused node still follows the current
  // fullWiki display title, media URL and callbacks.
  const currentWork = { ...work };
  let currentCallbacks = { onOpenDetails, onOpenMedia, onContextMenu, onDragStart, onDragEnd,
    shouldSuppressMediaClick, isCardActivationEnabled };
  let currentMedia = { assetBase, coverUrl, previewUrl };
  let currentMediaKey = rankingMediaKey(currentWork, currentMedia);

  const displayTitle = typeof currentWork.displayTitle === 'string' && currentWork.displayTitle.length > 0
    ? currentWork.displayTitle
    : currentWork.title;
  ...
  if (typeof currentMedia.coverUrl === 'string' && currentMedia.coverUrl.length > 0) {
    if (!currentMedia.coverUrl.startsWith('blob:')) image.crossOrigin = 'anonymous';
    applyAdaptiveImageSource(image, { thumbnailUrl: currentMedia.coverUrl, previewUrl: currentMedia.previewUrl });
  } else {
    try {
      applyImageAsset(image, currentWork, currentMedia.assetBase);
    } catch (error) {
  ...
  card.addEventListener('contextmenu', event => {
    ...
    currentCallbacks.onContextMenu(currentWork, card, event);
  });
  ...
  card.__rankingUpdate = (nextWork, nextCallbacks = {}, nextMedia = {}) => {
    ...
    for (const key of Object.keys(currentWork)) if (!Object.hasOwn(nextWork, key)) delete currentWork[key];
    Object.assign(currentWork, nextWork);
    currentCallbacks = { ...currentCallbacks, ...nextCallbacks };
    currentMedia = { ...currentMedia, ...nextMedia };
    const nextTitle = typeof currentWork.displayTitle === 'string' && currentWork.displayTitle.length > 0
      ? currentWork.displayTitle : currentWork.title;
    syncAttribute(card, 'aria-label', nextTitle);
    const titleNode = card.querySelector?.('.ranking-card-title');
    if (titleNode) syncProperty(titleNode, 'textContent', nextTitle);
    syncAttribute(cover, 'aria-label', `放大 ${nextTitle}`);
    syncProperty(cover, 'title', `放大 ${nextTitle}`);
    syncAttribute(handle, 'aria-label', `整理 ${nextTitle}`);
    syncProperty(handle, 'title', `整理 ${nextTitle}`);
    const nextMediaKey = rankingMediaKey(currentWork, currentMedia);
    if (nextMediaKey !== currentMediaKey) {
      ...
      if (typeof currentMedia.coverUrl === 'string' && currentMedia.coverUrl.length > 0) {
        ...
        try { applyImageAsset(image, currentWork, currentMedia.assetBase); }
        catch (error) {
          if (error instanceof AssetUrlError) throw new TypeError('work.coverPath must use the approved public asset path');
          throw error;
        }
      }
    }
...
export function createRankingView({
  ...
  // Keep the vertical activation band tall enough to cover the sticky
  // ranking heading/toolbar, so users do not need to drag outside the canvas
  // before the page starts revealing upper or lower tiers.
  const verticalAutoScrollThreshold = 180;
  ...
  function rankingEntityKey(work) {
    const kind = Object.hasOwn(work, 'companyImageUrl') || Object.hasOwn(work, 'company') ? 'company' : 'work';
  ...
  function removeCandidateCardControls(card) {
    for (const selector of ['.ranking-candidate-remove', '.ranking-candidate-select']) {
      for (const control of arrayFrom(card.querySelectorAll?.(selector))) control.remove?.();
    }
  }

  function ensureCandidateCardControls(card, work) {
    ...
    syncAttribute(remove, 'aria-label', `移除候选作品：${work.title}`);
    syncAttribute(remove, 'title', `移除候选作品：${work.title}`);
    ...
    if (!select) {
      ...
      select.addEventListener('change', event => {
        event.stopPropagation(); setCandidateSelected(card.dataset.workId, select.checked);
      });
      card.append(select);
    }
    syncProperty(select, 'checked', candidateSelection.has(work.workId));
    syncAttribute(select, 'aria-label', `选择候选作品：${work.title}`);
  }
  ...
  function cachedRankingCard(item, callbacks, media, { candidate = false } = {}) {
    ...
    if (!record || (record.card && typeof record.card.__rankingUpdate !== 'function' && record.work !== item)) {
      record = { work: { ...item }, callbacks: { ...callbacks, ...media }, card: null };
      ...
      Object.assign(record.callbacks, callbacks, media);
    ...
    cardCache.delete(cacheKey); cardCache.set(cacheKey, record);
    return record.card;
  }

  function pruneRankingCaches(activeCardKeys, activeTierIds) {
    for (const [key, record] of cardCache) {
      if (cardCache.size <= MAX_RANKING_CARD_CACHE || activeCardKeys.has(key)) continue;
      record.card?.__rankingImageCleanup?.();
      cardCache.delete(key);
    }
    for (const [tierId, record] of tierRowCache) {
      if (activeTierIds.has(tierId)) continue;
      record.row?.remove?.();
      tierRowCache.delete(tierId);
    }
  }
  ...
  function removeIndicator({ keepTrack = null } = {}) {
    const parent = indicator.parentElement;
    if (!parent || parent === keepTrack) return;
    ...
    try {
      ...
      if (autoScrollRoot) {
        ...
        const velocity = edgeScrollVelocity({
          ...
          left: rect.top,
          right: rect.bottom,
          threshold: verticalAutoScrollThreshold,
          maxSpeed: 20
        });
        if (velocity === 0) {
          autoScrollRoot = false;
  ...
  // Native desktop drag events stop targeting a tier row once the pointer
  // reaches the sticky header/toolbar. Keep the page-edge auto-scroll alive
  // from the document so dragging toward the top can still reveal tier A.
  function handleDocumentDragOver(event) {
    if (draggedWorkId === null || !Number.isFinite(event?.clientY)) return;
    // Keep the native drag operation alive while the pointer crosses the
    // sticky header/toolbar. Without a valid dragover target the browser ends
    // the operation before the edge-scroll frame can reveal an upper tier.
    event.preventDefault?.();
  ...
  function placeIndicator(track, pointerX, pointerY) {
    removeIndicator({ keepTrack: track });
    ...
    let insertionIndex;
    if (usesTwoRows) {
      const entries = dragGeometry.read(track, allCards).map(({ card, rect, row }) => {
        if ((row !== 1 && row !== 2)
      ...
      const rects = dragGeometry.read(track, destinationCards).map(({ rect }) => {
    ...
    const currentChildren = arrayFrom(track.children);
    const unchanged = currentChildren.length === desiredChildren.length
      && desiredChildren.every((child, index) => currentChildren[index] === child);
    if (!unchanged) {
      track.replaceChildren(...desiredChildren);
      dragGeometry.invalidate(track);
    }
  ...
  function createTierRow(tier) {
    ...
    const tierIndex = model.tiers.findIndex(item => item.id === currentTier.id);
    const color = tierColor(currentTier.colorId);
    const row = documentRef.createElement('section');
    row.className = 'tier-row';
    row.id = `ranking-${currentTier.id}`;
    row.dataset.tierId = currentTier.id;
    row.setAttribute('aria-label', `${currentTier.name} 级`);
    ...
    label.tabIndex = 0;
    label.setAttribute('aria-label', currentTier.name);
    label.setAttribute('title', currentTier.name);
    ...
    name.textContent = currentTier.name;
    const input = documentRef.createElement('input');
    input.className = 'tier-name-input';
    input.type = 'text';
    input.value = currentTier.name;
    input.maxLength = 24;
    input.hidden = true;
    input.setAttribute('aria-label', `${currentTier.name} 绾у悕`);
    const count = documentRef.createElement('output');
    count.textContent = String(currentTier.works.length);
    ...
    const controls = [
      ['move-up', '↑', '上移', tierIndex === 0, () => {
        focusTierId = currentTier.id;
        onTierConfigChange(snapshotTierConfig(moveTier(snapshotTierConfig(model.tiers), currentTier.id, -1)));
      }],
    ...
    paletteTrigger.addEventListener('click', event => {
      ...
      else openColorPalette(currentTier, paletteTrigger);
    });
    ...
    function beginEdit() {
      if (immersive) return;
      if (editingTierId !== null && editingTierId !== currentTier.id) closeTierEditing();
      editingTierId = currentTier.id;
    ...
    function commitName() {
      ...
      const nextTiers = model.tiers.map(item => ({
        ...item,
        name: item.id === currentTier.id ? nextName : item.name
      }));
      onTierConfigChange(snapshotTierConfig(nextTiers));
      closeTierEditing();
    }
    ...
    input.addEventListener('blur', event => {
      ...
      if (editingTierId === currentTier.id) commitName();
    });

    const track = documentRef.createElement('div');
    track.className = 'tier-track';
    track.dataset.destination = currentTier.id;
    track.setAttribute('role', 'list');
    track.setAttribute('aria-label', `${currentTier.name} 级作品`);
    track.tabIndex = 0;
    row.addEventListener('dragover', event => handleTierDragOver(currentTier.id, event));
    row.addEventListener('dragleave', event => {
      if (!isDescendant(row, event.relatedTarget)) clearDropState();
    });
    row.addEventListener('drop', event => handleTierDrop(currentTier.id, event));
    row.append(label, track);
    const update = (nextTier, nextIndex = model.tiers.findIndex(item => item.id === nextTier.id)) => {
      currentTier = { ...nextTier };
      const nextColor = tierColor(currentTier.colorId);
      row.id = `ranking-${currentTier.id}`;
      row.dataset.tierId = currentTier.id;
      row.setAttribute('aria-label', `${currentTier.name} 级`);
      row.style.setProperty('--tier-background', nextColor.background);
      row.style.setProperty('--tier-foreground', nextColor.foreground);
      syncAttribute(label, 'aria-label', currentTier.name);
      syncAttribute(label, 'title', currentTier.name);
      syncProperty(name, 'textContent', currentTier.name);
      if (editingTierId !== currentTier.id) syncProperty(input, 'value', currentTier.name);
      syncAttribute(input, 'aria-label', `${currentTier.name} 绾у悕`);
      syncProperty(count, 'textContent', String(currentTier.works.length));
      syncAttribute(track, 'aria-label', `${currentTier.name} 级作品`);
      for (const button of editingState.querySelectorAll?.('.tier-edit-control') ?? []) {
        if (button.dataset.action === 'move-up') button.disabled = nextIndex === 0;
        if (button.dataset.action === 'move-down') button.disabled = nextIndex === model.tiers.length - 1;
        if (button.dataset.action === 'delete') button.disabled = model.tiers.length <= 3;
      }
      return { row, track, update };
    };
    return { row, track, update };
  }
  ...
  return Object.freeze({
    render(nextModel, coverUrls = null) {
      ...
      for (let index = 0; index < model.tiers.length; index += 1) {
        ...
        if (!tierRecord) {
          tierRecord = createTierRow(tier);
          tierRowCache.set(tier.id, tierRecord);
        } else {
          tierRecord.update(tier, index);
        }
        const { row, track } = tierRecord;
        activeTierIds.add(tier.id);
        const cards = tier.works.map(item => {
          activeCardKeys.add(rankingEntityKey(item));
          return cachedRankingCard(item, callbacks, {
      ...
      pruneRankingCaches(activeCardKeys, activeTierIds);
      updateCandidateSelection();
      updateTierTrackRows();
