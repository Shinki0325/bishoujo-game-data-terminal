...
function renderTable(){
  ...
  $('table-body').replaceChildren(...rows.slice(page*10,page*10+10).map(r=>{
    ...
    a.textContent=r.title;a.title=r.originalTitle;a.href='https://favorite.bishojo.date/#works/work/'+encodeURIComponent(r.workId??r.workIds?.[0]??r.id);a.target='_blank';a.rel='noopener';small.textContent=individual?r.characterId:'EGS '+r.id;title.append(a,small);tr.append(title);
...
async function start(){
  ...
  }catch(error){$('chart-title').textContent='公开快照未能加载';$('chart-subtitle').textContent='请重新加载；已保存的方案会保留。';$('result-summary').textContent=String(error.message);document.body.dataset.error='true';}
}
start();
