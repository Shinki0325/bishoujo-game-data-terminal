...
import { filterPersonsBySearch, withCjkPersonSearchKey, withLazyCjkPersonSearchKey } from './person-search.js';
  ...
  async function prepareDirectory() {
    if (performanceRuntime !== null) {
      try {
        ...
        // The hash-pinned full-wiki directory carries both search keys as
        // non-enumerable trusted symbols.  Keep those records in place: the
        // generic wrapper would allocate/copy 53,075 objects before the first
        // row can render, while filterPersonsBySearch already consumes the
        // symbol-backed keys through person-search.js.  Older/fallback
        // directories retain the historical wrapper and idle warm-up.
        records = directory.lazySearch && directory.searchKeysPrecomputed
          ? directory.records
          : directory.records.map(directory.lazySearch ? withLazyCjkPersonSearchKey : withCjkPersonSearchKey);
        if (directory.lazySearch && !directory.searchKeysPrecomputed && typeof globalThis.requestIdleCallback === 'function') {
          const snapshot = records; let index = 0;
          const warm = deadline => {
            let count = 0;
            while (index < snapshot.length && deadline.timeRemaining() > 3 && count++ < 100) {
              const row = snapshot[index++]; void row.searchKey; void row.pinyinSearchKey;
            }
            if (index < snapshot.length && records === snapshot) globalThis.requestIdleCallback(warm);
          };
          globalThis.requestIdleCallback(warm);
        }
        return records;
      } catch (error) {
