...
import { RELEASE_DATE_DEFAULT_MAX_YEAR, RELEASE_DATE_MIN_YEAR, RELEASE_STATUS } from './work-release-date.js';
...
const V5_FILTER_STATE_FIELDS_WITHOUT_PERSON = Object.freeze(
  V5_FILTER_STATE_FIELDS.filter(field => field !== 'personIds' && field !== 'personRole')
);
const LEGACY_V5_FILTER_STATE_FIELDS = Object.freeze(
  V5_FILTER_STATE_FIELDS_WITHOUT_PERSON.filter(field => field !== 'excludeNukige' && field !== 'releaseStatus')
);
...
const VALID_RELEASE_STATUSES = new Set([RELEASE_STATUS.ALL, RELEASE_STATUS.RELEASED, RELEASE_STATUS.UNRELEASED, RELEASE_STATUS.TBD]);
...
const SAFE_WORK_GROUP_PATTERN = /^(?:vndb:v[1-9][0-9]*|egs:[1-9][0-9]*)$/u;
...
const MAX_AUTHORITY_WORK_IDS = CATALOG_RECORD_LIMIT;
...
function frozenDefaultFilterState() {
  return Object.freeze(ownObject([
    ...
    ['releaseStatus', RELEASE_STATUS.ALL],
    ['sortKey', 'voteCount'],
    ['sortDirection', 'desc'],
    ['selectedOnly', false],
...
// Opaque handles are issued only after the complete existing validation path.
// Never cache caller-owned objects: their arrays/maps may change after a call.
const preparedAuthoritySnapshots = new WeakMap();
export function prepareStateAuthority(authority) {
  return runValidationInvocation(() => {
    const snapshot = normalizeAuthority(authority);
    const handle = Object.freeze(Object.create(null));
    preparedAuthoritySnapshots.set(handle, snapshot);
    return handle;
  });
}

function normalizeAuthority(authority) {
  const prepared = preparedAuthoritySnapshots.get(authority);
  ...
  try {
    ...
    for (const editionWorkId of work.ids) {
      ...
      if (!isSafeId(workGroupId) && !SAFE_WORK_GROUP_PATTERN.test(workGroupId)) {
        throw stateError(
          'INVALID_AUTHORITY',
...
function normalizeFilterState(source, authoritySnapshot) {
  const hasPersonFields = Object.hasOwn(source, 'personIds') || Object.hasOwn(source, 'personRole');
  const hasReleaseStatus = Object.hasOwn(source, 'releaseStatus');
  const filterFields = hasPersonFields
    ? (hasReleaseStatus ? V5_FILTER_STATE_FIELDS : V5_FILTER_STATE_FIELDS.filter(field => field !== 'releaseStatus'))
    : Object.hasOwn(source, 'excludeNukige')
      ? (hasReleaseStatus ? V5_FILTER_STATE_FIELDS_WITHOUT_PERSON : V5_FILTER_STATE_FIELDS_WITHOUT_PERSON.filter(field => field !== 'releaseStatus'))
    : LEGACY_V5_FILTER_STATE_FIELDS;
  const filterState = exactSnapshot(source, filterFields, 'state.filterState');
  if (!hasPersonFields) {
    filterState.personIds = [];
    filterState.personRole = 'all';
  }
  if (!Object.hasOwn(filterState, 'excludeNukige')) filterState.excludeNukige = false;
  if (!Object.hasOwn(filterState, 'releaseStatus')) filterState.releaseStatus = RELEASE_STATUS.ALL;
  try {
    const collectionLimits = ownObject([
      ['brandIds', MAX_BRAND_IDS],
      ['positiveFilterIds', MAX_FILTER_IDS],
      ['excludedFilterIds', MAX_FILTER_IDS],
      ['personIds', MAX_PERSON_IDS]
    ]);
    ...
    for (const id of filterState.personIds) {
      assertStringLength(id, MAX_ID_LENGTH, 'state.filterState.personIds');
      if (!isSafeId(id)) throw new TypeError('state.filterState.personIds contains an unsafe ID');
    }
    if (typeof filterState.personRole !== 'string' || (filterState.personRole !== 'all' && !PERSON_WORK_ROLES.includes(filterState.personRole))) {
      throw new RangeError('state.filterState.personRole is invalid');
    }
    ...
    if (
      filterState.releaseYearStart < RELEASE_DATE_MIN_YEAR
      || filterState.releaseYearEnd > RELEASE_DATE_DEFAULT_MAX_YEAR
    ...
    if (!VALID_RELEASE_STATUSES.has(filterState.releaseStatus)) {
      throw new RangeError('state.filterState releaseStatus is invalid');
    }
  ...
  return ownObject([
    ...
    ['releaseStatus', filterState.releaseStatus],
    ['sortKey', filterState.sortKey],
    ['sortDirection', filterState.sortDirection],
    ['selectedOnly', filterState.selectedOnly],
    ['personIds', stableUnique(filterState.personIds)],
    ['personRole', filterState.personRole]
  ]);
}

function migrateNukigeDefaultToExcludedTag(filterState) {
  if (filterState.excludeNukige !== true) return filterState;
  return ownObject([
    ['mode', filterState.mode],
    ['titleQuery', filterState.titleQuery],
    ['minimumScore', filterState.minimumScore],
    ['minimumVoteCount', filterState.minimumVoteCount],
    ['brandIds', filterState.brandIds],
    ['attributeSelections', filterState.attributeSelections],
    ['basicOperator', filterState.basicOperator],
    ['positiveFilterIds', filterState.positiveFilterIds],
    ['excludedFilterIds', stableUnique([...filterState.excludedFilterIds, 'pov-205'])],
    ['excludeNukige', false],
    ['advancedExpression', filterState.advancedExpression],
    ['releaseYearStart', filterState.releaseYearStart],
    ['releaseYearEnd', filterState.releaseYearEnd],
    ...
    ['sortDirection', filterState.sortDirection],
    ['selectedOnly', filterState.selectedOnly],
    ['personIds', filterState.personIds],
    ['personRole', filterState.personRole]
  ]);
}
...
function withDefaultPlatformFormula(source) {
  ...
  if (platformSource.length === 0) return trimmed;
  return trimmed.length === 0 ? platformSource : `${platformSource} AND (${source})`;
}

function stripPersonFilterFields(source) {
  if (!Object.hasOwn(source, 'personIds') && !Object.hasOwn(source, 'personRole')) return source;
  const copy = { ...source };
  delete copy.personIds;
  delete copy.personRole;
  return copy;
}

function migrateV4FilterState(source, authoritySnapshot) {
  const legacy = exactSnapshot(stripPersonFilterFields(source), V4_FILTER_STATE_FIELDS, 'state.filterState');
...
function normalizeSelectedWorkRefs(source, selectedWorkIds, authoritySnapshot) {
  ...
  for (let index = 0; index < refs.length; index += 1) {
    ...
    for (const field of ['workGroupId', 'editionWorkId']) {
      ...
      if (!isSafeId(ref[field]) && !(field === 'workGroupId' && SAFE_WORK_GROUP_PATTERN.test(ref[field]))) {
...
function normalizeV5State(candidate, authoritySnapshot) {
  try {
    ...
    const filterState = migrateNukigeDefaultToExcludedTag(
      normalizeFilterState(state.filterState, authoritySnapshot)
    );
...
function cloneDefaultFilterState() {
  return ownObject([
    ...
    ['sortDirection', DEFAULT_FILTER_STATE.sortDirection],
    ['selectedOnly', DEFAULT_FILTER_STATE.selectedOnly],
    ['personIds', Array.from(DEFAULT_FILTER_STATE.personIds)],
