...
export function createReviewedCompanyClient({loadPayload,loadDirectoryPayload=loadPayload,restoreCompanySummary}) {
  let directoryPending, pending;
  const validateDirectory=(payload, schema='galpedia-company-integration-v1')=>{
    if((payload.schema!==schema&&payload.schema!=='galpedia-company-directory-v1')||!Array.isArray(payload.companies)||!Array.isArray(payload.aliases))throw new TypeError('invalid company directory payload');
    const directory=restoreCompanySummary(payload.companies),companies=new Map(directory.companies.map(c=>[c.companyId,c]));
    const aliases=new Map(payload.aliases);
    ...
    return {directory:Object.freeze({...directory,companyIdByAlias:aliases}),companies,aliases};
  };
  const loadDirectory=()=>directoryPending??=Promise.resolve().then(loadDirectoryPayload).then(validateDirectory).catch(error=>{directoryPending=null;throw error;});
  const load=()=>pending??=Promise.resolve().then(async()=>{
    const directoryState=await loadDirectory(),payload=await loadPayload();
    if(payload.schema!=='galpedia-company-integration-v1'||!Array.isArray(payload.companies)||!Array.isArray(payload.aliases)||!Array.isArray(payload.lists))throw new TypeError('invalid company payload');
    if(payload.companies.length!==directoryState.companies.size||payload.aliases.length!==directoryState.aliases.size)throw new TypeError('company directory mismatch');
    const lists=new Map();
    for(const list of payload.lists){
      const key=companyListKey(list.companyId,list.scope,list.role);
      if(lists.has(key)||!directoryState.companies.has(list.companyId))throw new TypeError('duplicate company list');
    ...
    return {payload,directory:directoryState.directory,companies:directoryState.companies,aliases:directoryState.aliases,lists};
  ...
  return Object.freeze({
    async loadDirectory(){return (await loadDirectory()).directory;},
    async resolveCompanyId(id){const state=await loadDirectory();return resolve(state,id);},
