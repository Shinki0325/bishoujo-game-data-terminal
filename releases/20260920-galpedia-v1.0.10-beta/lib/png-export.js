...
// Preserve BUILD's published work namespaces while allowing the framework
// display/export additions. Tier IDs remain flat keys; work IDs may use the
// validated VNDB/EGS namespaces emitted by the fullWiki catalog.
const SAFE_WORK_ID_PATTERN = /^(?:[A-Za-z0-9][A-Za-z0-9_-]*|vndb:[pv][0-9]+|egs:(?:[0-9]+|brand:[0-9]+))$/u;
const UNSAFE_KEYS = new Set(['__proto__', 'constructor', 'prototype']);
const MAX_ID_LENGTH = 128;
const MAX_TITLE_LENGTH = 2048;
const MAX_COVER_PATH_LENGTH = 2048;
const MAX_RANKED_ITEMS = USER_WORK_LIMIT;
  ...
  for (const tierId of tierIds) {
    ...
    for (let index = 0; index < row.length; index += 1) {
      ...
      if (rankedCount > MAX_RANKED_ITEMS) {
        throw pngError(invocation, 'TIER_ORDER_TOO_LARGE', `tierOrder exceeds ${MAX_RANKED_ITEMS} works`, {
          path: 'tierOrder'
        });
      }
    }
...
function snapshotPresentation(value, invocation) {
  ...
  const keys = Reflect.ownKeys(value);
  const allowed = new Set(['showCounts', 'showTitles', 'annotations', 'immersive', 'uiScale', 'display']);
  const hasUiScale = Object.hasOwn(value, 'uiScale');
  const hasDisplay = Object.hasOwn(value, 'display');
  const completeSnapshot = keys.length === (hasUiScale ? 5 : 4) + (hasDisplay ? 1 : 0);
  ...
  if (hasUiScale) {
    const scale = ownDataValue(value, 'uiScale', 'input.presentation', invocation);
    assertPlainObject(scale, 'input.presentation.uiScale', invocation);
    const scaleKeys = Reflect.ownKeys(scale);
    const requiredScaleKeys = ['overall', 'card', 'rail', 'annotation', 'tierName'];
    const allowedScaleKeys = new Set([...requiredScaleKeys, 'tierWidth']);
    if (
      requiredScaleKeys.some(key=>!Object.hasOwn(scale,key))
      || scaleKeys.some(key => typeof key !== 'string' || !allowedScaleKeys.has(key))
      || scaleKeys.some(key => !Number.isFinite(Number(ownDataValue(scale, key, 'input.presentation.uiScale', invocation))))
    ) {
      throw pngError(invocation, 'INVALID_INPUT', 'input.presentation.uiScale is invalid', {
        path: 'input.presentation.uiScale'
      });
    }
  }
  let display = { style: 'garden', shape: 'square' };
  if (hasDisplay) {
    const sourceDisplay = ownDataValue(value, 'display', 'input.presentation', invocation);
    assertPlainObject(sourceDisplay, 'input.presentation.display', invocation);
    const style = ownDataValue(sourceDisplay, 'style', 'input.presentation.display', invocation);
    const shape = ownDataValue(sourceDisplay, 'shape', 'input.presentation.display', invocation);
    if (Reflect.ownKeys(sourceDisplay).length !== 2 || !['garden', 'classic'].includes(style) || !['square', 'portrait', 'landscape', 'contain'].includes(shape)) {
      throw pngError(invocation, 'INVALID_INPUT', 'input.presentation.display is invalid');
    }
    display = { style, shape };
  }
  const scale = hasUiScale ? ownDataValue(value, 'uiScale', 'input.presentation', invocation) : {};
  const scaleValue = key => Math.min(160, Math.max(80, Number(scale[key]) || 100)) / 100;
  return { showTitles, annotations, display, cardScale: scaleValue('card'), tierScale: scaleValue('tierName'), tierWidthScale: scaleValue('tierWidth'),
    showCounts: completeSnapshot && ownDataValue(value, 'showCounts', 'input.presentation', invocation) === true };
}
...
export function planTierPng(input) {
  return withSyncBoundary(invocation => {
    ...
    const allowed = new Set(['tiers', 'tierOrder', 'worksById', 'logicalMaxWidth', 'logicalMinHeight', 'pixelRatio', 'presentation']);
    ...
    const logicalMinHeight=ownDataValue(input,'logicalMinHeight','input',invocation,{optional:true,defaultValue:0});
    ...
    if (
      !Number.isSafeInteger(logicalWidth)
      || !Number.isSafeInteger(logicalMinHeight) || logicalMinHeight < 0 || logicalMinHeight > PNG_LIMITS.maxDimension
      || logicalWidth > PNG_LIMITS.logicalMaxWidth
      || typeof pixelRatio !== 'number'
      || !Number.isFinite(pixelRatio)
      || pixelRatio <= 0
    ...
    const classic = presentation.display?.style === 'classic';
    const shape = presentation.display?.shape;
    const LAYOUT = classic ? { ...DEFAULT_LAYOUT, outerPadding: 0, tierLabelWidth: Math.round(100 * presentation.tierWidthScale), gap: 0,
      itemSize: Math.round((shape === 'landscape' ? 130 : 80) * presentation.cardScale),
      titleStripHeight: 26 } : DEFAULT_LAYOUT;
    const itemHeight = classic ? Math.round((shape === 'portrait' ? 120 : 80) * presentation.cardScale) : LAYOUT.itemSize;
    ...
    const proportional = classic && shape === 'contain';
    let cursorY = LAYOUT.outerPadding;
    for (const tier of tierDefinitions) {
      let offsetX = 0, rowIndex = 0;
      const placements = tierOrder[tier.id].map((workId, index) => {
        if (!workRegistry.has(workId)) throw pngError(invocation, 'UNKNOWN_WORK_ID', `work ID ${workId} is not present in worksById`, { path: `tierOrder.${tier.id}[${index}]` });
        if (!workSnapshots.has(workId)) workSnapshots.set(workId, snapshotWork(workRegistry.get(workId), workId, invocation));
        const snapshot = workSnapshots.get(workId);
        const width = proportional ? Math.min(contentWidth, itemHeight * snapshot.coverWidth / snapshot.coverHeight) : LAYOUT.itemSize;
        if (offsetX > 0 && offsetX + width > contentWidth + 0.000001) { rowIndex++; offsetX = 0; }
        const placement = { x: offsetX, row: rowIndex, width };
        offsetX += width + LAYOUT.gap;
        return placement;
      });
      const rowCount = proportional ? rowIndex + 1 : Math.max(1, Math.ceil(tierOrder[tier.id].length / columns));
      const height = (rowCount * itemHeight) + ((rowCount - 1) * LAYOUT.gap);
      tierGeometry.push({ tier, y: cursorY, height, placements });
      cursorY += height + LAYOUT.gap;
    }
    // Minimum canvas size must not stretch tier rows beyond their cards.
    // Any spare space belongs after the board, not between its tiers.
    const logicalHeight = Math.max(logicalMinHeight, cursorY - LAYOUT.gap + LAYOUT.outerPadding);
    ...
    for (const geometry of tierGeometry) {
      ...
      for (let index = 0; index < tierOrder[tierId].length; index += 1) {
        ...
        const x = LAYOUT.outerPadding
          + LAYOUT.tierLabelWidth
          + LAYOUT.gap
          + (proportional ? placement.x : column * (LAYOUT.itemSize + LAYOUT.gap));
        const y = geometry.y + ((proportional ? placement.row : row) * (itemHeight + LAYOUT.gap));
        const itemWidth = proportional ? placement.width : LAYOUT.itemSize;
        ...
        items.push({
          ...
          titleStrip: {
            x,
            y: y + itemHeight - titleStripHeight,
            width: itemWidth,
    ...
    const maxItemsInRow = proportional ? Math.max(1, ...tierGeometry.flatMap(geometry => {
      const counts = new Map();
      for (const placement of geometry.placements) counts.set(placement.row, (counts.get(placement.row) ?? 0) + 1);
      return [...counts.values()];
    })) : columns;
    const internalPlan = {
      classic, contain: classic && shape === 'contain', tierScale: presentation.tierScale, showCounts: presentation.showCounts,
      logicalWidth,
      logicalHeight,
      pixelRatio,
      pixelWidth,
      pixelHeight,
      columns: maxItemsInRow,
...
function coverCrop(cover, ratio = 1) {
  const width = Math.min(cover.width, cover.height * ratio);
  const height = width / ratio;
  return {
    sourceX: (cover.width - width) / 2,
    sourceY: (cover.height - height) / 2,
...
function drawPlan(internalPlan, contextState, loadedCovers, invocation) {
  ...
  try {
    ...
    if (internalPlan.classic) {
      context.fillStyle = '#1a1a1a';
      callContext(contextState, 'fillRect', [0, 0, internalPlan.logicalWidth, internalPlan.logicalHeight]);
    }
    for (const tier of internalPlan.tiers) {
      context.fillStyle = internalPlan.classic ? '#1a1a1a' : TIER_BACKGROUND;
      ...
      try {
        ...
        context.fillStyle = internalPlan.classic ? tier.foreground : '#ffffff';
        context.font = `${internalPlan.classic ? 600 : 700} ${internalPlan.classic ? 20 * internalPlan.tierScale : 20}px sans-serif`;
        ...
        for (let lineIndex = 0; lineIndex < tier.label.nameLines.length; lineIndex += 1) {
          ...
          if (!internalPlan.classic) callContext(contextState, 'strokeText', textArgs);
      ...
      if (internalPlan.classic && internalPlan.showCounts) {
        context.fillStyle = tier.foreground; context.font = '600 11px sans-serif';
        context.textAlign = 'center'; context.textBaseline = 'bottom';
        callContext(contextState, 'fillText', [String(tier.items.length), tier.label.x + tier.label.width / 2, tier.label.y + tier.label.height - 4]);
      }
      ...
      for (const item of tier.items) {
        ...
        if (cover.missing) {
          ...
          const contain = internalPlan.contain;
          const crop = coverCrop(cover, contain ? cover.width / cover.height : item.width / item.height);
          const fit = contain ? Math.min(item.width / cover.width, item.height / cover.height) : null;
          const width = contain ? cover.width * fit : item.width;
          const height = contain ? cover.height * fit : item.height;
...
function snapshotExportInput(value) {
  return withSyncBoundary(invocation => {
    ...
    const logicalMinHeight = ownDataValue(value, 'logicalMinHeight', 'input', invocation, { optional: true });
    if (logicalMinHeight !== undefined) planInput.logicalMinHeight = logicalMinHeight;
