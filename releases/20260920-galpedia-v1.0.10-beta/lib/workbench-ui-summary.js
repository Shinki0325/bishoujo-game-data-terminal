...
export function createWorkbenchUISummary(data,{includeCompanies=true}={}) {
  ...
  const directory=includeCompanies ? buildCompanyDirectory({brands:data.brands,works:data.ratedDisplayWorks,
    companyAliasesById:data.enrichment?.companyAliasesById,companyPinyinById:data.enrichment?.companyPinyinById,
    avatarByCompanyId:data.companyProfile?.avatarByCompanyId,includeWorks:false}) : null;
  return {schema:'galpedia-workbench-ui-summary-v1',workIds,workGroupByEditionWorkId,releaseYearCounts,
    companies:includeCompanies ? serializeCompanySummary(directory) : []};
}
...
export function createOwnedWorkbenchUI(data) {
  ...
  return {
    ...
    ...(data.fullWiki ? {fullWiki:data.fullWiki} : {}),
    searchText: null
  };
}
