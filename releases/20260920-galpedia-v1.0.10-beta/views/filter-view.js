import {currentCompanyWorkFilter} from '../lib/company-work-filter-loader.js';
import {
  FormulaSyntaxError,
  basicToFormula,
  formatFormula
...
import { RELEASE_STATUS } from '../lib/work-release-date.js';
...
const RELEASE_STATUS_LABELS = Object.freeze({
  [RELEASE_STATUS.ALL]: '全部',
  [RELEASE_STATUS.RELEASED]: '已发售',
  [RELEASE_STATUS.UNRELEASED]: '未发售（含未定）',
  [RELEASE_STATUS.TBD]: '发售未定'
});
...
function normalizeYearRangeGroups(source) {
  if (!Array.isArray(source)) return [];
  return source.filter(group => Array.isArray(group)
    && group.length === 3
    && Number.isSafeInteger(group[0]) && group[0] >= 0
    && Number.isSafeInteger(group[1]) && group[1] >= 0
    && Number.isSafeInteger(group[2]) && group[2] >= 0)
    .map(group => [group[0] >>> 0, group[1] >>> 0, group[2]]);
}
...
function activeFilterCount(state) {
  ...
  return Number(state.titleQuery.trim().length > 0)
    ...
    + Number((state.releaseStatus ?? DEFAULT_FILTER_STATE.releaseStatus) !== DEFAULT_FILTER_STATE.releaseStatus)
...
function resetDrawerFilterState(state) {
  return {
    ...
    releaseStatus: DEFAULT_FILTER_STATE.releaseStatus,
    personIds: [],
...
export function createFilterView({
  ...
  const elements = {
    ...
    releaseStatus: optionalElement(root, 'release-status'),
    releaseStatusHint: optionalElement(root, 'release-status-hint'),
  ...
  let currentCounts = {
    current: 0, filters: {}, brands: {}, yearRangeGroups: null, yearUniverseCount: null
  };
  let advancedDraft = '';
  ...
  const companyAliasIds=new Map(currentCompanyWorkFilter()?.aliases??brands.flatMap(b=>(b.companyIdAliases??[b.brandId]).map(id=>[id,b.brandId])));
  const normalizeCompanyFilter=filter=>({...cloneFilterState(filter),brandIds:[...new Set(filter.brandIds.map(id=>companyAliasIds.get(id)??id))]});
  ...
  function personMatchesQuery(person, query) {
    ...
    const state = personMatchesQuery;
    const rawQuery = String(query ?? '');
    if (state.rawQuery !== rawQuery) {
      state.rawQuery = rawQuery;
      state.needle = rawQuery.trim().toLocaleLowerCase('zh-Hans');
    }
    const needle = state.needle;
    if (!needle) return true;
    if (typeof person?.matchKey === 'string') return person.matchKey.includes(needle);
    const includes = value => Boolean(value)
      && String(value).toLocaleLowerCase('zh-Hans').includes(needle);
    if (includes(personNameOf(person)) || includes(person?.canonicalName)) return true;
    for (const alias of Array.isArray(person?.aliases) ? person.aliases : []) {
      if (includes(alias)) return true;
    }
    return includes(person?.searchKey) || includes(person?.pinyinSearchKey);
  }

  const personNameCollator = new Intl.Collator('zh-Hans');
  function visiblePersonSuggestions() {
    const selected = new Set(currentState.personIds ?? []);
    const query = elements.personSearch?.value ?? '';
    const active = [], other = [];
    // Options are stably name-sorted once. Partitioning keeps selected people
    // first without sorting the whole directory on every input or empty focus.
    for (const person of personOptionsState) {
      const id = personIdOf(person);
      if (!id || !personMatchesQuery(person, query)) continue;
      const group = selected.has(id) ? active : other;
      if (group.length < 12) group.push(person);
    }
    return [...active, ...other].slice(0, 12);
  }
  ...
  function renderActiveChips() {
    ...
    if ((currentState.releaseStatus ?? DEFAULT_FILTER_STATE.releaseStatus) !== DEFAULT_FILTER_STATE.releaseStatus) {
      const label = RELEASE_STATUS_LABELS[currentState.releaseStatus] ?? currentState.releaseStatus;
      addChip('releaseStatus', `发售状态 ${label}`, () => emit({
        ...currentState,
        releaseStatus: DEFAULT_FILTER_STATE.releaseStatus
      }));
    }
  ...
  elements.releaseStatus?.addEventListener('change', () => {
    const releaseStatus = RELEASE_STATUS_LABELS[elements.releaseStatus.value]
      ? elements.releaseStatus.value : DEFAULT_FILTER_STATE.releaseStatus;
    // Do not let an earlier debounced rating edit restore the cleared threshold.
    if (['unreleased', 'tbd'].includes(releaseStatus)) emitNumeric.cancel();
    emit({
      ...currentState, releaseStatus,
      ...(['unreleased', 'tbd'].includes(releaseStatus) ? {minimumScore: 0, minimumVoteCount: 0} : {})
    });
  });
  ...
  function renderAppliedSummary() {
    ...
    if (elements.releaseStatusHint) {
      elements.releaseStatusHint.textContent = [RELEASE_STATUS.UNRELEASED, RELEASE_STATUS.TBD]
        .includes(currentState.releaseStatus)
  ...
  function applyProspectiveCounts(prospectiveCounts, resultCount) {
    if (!prospectiveCounts || typeof prospectiveCounts !== 'object') {
      ...
      currentCounts.yearRangeGroups = null;
      currentCounts.yearUniverseCount = null;
      yearProjectionReady = false;
      return;
    }
    currentCounts = {
      ...currentCounts,
      current: resultCount,
      filters: prospectiveCounts.filters ?? currentCounts.filters,
      brands: prospectiveCounts.brands ?? currentCounts.brands
    };
    if (Object.hasOwn(prospectiveCounts, 'yearCounts') && prospectiveCounts.yearCounts) {
      yearCounts = new Map();
      for (let year = DEFAULT_FILTER_STATE.releaseYearStart; year <= DEFAULT_FILTER_STATE.releaseYearEnd; year += 1) {
        yearCounts.set(year, Number(prospectiveCounts.yearCounts[year] ?? 0));
      }
    }
    if (Object.hasOwn(prospectiveCounts, 'yearRangeGroups')
      && Object.hasOwn(prospectiveCounts, 'yearUniverseCount')
      && Number.isSafeInteger(prospectiveCounts.yearUniverseCount)
      && prospectiveCounts.yearUniverseCount >= 0) {
      currentCounts.yearRangeGroups = normalizeYearRangeGroups(prospectiveCounts.yearRangeGroups);
      currentCounts.yearUniverseCount = prospectiveCounts.yearUniverseCount;
      yearProjectionReady = true;
    } else if (Object.hasOwn(prospectiveCounts, 'yearCounts')) {
      currentCounts.yearRangeGroups = null;
      currentCounts.yearUniverseCount = null;
      yearProjectionReady = false;
    }
  ...
  function countYearRange(values) {
    if (!yearProjectionReady || !Array.isArray(currentCounts.yearRangeGroups)) return null;
    const start = Number(values.releaseYearStart);
    const end = Number(values.releaseYearEnd);
    if (start === DEFAULT_FILTER_STATE.releaseYearStart
      && end === DEFAULT_FILTER_STATE.releaseYearEnd
      && Number.isSafeInteger(currentCounts.yearUniverseCount)) {
      return currentCounts.yearUniverseCount;
    }
    const lowStart = Math.max(DEFAULT_FILTER_STATE.releaseYearStart, start);
    const highEnd = Math.min(DEFAULT_FILTER_STATE.releaseYearEnd, end);
    if (lowStart > highEnd) return 0;
    let lowMask = 0;
    let highMask = 0;
    for (let year = lowStart; year <= highEnd; year += 1) {
      const offset = year - DEFAULT_FILTER_STATE.releaseYearStart;
      if (offset < YEAR_WORD_BITS) lowMask = (lowMask | (1 << offset)) >>> 0;
      else highMask = (highMask | (1 << (offset - YEAR_WORD_BITS))) >>> 0;
    }
    let count = 0;
    for (const [groupLow, groupHigh, groupCount] of currentCounts.yearRangeGroups) {
      if (((groupLow & lowMask) !== 0) || ((groupHigh & highMask) !== 0)) count += groupCount;
    }
    return count;
  }
  ...
  function renderYearPreview(values, committed = false) {
    ...
    if (!committed) {
      ...
      if (projected === null) {
        ...
        elements.releaseYearPreview.textContent = `已提交 ${count} 个结果，正在计算预览`;
        return;
      }
      count = projected;
    }
  ...
  return Object.freeze({
    ...
    renderSummary(filterState, resultCount, prospectiveCounts = null) {
      if(draftEditing){
        ...
        draftEditing=false;currentState = normalizeCompanyFilter(filterState);currentCounts={...currentCounts,current:resultCount};
        try{renderAppliedSummary();renderActiveChips();}
        finally{currentState=savedState;currentCounts=savedCounts;draftEditing=true;}
        return;
      }
      currentState = normalizeCompanyFilter(filterState);
    ...
    render(filterState, prospectiveCounts = {}) {
      ...
      currentCounts = {
        ...
        yearRangeGroups: null,
        yearUniverseCount: null
      };
      ...
      if (elements.releaseStatus) elements.releaseStatus.value = currentState.releaseStatus ?? DEFAULT_FILTER_STATE.releaseStatus;
      renderYearLabels();
      renderYearHistogram();
      renderYearPreview(currentState, true);
    ...
    setPersonOptions(options) {
      personOptionsState = Array.isArray(options) ? [...options].sort((left, right) => personNameCollator.compare(personNameOf(left), personNameOf(right))) : [];
