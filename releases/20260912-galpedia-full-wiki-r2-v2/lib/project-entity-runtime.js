...
import { applyCharacterImageMapToCharacters, resolveCharacterImage } from './character-image-map.js';
import { applyProjectIdentityToCast } from './project-identity-crosswalk.js';
...
export async function createProjectEntityRuntime({ bridge, catalog, dataRevision, characterImageMap = null, characterAssetBase = null, cryptoRef = globalThis.crypto } = {}) {
  ...
  return Object.freeze({
    ...
    projectCredits(credits, {
      ...
      characterAssetFallbackBase: activeCharacterAssetFallbackBase = null,
      projectIdentityCrosswalk = null,
    } = {}) {
      const cast = credits === null ? [] : applyProjectIdentityToCast(credits.cast, {
        workId: credits.workId,
        identityCrosswalk: projectIdentityCrosswalk,
        resolveImage: entry => resolveCharacterImage({
        ...
        }, activeCharacterImageMap, {
          assetBase: activeCharacterAssetBase,
          fallbackAssetBase: activeCharacterAssetFallbackBase,
        })
      });
      const projectedCredits = credits === null ? null : Object.freeze({ ...credits, cast });
      const projections = buildPersonCharacterProjections(projectedCredits === null ? [] : [projectedCredits]);
      const characters = applyCharacterImageMapToCharacters(projections.characters, activeCharacterImageMap, {
        assetBase: activeCharacterAssetBase,
        fallbackAssetBase: activeCharacterAssetFallbackBase,
      });
      const displayCast = cast.map(entry => Object.freeze({
        ...entry,
        scopeLabel: entry?.scope === 'admission' || entry?.sourceScope === 'admission' ? '入池作品' : null
      }));
      return Object.freeze({
        projections: Object.freeze({ ...projections, characters }),
        credits: Object.freeze({ ...(credits ?? {}), cast: Object.freeze(displayCast) }),
