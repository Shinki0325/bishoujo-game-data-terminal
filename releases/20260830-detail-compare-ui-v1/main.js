...
async function initialize() {
  ...
  if (bangumiRatingsSource !== null && bangumiPublicBindingsSource !== null && bangumiPublicBindings !== null) {
    try {
      ...
      bangumiRatings = prepareBangumiRatingsSidecar(bangumiRatingsSource.value, {
        ...
        // The current Bangumi ratings carrier covers the merged 7,264-work
        // runtime population; admissions are represented with explicit status.
        workIds: populationContract.runtime.workIds
      });
    } catch (error) {
  ...
  function renderWorkCompare() {
    ...
    if (works.length === 2) {
      ...
      for (const width of ['40px', '64px', '220px', '76px', '150px', '92px', '104px', '92px', '104px', '100px', '112px']) {
      ...
      const appendSortButton = (cell, column, label = column.label) => {
        ...
        button.addEventListener('click', () => {
          ...
          else { compareSortKey = column.key; compareSortDirection = column.key === 'year' || column.type === 'string' ? 'asc' : 'desc'; }
          renderWorkCompare();
        });
        cell.append(button);
      };
