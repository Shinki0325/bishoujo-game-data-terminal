...
const PUBLIC_CREDIT_FIELDS = new Set(['workId', 'editionId', 'editionEntityId', 'vndbStaffId', 'vndbCharacterId', 'egsCharacterId', 'characterName', 'role', 'name', 'source', 'scope', 'scopeFallbackReason', 'creditType', 'status', 'visibility']);
...
function projectCharacterRecord(record, requestedId) {
  ...
  const state = publicStatus(record.status, record.visibility);
  const image = record.image && typeof record.image === 'object' && typeof record.image.assetPath === 'string'
    ? Object.freeze({ source: publicSource(record.image.source), assetPath: record.image.assetPath, assetSha256: record.image.assetSha256, url: typeof record.image.url === 'string' ? record.image.url : null })
    : null;
  return Object.freeze({ characterEntityId: record.characterEntityId ?? `vndb:${identity}`, vndbCharacterId: record.vndbCharacterId ?? null, egsCharacterId: record.egsCharacterId ?? null, sourceRef: publicSourceRef(record.sourceRef, identity), characterName: typeof record.characterName === 'string' ? record.characterName : '', aliases: Object.freeze(publicAliases(record.aliases)), credits: Object.freeze(credits), voiceActors: Object.freeze(actors), image, statistics: Object.freeze({ confirmedVoiceActorCount: actors.filter(item => item.status === 'confirmed').length }), status: state.status, visibility: state.visibility, source: 'projected' });
}
...
export function buildPersonCharacterProjections(workRecords = []) {
  ...
  for (const work of workRecords) {
    ...
    for (const entry of Array.isArray(work.cast) ? work.cast : []) {
      const egsCharacterId = Number.isSafeInteger(entry?.characterId)
        ? entry.characterId
        : (typeof entry?.characterId === 'string' && /^[1-9][0-9]*$/u.test(entry.characterId) ? Number(entry.characterId) : null);
      const rawCharacterId = entry?.vndbCharacterId ?? (egsCharacterId === null ? entry?.characterId : null);
      const characterId = entityId(rawCharacterId, ID_CHARACTER); const status = PERSON_STATUSES.has(entry?.status) ? entry.status : (characterId || egsCharacterId !== null ? 'confirmed' : 'unmapped');
      ...
      if (!character) { character = { characterEntityId: characterId ? `vndb:${characterId}` : (egsCharacterId !== null ? `egs:${egsCharacterId}` : null), vndbCharacterId: characterId, egsCharacterId: egsCharacterId, sourceRef: publicSourceRef(entry?.sourceRef ?? { source: entry?.source }, characterId ?? String(egsCharacterId ?? rawCharacterId ?? entry?.characterName ?? `unmapped-${serial}`)), characterName: entry?.characterName ?? '', role: entry?.role, source: publicSource(entry?.source), aliases: [], credits: [], voiceActors: [], status, visibility: 'public' }; characters.set(key, character); }
      character.credits.push({ workId: work.workId, editionId: work.editionId, editionEntityId: work.editionEntityId, vndbCharacterId: character.vndbCharacterId, egsCharacterId: character.egsCharacterId, characterName: character.characterName, role: entry?.role, source: entry?.source ?? 'vndb', status, visibility: 'public', scope: safeScope(work.scope ?? entry?.scope), scopeFallbackReason: work.scope || entry?.scope ? undefined : 'detail-shard-only-work-scope' });
...
// M3 is deliberately a projection boundary, not a media-maintenance client.
// It accepts only relation facts that may be shown to a reader; private review
// notes, original URLs, upload locations and queue internals are never copied.
const MEDIA_TARGET_TYPES = new Set(['work', 'edition', 'family', 'company', 'person']);
const MEDIA_USAGES = new Set(['thumbnail', 'preview', 'gallery', 'banner', 'avatar', 'screenshot']);
const MEDIA_RIGHTS = new Set(['cleared-public', 'source-attributed', 'owner-cleared', 'not-cleared', 'unknown']);
const MEDIA_ASSERTION_STATUSES = new Set(['confirmed', 'needs-review', 'ambiguous', 'unmapped', 'source-not-returned', 'rejected']);
const MEDIA_VISIBILITIES = new Set(['public', 'private-review', 'hidden']);
const MEDIA_SOURCE_DEFAULT_ORDER = Object.freeze(['owner', 'vndb', 'egs']);
const MEDIA_USAGE_DEFAULT_ORDER = Object.freeze(['thumbnail', 'preview', 'gallery', 'banner', 'screenshot', 'avatar']);
const PUBLIC_CLEARANCE_RIGHTS = new Set(['cleared-public', 'owner-cleared']);

export const DEFAULT_MEDIA_SELECTION_POLICY = Object.freeze({
  policyId: 'media-selection-policy-v1',
  sourcePriority: MEDIA_SOURCE_DEFAULT_ORDER,
  usagePriority: MEDIA_USAGE_DEFAULT_ORDER,
  requirePublicVisibility: true,
  requireConfirmedAssertion: true,
  requireClearedRights: true,
  requireClearedReview: true
});

function mediaSafeIdentifier(value) {
  return typeof value === 'string' && /^[A-Za-z0-9][A-Za-z0-9._:-]*$/u.test(value) ? value : null;
}

function mediaSourceRef(value) {
  const source = publicSource(value?.source, 'unknown');
  const id = mediaSafeIdentifier(value?.id ?? value?.sourceId ?? value?.claimId);
  return id === null ? null : Object.freeze({ source, id });
}

function mediaScope(relation) {
  const raw = relation?.scope && typeof relation.scope === 'object' && !Array.isArray(relation.scope) ? relation.scope : {};
  const scope = Object.freeze({
    workEntityId: mediaSafeIdentifier(raw.workEntityId),
    editionEntityId: mediaSafeIdentifier(raw.editionEntityId),
    familyEntityId: mediaSafeIdentifier(raw.familyEntityId)
  });
  const targetId = mediaSafeIdentifier(relation?.targetEntityId);
  const targetType = relation?.targetType;
  if (targetId === null || !MEDIA_TARGET_TYPES.has(targetType)) return null;
  if (targetType === 'work' && scope.workEntityId !== targetId) return null;
  if (targetType === 'edition' && scope.editionEntityId !== targetId) return null;
  if (targetType === 'family' && scope.familyEntityId !== targetId) return null;
  return scope;
}

function normalizedMediaPolicy(value = {}) {
  const candidate = value && typeof value === 'object' && !Array.isArray(value) ? value : {};
  const securityFlags = ['requirePublicVisibility', 'requireConfirmedAssertion', 'requireClearedRights', 'requireClearedReview'];
  for (const flag of securityFlags) {
    if (Object.hasOwn(candidate, flag) && candidate[flag] !== true) {
      throw new TypeError(`media selection policy cannot disable ${flag}`);
    }
  }
  const sourcePriority = Array.isArray(candidate.sourcePriority) ? candidate.sourcePriority : DEFAULT_MEDIA_SELECTION_POLICY.sourcePriority;
  const usagePriority = Array.isArray(candidate.usagePriority) ? candidate.usagePriority : DEFAULT_MEDIA_SELECTION_POLICY.usagePriority;
  const uniqueSafe = (items, allowed) => [...new Set(items.filter(item => typeof item === 'string' && allowed.has(item)))];
  const normalizedSources = uniqueSafe(sourcePriority, new Set(['owner', 'vndb', 'egs', 'unknown']));
  const normalizedUsages = uniqueSafe(usagePriority, MEDIA_USAGES);
  return Object.freeze({
    policyId: typeof candidate.policyId === 'string' && /^[a-z][a-z0-9-]*-v[0-9]+$/u.test(candidate.policyId) ? candidate.policyId : DEFAULT_MEDIA_SELECTION_POLICY.policyId,
    sourcePriority: Object.freeze(normalizedSources.length ? normalizedSources : [...DEFAULT_MEDIA_SELECTION_POLICY.sourcePriority]),
    usagePriority: Object.freeze(normalizedUsages.length ? normalizedUsages : [...DEFAULT_MEDIA_SELECTION_POLICY.usagePriority]),
    requirePublicVisibility: true,
    requireConfirmedAssertion: true,
    requireClearedRights: true,
    requireClearedReview: true
...
function mediaClearanceStatus(relation) {
  if (relation?.clearanceStatus === 'cleared' || relation?.clearanceStatus === 'not-cleared' || relation?.clearanceStatus === 'unreviewed' || relation?.clearanceStatus === 'unknown') return relation.clearanceStatus;
  // A relation's public rights label establishes neither a maintenance review
  // nor a candidate-level clearance decision. G1 has no such decision field,
  // so it stays unavailable until a future source-only input supplies one.
  return 'unknown';
}

function mediaEligibility(relation, policy) {
  if (policy.requirePublicVisibility && relation.visibility !== 'public') return 'not-public';
  if (policy.requireConfirmedAssertion && relation.assertionStatus !== 'confirmed') return 'assertion-not-confirmed';
  if (policy.requireClearedRights && !PUBLIC_CLEARANCE_RIGHTS.has(relation.rightsStatus)) return 'rights-not-cleared';
  if (policy.requireClearedReview && relation.clearanceStatus !== 'cleared') return 'clearance-not-approved';
  return 'eligible';
}

function projectMediaCandidate(value, policy) {
  const mediaEntityId = mediaSafeIdentifier(value?.mediaEntityId ?? value?.mediaId);
  const mediaRelationId = mediaSafeIdentifier(value?.mediaRelationId ?? value?.relationId);
  const targetEntityId = mediaSafeIdentifier(value?.targetEntityId);
  const scope = mediaScope(value);
  if (mediaEntityId === null || mediaRelationId === null || targetEntityId === null || scope === null || (value?.relationType !== undefined && value.relationType !== 'media-for') || !MEDIA_USAGES.has(value?.usage) || !MEDIA_RIGHTS.has(value?.rightsStatus) || !MEDIA_ASSERTION_STATUSES.has(value?.assertionStatus) || !MEDIA_VISIBILITIES.has(value?.visibility)) return null;
  const sourceRefs = (Array.isArray(value?.sourceRefs) ? value.sourceRefs : [value?.sourceRef]).map(mediaSourceRef).filter(Boolean);
  const sourceClaimIds = [...new Set((Array.isArray(value?.sourceClaimIds) ? value.sourceClaimIds : []).map(mediaSafeIdentifier).filter(Boolean))].sort();
  const source = sourceRefs[0]?.source ?? publicSource(value?.source, 'unknown');
  const candidate = {
    mediaId: mediaEntityId,
    mediaRelationId,
    relationType: value.relationType === undefined ? 'media-for' : value.relationType,
    mediaRevision: mediaSafeIdentifier(value.mediaRevision),
    targetEntityId,
    targetType: value.targetType,
    scope,
    usage: value.usage,
    source,
    sourceRefs: Object.freeze(sourceRefs.sort((a, b) => a.source.localeCompare(b.source) || a.id.localeCompare(b.id))),
    sourceClaimIds: Object.freeze(sourceClaimIds),
    rightsStatus: value.rightsStatus,
    rightsRef: mediaSafeIdentifier(value.rightsRef),
    clearanceStatus: mediaClearanceStatus(value),
    selectionRank: Number.isSafeInteger(value.selectionRank) && value.selectionRank >= 0 ? value.selectionRank : Number.MAX_SAFE_INTEGER,
    assertionStatus: value.assertionStatus,
    visibility: value.visibility
  };
  const selectionEligibility = mediaEligibility(candidate, policy);
  return Object.freeze({ ...candidate, selectionEligibility, availability: selectionEligibility === 'eligible' ? 'available' : 'unavailable' });
}

function candidateComparator(policy) {
  const sourceRank = new Map(policy.sourcePriority.map((source, index) => [source, index]));
  const usageRank = new Map(policy.usagePriority.map((usage, index) => [usage, index]));
  return (left, right) => (left.availability === 'available' ? 0 : 1) - (right.availability === 'available' ? 0 : 1)
    || (usageRank.get(left.usage) ?? Number.MAX_SAFE_INTEGER) - (usageRank.get(right.usage) ?? Number.MAX_SAFE_INTEGER)
    || left.selectionRank - right.selectionRank
    || (sourceRank.get(left.source) ?? Number.MAX_SAFE_INTEGER) - (sourceRank.get(right.source) ?? Number.MAX_SAFE_INTEGER)
    || left.mediaId.localeCompare(right.mediaId)
    || left.mediaRelationId.localeCompare(right.mediaRelationId);
}

function uniqueMediaCandidates(candidates, comparator, keyOf = candidate => candidate.mediaRelationId) {
  const seen = new Set();
  return candidates.slice().sort(comparator).filter(candidate => {
...
function mediaCompatibility(candidates, usage, comparator) {
  const selected = uniqueMediaCandidates(candidates.filter(candidate => candidate.usage === usage && candidate.availability === 'available'), comparator, candidate => candidate.mediaId);
  return Object.freeze({
    availability: selected.length ? 'available' : 'unavailable',
    mediaId: selected[0]?.mediaId ?? null,
    fallbackMediaIds: Object.freeze(selected.slice(1).map(candidate => candidate.mediaId))
  });
}

/**
 * Build a deterministic, source-only media projection.  Array order is never
 * semantic: policy priority and stable IDs decide every primary/fallback slot.
 */
export function buildMediaProjection(mediaRelations = [], { selectionPolicy = {} } = {}) {
  if (!Array.isArray(mediaRelations)) throw new TypeError('media relations must be an array');
  const policy = normalizedMediaPolicy(selectionPolicy);
  const comparator = candidateComparator(policy);
  const groups = new Map();
  for (const source of mediaRelations) {
    const candidate = projectMediaCandidate(source, policy);
    if (candidate === null) continue;
    const key = [candidate.targetType, candidate.targetEntityId, candidate.scope.workEntityId ?? '', candidate.scope.editionEntityId ?? '', candidate.scope.familyEntityId ?? ''].join('|');
    const list = groups.get(key) ?? [];
    list.push(candidate);
    groups.set(key, list);
  }
  const records = [...groups.values()].map(group => {
    const candidates = uniqueMediaCandidates(group, comparator);
    const eligible = candidates.filter(candidate => candidate.availability === 'available');
    const primary = eligible[0] ?? null;
    const gallery = uniqueMediaCandidates(candidates.filter(candidate => candidate.usage === 'gallery'), comparator);
    const thumbnail = mediaCompatibility(candidates, 'thumbnail', comparator);
    const preview = mediaCompatibility(candidates, 'preview', comparator);
    return Object.freeze({
      targetEntityId: candidates[0].targetEntityId,
      targetType: candidates[0].targetType,
      scope: candidates[0].scope,
      selectionPolicy: Object.freeze({ ...policy }),
      primaryMediaId: primary?.mediaId ?? null,
      fallbackMediaIds: Object.freeze([...new Set(eligible.slice(1).map(candidate => candidate.mediaId))]),
      candidates: Object.freeze(candidates),
      gallery: Object.freeze(gallery),
      compatibility: Object.freeze({ thumbnail, preview }),
      availability: primary === null ? 'unavailable' : 'available',
      source: 'projected'
    });
  }).sort((left, right) => left.targetType.localeCompare(right.targetType) || left.targetEntityId.localeCompare(right.targetEntityId) || (left.scope.workEntityId ?? '').localeCompare(right.scope.workEntityId ?? '') || (left.scope.editionEntityId ?? '').localeCompare(right.scope.editionEntityId ?? '') || (left.scope.familyEntityId ?? '').localeCompare(right.scope.familyEntityId ?? ''));
  return Object.freeze({ selectionPolicy: Object.freeze({ ...policy }), records: Object.freeze(records) });
}

export function adaptMediaVM({ mediaId, targetEntityId, legacy = {}, projection = null, featureFlags = {} } = {}) {
  if (mediaId !== undefined) assertText(mediaId, 'media mediaId');
  if (targetEntityId !== undefined) assertText(targetEntityId, 'media targetEntityId');
  if (mediaId === undefined && targetEntityId === undefined) throw new TypeError('media requires mediaId or targetEntityId');
  const flags = resolveProjectEntityFeatureFlags(featureFlags);
  if (!useProjection(projection, flags, 'mediaProjectionV1')) return legacyVm('media', legacy, 'mediaId');
  const record = recordFor(projection.records, item => item?.targetEntityId === targetEntityId || item?.primaryMediaId === mediaId || item?.fallbackMediaIds?.includes(mediaId) || item?.candidates?.some(candidate => candidate.mediaId === mediaId));
  return record === null ? legacyVm('media', legacy, 'mediaId') : Object.freeze({ ...assertObject(legacy ?? {}, 'media legacy view model'), ...record, mediaId: record.primaryMediaId, source: 'projected' });
}
