...
export function createFilterWorkerClient(input) {
  ...
  let queryEpoch = 0, countsEpoch = 0;
  const pending = new Map();

  function retireQueries(types) {
    for (const [id, entry] of pending) {
      if (!types.includes(entry.type)) continue;
      pending.delete(id);
      globalThis.clearTimeout(entry.timer);
      entry.resolve({ status: 'stale', requestId: id });
    }
  }
  ...
  function onMessage(generation, event) {
    ...
    if (entry.type === 'query' && message.type === 'result') {
      const result = {
        ...
        ...(Array.isArray(message.nextWorkIds)?{nextWorkIds:message.nextWorkIds}:{}),
  ...
  return Object.freeze({
    ...
    prepareSearch() {
      ...
      try{ensureWorker().postMessage({type:'prepare-search'});}catch{/* Optional hint; the normal request path reports failures. */}
    },
    init(payload) {
      initPayload = payload;
    ...
    query(payload) {
      ...
      return ensureInitialized().then(() => epoch === queryEpoch
        ? request('query', payload) : { status: 'stale' });
    },
    async counts(payload) {
      const epoch=++countsEpoch;retireQueries(['query-counts']);
      await ensureInitialized();return epoch===countsEpoch?request('query-counts',payload):{status:'stale'};
    },
    cancelQueries() {
      queryEpoch++;countsEpoch++;latestQueryId=0;latestCountsId=0;
      retireQueries(['query','query-counts']);
      // Cancellation retires observers, not shared initialization or caches.
      // The Worker can also skip queries still waiting in its serial queue.
      try { worker?.postMessage({type:'cancel-queries'}); } catch {}
    },
