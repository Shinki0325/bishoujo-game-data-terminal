...
export function withRemoteWorkQueries(local,config,{fetchImpl=globalThis.fetch,now=()=>Date.now(),timeoutMs=1800}={}){
 ...
 return Object.freeze({...local,
  async query(value,options={}){
   ...
   const eligible=input.analysisWorkIds===undefined&&available&&entry&&input.paged&&!input.includeProjectedCounts&&!input.selectedWorkIds?.length&&Number.isSafeInteger(input.pageNumber)&&input.pageNumber>0&&now()>=cooldown&&(!entry.validFrom||day>=entry.validFrom)&&(!entry.validUntil||day<entry.validUntil);
   if(eligible){
  ...
  cancelQueries(){invalidate();local.cancelQueries?.();},
