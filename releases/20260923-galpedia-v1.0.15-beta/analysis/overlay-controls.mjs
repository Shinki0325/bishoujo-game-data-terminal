import {THEME} from './chart-theme.mjs';
...
export function syncOverlayControls(state){
  $('smoothing-control').firstChild.textContent='密度平滑程度';
  $('smoothing-control').title='只影响密度曲线、密度轮廓或二维密度等高线，不改变累计线型。';
  const categorical=state.chart==='histogram'&&field(state.x).type==='category';
  const curveView=state.chart==='density'&&state.densityStyle==='curve';
  const scatter=state.chart==='scatter',distribution=!categorical&&['histogram','density','box'].includes(state.chart);
  ...
  $('cumulative-control').hidden=state.chart!=='histogram'||categorical;$('cumulative-curve').checked=state.cumulativeCurve;
...
function describeOverlay(overlay,result){
  ...
  if(overlay.kind==='scatter-layers'){
    ...
    description=(d.available?'浅 → 深线条分别为密度峰值的 10%、25%、50%、75%；表示局部集中程度，并非圈内记录占比。'+(d.logX?'密度在 log₁₀(横轴) 空间估计。':'')+scope:d.reason)+(fit?' '+fit.description:'');
    method='二维乘积高斯核密度，Scott 二维带宽乘所选平滑倍数；使用全部绘图记录，129 × 129 网格提取等高线。颜色分组不分别拟合。'+(fit?fit.method:'');
    ...
    title=`累计分布${result.state.cdfStyle==='smooth'?' · 平滑显示':' ECDF'} · n = ${overlay.cdf.n.toLocaleString()}`+(kde?' ｜ '+kde.title:'');
    description=(result.state.cdfStyle==='smooth'?'玫红平滑线为核分布估计，读右侧':'玫红阶梯线读右侧')+' 0–100% 轴，表示小于等于横轴数值的记录比例。柱子'+(kde?'与青色密度曲线':'')+'读左侧记录数轴。'+(kde?kde.description:'选择或放大视野不会改变累计分布的分母。');
    method='原始累计比例按有效数值直接计算，同值一起计入，统计不受线型影响。'+(kde?kde.method:'');
  }else if(overlay.kind==='regression'){
    const f=overlay.fit;available=f.available;
    if(f.method==='lowess')return {available,title:f.available?`平滑趋势 · LOWESS · n = ${f.n.toLocaleString()}`:'平滑趋势未绘制',description:f.available?`每处参考附近 ${Math.round(f.fraction*100)}% 的记录，观察弯曲变化；按横轴距离加权。`+(f.logX?'横轴使用 log₁₀。':'')+scope:f.reason,method:'LOWESS 局部线性回归，三次立方核距离权重；不进行残差鲁棒迭代。在最多 129 个位置求值，全部样本参与邻域选择，不外推。'};
    if(f.available){
    ...
    method='叠加最小二乘线性回归，所有绘图记录等权；对数横轴使用 log₁₀(x) 拟合。仅绘制已有横轴数据范围，不外推。R² 描述样本内拟合程度。';
    ...
    description=c.available?'曲线按样本数 × 分箱宽度换算到记录数量轴。'+scope:c.reason+'，保留原直方图。';
    method='叠加高斯核密度估计，以 Silverman 单变量规则确定带宽，再乘所选平滑倍数。曲线是连续估计，原始分箱与记录数不变。';
  }else{
    ...
    description=(overlay.countScale?'宽度表示平滑记录数密度':'宽度表示平滑组内概率密度')+'，所有组共用宽度标尺。'+scope;
    if(valid.length<overlay.curves.length)description+=`${overlay.curves.length-valid.length} 组样本不足或数值完全相同，保留主图。`;
    method='各组独立计算高斯核密度估计，使用 Silverman 带宽与统一平滑倍数；轮廓越宽，对应数值附近的平滑密度越高。每个记录在同一组内仅计一次，组间可重叠。';
  }
...
export function renderOverlaySummary(result){
  ...
  if(overlay.kind==='scatter-layers'){
    ...
    if(overlay.fit?.available)keys.push(overlay.fit.method==='lowess'?['平滑趋势',THEME.trend]:['线性回归',THEME.fit]);
  }else if(overlay.kind==='histogram-layers'){
    if(overlay.curve?.available)keys.push(['密度曲线 · 左轴',THEME.teal]);
    if(overlay.cdf.n)keys.push([result.state.cdfStyle==='smooth'?'累计估计 · 右轴':'累计分布 · 右轴',THEME.cdf]);
  }
