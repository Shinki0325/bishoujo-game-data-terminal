...
export function drawOverlay(chart){
  ...
  const stroke=(points,color,closed=false,dashed=false)=>{
    ...
    for(const [lineColor,width] of [[THEME.background,3.6],[color,2]]){
  ...
  if(overlay.kind==='scatter-layers'&&overlay.density.available){
    overlay.density.contours.forEach((contour,i)=>{
      const color=[THEME.teal+'66',THEME.teal+'99',THEME.teal+'cc',THEME.teal][i];
      ...
      for(const [lineColor,width] of [[THEME.background+'99',2.5],[color,1+i*.2]]){
  ...
  if(['regression','scatter-layers'].includes(overlay.kind)&&overlay.fit?.available){
    stroke(overlay.fit.points.map(p=>({x:chart.x(p.x),y:chart.y(p.y)})),overlay.fit.method==='lowess'?THEME.trend:THEME.fit,false,true);
    ...
    stroke(overlay.curve.points.map(p=>({x:chart.x(p.value),y:chart.y(p.pdf*factor)})),THEME.teal);
...
export function drawCdf(chart,curve,secondary=false){
  ...
  chart.overlayPaths.push({kind:'ecdf',points,color:THEME.cdf});
  ...
  for(const [color,width] of [[THEME.background,4.4],[THEME.cdf,2.2]]){
    ctx.beginPath();traceCdfPath(ctx,points);ctx.strokeStyle=color;ctx.lineWidth=width;ctx.stroke();
  }ctx.restore();
  if(secondary){
    ctx.save();ctx.strokeStyle='#dcb6c9';ctx.beginPath();ctx.moveTo(r,t);ctx.lineTo(r,b);ctx.stroke();ctx.fillStyle=THEME.cdf;ctx.textAlign='left';ctx.font='11px "Segoe UI",sans-serif';
