import {incomingContext,contextState,mountSiteBridge} from './site-bridge.mjs';
import {ratingActive,ratingLabel} from './rating-filter.mjs';
...
let draftKey=DRAFT_KEY,siteBridge;
...
new MutationObserver(()=>{if(result&&document.body.dataset.computing!=='true')refresh({reuse:true});}).observe(document.documentElement,{attributes:true,attributeFilter:['data-theme']});
function selectRows(ids,mode='replace'){
  ...
  selection=next;page=0;updateSelection(false);renderTable();plot.set(result,selection);syncFullUI(result,data,selectRows,selection);builder?.sync();siteBridge?.sync();syncHistory();queueDraft();
}
...
function flushDraft(){
  ...
  try{localStorage.setItem(draftKey,JSON.stringify({...snapshot(),schemaVersion:DRAFT_KEY,releaseId:data.releaseId,catalogSha256:data.catalogSha256}));$('draft-status').textContent='草稿已自动保存 · 本机';}
...
function apply(patch,{record=true,preserveSelection=false}={}){
  ...
  if(Object.hasOwn(patch,'minVotes')&&!Object.hasOwn(patch,'voteFilterMode'))patch={...patch,voteFilterMode:'count'};
...
async function refresh({reuse=false}={}){
  if(!data)return;
  const ticket=++refreshId;document.body.dataset.computing='true';builder?.busy(true);$('result-summary').textContent='正在更新图表…';
  let next;try{next=reuse?result:await engine.run(state);}catch(error){if(ticket===refreshId){document.body.dataset.computing='false';if(result){state=result.state;syncControls();syncFullUI(result,data);}$('result-summary').textContent=error.message;builder?.busy(false,error.message+'；已保留上一结果，请重试。');toast(error.message);}return;}
  ...
  $('steps').replaceChildren(...[
    '读取 '+data.releaseId,`${ratingLabel(state,plottedFields(state))}；发行年 ${state.yearFrom}–${state.yearTo}`,
  ...
  syncFullUI(result,data,selectRows,selection);builder?.sync();siteBridge?.sync();
...
function renderTable(){
  ...
  $('table-body').replaceChildren(...rows.slice(page*10,page*10+10).map(r=>{
    ...
    a.textContent=r.title;a.title=r.originalTitle;a.href='https://favorite.bishojo.date/#works/work/'+encodeURIComponent(r.workId??r.workIds?.[0]??r.id);a.target='_blank';a.rel='noopener';if(individual){a.removeAttribute('href');a.removeAttribute('target');a.title='角色：'+r.characterId+'；关联作品见后一列';}small.textContent=individual?r.characterId:'EGS '+r.id;title.append(a,small);tr.append(title);
    if(individual){const td=document.createElement('td');for(const [i,id] of r.workIds.entries()){if(i)td.append(document.createTextNode(' / '));const link=document.createElement('a');link.textContent=r.workTitles[i];link.href='https://favorite.bishojo.date/#works/work/'+encodeURIComponent(id);link.target='_blank';link.rel='noopener';td.append(link);}tr.append(td);}
...
async function restorePlan(p){
  ...
  updateSelection(true);renderTable();plot.set(result,selection);builder?.sync();siteBridge?.sync();builder?.compact();$('analysis-launcher')?.close();queueDraft();$('dirty-label').textContent='已恢复';toast('已恢复方案与图中选择。');
}
...
function renderFilterChips(){
  const chips=[...(state.recordFilter?[{text:'仅看所选 '+state.recordFilter.ids.length.toLocaleString()+' 个'+unitLabel(state),patch:{recordFilter:null}}]:[]),...(ratingActive(state)?[{text:ratingLabel(state,plottedFields(state)),patch:{minVotes:0,voteFilterMode:'count'}}]:[]),
...
function bind(){
  ...
  const defaultCompanies=()=>{const keys=new Set(data.rows.map(r=>r.company));return [...new Set(['Key','AUGUST','ALICESOFT','CIRCUS'].filter(k=>keys.has(k)).concat([...keys]))].slice(0,2);};
...
async function start(){
  try{({data,engine}=await loadFullData());
    ...
    const ingress=incomingContext();if(ingress)draftKey=DRAFT_KEY+':'+ingress.token;
    let draft=null;try{draft=readDraft(localStorage.getItem(draftKey),data);}catch{}
    if(draft){state=draft.state;selection=new Set(draft.selection);$('plan-name').value=draft.name;}
    if(ingress&&!draft){state={...INITIAL,...contextState(ingress.value)};selection=new Set();$('plan-name').value=ingress.value.label;}
    ...
    siteBridge=mountSiteBridge({state:()=>state,rows:()=>tableRows(),toast,save:flushDraft},ingress);
