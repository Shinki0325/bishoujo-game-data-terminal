...
import {mountRatingFilter,syncRatingFilter} from './rating-filter-ui.mjs';
...
export function mountFullUI(data,apply,dashboard){
  ...
  $('min-votes').closest('label').before(select('vote-source','评价人数来源',[['auto','随评分字段'],['votes','固定 EGS'],['bangumiVotes','固定 Bangumi'],['vndbVotes','固定 VNDB']],voteSource=>apply({voteSource})));
  mountRatingFilter(apply,dashboard);
