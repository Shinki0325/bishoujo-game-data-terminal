...
import {filterRatings} from './rating-filter.mjs';
...
export const DEFAULT = Object.freeze({chart:'scatter',x:'year',y:'median',color:'decade',minVotes:30,voteSource:'auto',voteFilterMode:'count',voteTrimPercent:10,
...
export function sanitize(input={}) {
  ...
  s.voteSource=['votes','bangumiVotes','vndbVotes'].includes(input.voteSource)?input.voteSource:'auto';
  s.voteFilterMode=input.voteFilterMode==='percentile'?'percentile':'count';
  s.voteTrimPercent=Number.isFinite(Number(input.voteTrimPercent))?Math.min(95,Math.max(0,Number(input.voteTrimPercent))):10;
...
export function restoreState(input){
  return sanitize({...input,voteSource:input?.voteSource??'votes',...(!input?.grainPolicyVersion&&plottedFields({...DEFAULT,...input}).some(id=>field(id).character)?{grain:'auto'}:{}),...(input?.chart==='box'&&input?.distribution==='density'?{chart:'density',densityWidth:5}:{})});
}
...
export function filterPopulation(rows,s){
  const query=s.search.toLocaleLowerCase();
  const subset=s.recordFilter?new Set(s.recordFilter.ids):null;
  const candidates=rows.filter(r=>(!subset||subset.has(r.id))&&(r.entityKind==='character'||(
  ...
  return filterRatings(candidates,s,plottedFields(s));
}
export const filterRows=(rows,s)=>filterPopulation(rows,s).rows;
...
function analyzeBase(rows,input,prepared=null){
  const s=sanitize(input),filtered=prepared??filterRows(rows,s);
  if(s.chart==='matrix'){
  ...
  if(s.chart==='line'||s.chart==='ecdf'&&s.seriesField!=='none'){
    const base=analyzeBase(rows,{...s,chart:'box',x:s.seriesField,y:s.chart==='ecdf'?s.x:s.y},filtered);
...
export function analyze(rows,input){
  const normalized=sanitize(input),population=filterPopulation(rows,normalized);
  const result=analyzeBase(rows,normalized,population.rows),s=result.state,axis=scoreAxis(s);
  result.ratingFilter=population.summary;
