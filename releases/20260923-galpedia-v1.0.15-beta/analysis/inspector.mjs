...
export function groupStatistics(result){
  ...
  const rows=(result.groups??[]).map(g=>{
    ...
    const stats=numericSummary((g.memberIds??g.ids).map(id=>lookup.get(id)?.[metric]));
...
export function renderGroupStatistics(result,selected,onSelect){
  ...
  $('group-table-body').replaceChildren(...groupStatistics(result).map(({group:g,values})=>{
    ...
    button.setAttribute('aria-label','查看 '+g.label+' 的 '+g.ids.length+' 个有效'+unitLabel(result.state));button.addEventListener('click',()=>onSelect(g.ids));td.append(button);tr.append(td);return tr;
  }));
}
