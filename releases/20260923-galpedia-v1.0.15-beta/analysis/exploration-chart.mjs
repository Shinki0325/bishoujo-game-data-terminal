import {cdfCanvasPath,traceCdfPath,cdfDisplayAt} from './cdf-presentation.mjs';
...
// Labels have their own lane. Keep their order and minimum separation at both edges.
export function placeSeriesLabels(items,top,bottom,gap=24){
  const sorted=items.map(item=>({...item})).sort((a,b)=>a.y-b.y);
  if(!sorted.length||bottom-top<(sorted.length-1)*gap)return [];
  for(let i=0;i<sorted.length;i++)sorted[i].labelY=Math.max(top,Math.min(bottom,sorted[i].y),i?sorted[i-1].labelY+gap:top);
...
export function drawSeries(chart){
  ...
  const panels=s.facet?groups.map((g,i)=>({groups:[g],index:i})): [{groups,index:0}];chart.panels=[];chart.brushPoints=[];chart.seriesLabels=[];
  for(const panel of panels){
    ...
    const labelledGroups=s.brushGroup?panel.groups.filter(g=>String(g.key)===s.brushGroup):panel.groups;
    const labelLane=!s.facet&&width>=600&&labelledGroups.length<=4?Math.min(210,width*.26):0;
    const p={l:ox+(width<400?60:53),r:ox+width-(labelLane||22),t:oy+39,b:oy+height-50},x=v=>p.l+(v-xmin)/(xmax-xmin)*(p.r-p.l),y=v=>p.b-(v-ymin)/(ymax-ymin)*(p.b-p.t);
    ...
    ctx.fillText(s.facet?shortLabel(ctx,`${panel.groups[0].label} · n=${panel.groups[0].n}`,p.r-p.l):ecdf?'累计比例 · 每组独立计算':field(s.y).short+' · '+(s.lineUnit==='edition'?'每个点是一条记录':s.aggregation==='mean'?'每个点是组内均值':'每个点是组内中位数'),p.l,oy+20);
    for(const g of panel.groups){const color=seriesColor(g.key),faded=s.brushGroup&&String(g.key)!==s.brushGroup;
      clipped(ctx,p,()=>{ctx.globalAlpha=faded?.22:1;ctx.strokeStyle=color;ctx.lineWidth=ecdf?2.3:2;ctx.lineJoin='round';ctx.lineCap='round';ctx.beginPath();
        if(ecdf){const estimate=r.cdfEstimates?.groups?.[g.key],points=cdfCanvasPath(estimate?.available?estimate:g.cdf,estimate?.available?'smooth':'step',x,y,p.l,p.r);if(!points.length)return;traceCdfPath(ctx,points);ctx.stroke();chart.overlayPaths.push({kind:'ecdf',groupKey:g.key,points,color});
          const ids=new Set(g.ids);for(const row of r.rows){if(!ids.has(row.id)||!Number.isFinite(row[s.x]))continue;const px=x(row[s.x]),py=y(estimate?.available?cdfDisplayAt(estimate,row[s.x],'smooth'):cdfAt(g.cdf,row[s.x]));if(px>=p.l&&px<=p.r)chart.brushPoints.push({x:px,y:py,ids:[row.id],groupKey:String(g.key)});}
        }else{
          const segments=lineSegments(g.points,s);
          for(const {from,to,gap} of segments){if(gap)continue;ctx.moveTo(x(from.x),y(from.y));ctx.lineTo(x(to.x),y(to.y));}ctx.stroke();
          ctx.save();ctx.globalAlpha*=.6;ctx.lineWidth=1.4;ctx.setLineDash([4,5]);ctx.beginPath();
          for(const {from,to,gap} of segments){if(!gap)continue;ctx.moveTo(x(from.x),y(from.y));ctx.lineTo(x(to.x),y(to.y));}ctx.stroke();ctx.restore();
          for(const v of g.points){const px=x(v.x),py=y(v.y);if(px<p.l||px>p.r||py<p.t||py>p.b)continue;
            const selected=v.ids.some(id=>chart.selected.has(id));ctx.beginPath();ctx.arc(px,py,selected?4.5:s.lineUnit==='edition'?2:2.5,0,Math.PI*2);ctx.fillStyle=selected?color:THEME.background;ctx.fill();ctx.strokeStyle=selected?'#ffffff':color;ctx.lineWidth=selected?1.6:1.4;ctx.stroke();ctx.strokeStyle=color;
            const hit={kind:'point',x:px,y:py,ids:v.ids,groupKey:String(g.key),text:`${g.label}\n${v.label}\n${field(s.y).short}：${label(v.y)} · ${v.n} 个记录\n点击查看这些记录`};chart.hits.push(hit);chart.brushPoints.push(hit);
          }
        }
      });
      if(!g.n&&s.facet){ctx.fillStyle='#998ca4';ctx.font='12px "Microsoft YaHei",sans-serif';ctx.fillText('当前筛选下没有有效记录',p.l+10,(p.t+p.b)/2);}
    }
    if(!ecdf&&labelLane){
      const ends=labelledGroups.filter(g=>g.points.length).map(g=>{const end=g.points.at(-1);return {g,x:x(end.x),y:y(end.y)};}).filter(v=>v.x>=p.l&&v.x<=p.r&&v.y>=p.t&&v.y<=p.b);
      for(const mark of placeSeriesLabels(ends,p.t+12,p.b-6)){
        const color=seriesColor(mark.g.key),left=p.r+22,maxWidth=ox+width-left-8;
        ctx.save();ctx.strokeStyle=color+'70';ctx.lineWidth=1;ctx.setLineDash([2,3]);ctx.beginPath();ctx.moveTo(mark.x+5,mark.y);ctx.lineTo(p.r+8,mark.y);ctx.lineTo(left-5,mark.labelY-4);ctx.stroke();ctx.setLineDash([]);
        ctx.font='600 11px "Microsoft YaHei",sans-serif';ctx.textAlign='left';ctx.fillStyle=color;ctx.fillText(shortLabel(ctx,mark.g.label,maxWidth),left,mark.labelY);ctx.restore();
        chart.seriesLabels.push({key:String(mark.g.key),left,top:mark.labelY-12,bottom:mark.labelY+3,right:ox+width-8});
      }
    }
    if(ecdf&&s.cdfThreshold>=xmin&&s.cdfThreshold<=xmax){
      ...
      chartLabel(ctx,'实际 ≤ '+label(s.cdfThreshold),Math.min(p.r-24,Math.max(p.l+24,px)),p.t+15,{color:THEME.muted});
      ...
      const positions=labelLane?placeSeriesLabels(marks.filter(m=>labelledGroups.includes(m.g)).map(m=>({...m,y:y(m.proportion)})),p.t+22,p.b-18,40):[];
      for(const m of marks){
        ...
        const position=positions.find(v=>v.g===m.g);
        if(position){const {labelY}=position;ctx.strokeStyle=color+'60';ctx.lineWidth=.8;ctx.beginPath();ctx.moveTo(px+5,py);ctx.lineTo(p.r+5,labelY-3);ctx.stroke();ctx.textAlign='left';ctx.fillStyle=color;ctx.font='600 11px "Microsoft YaHei",sans-serif';ctx.fillText(shortLabel(ctx,m.g.label,ox+width-p.r-24),p.r+16,labelY-7);ctx.font='600 14px "Segoe UI",sans-serif';ctx.fillText((m.proportion*100).toFixed(1)+'%',p.r+16,labelY+10);chart.seriesLabels.push({key:String(m.g.key),left:p.r+16,top:labelY-20,bottom:labelY+13,right:ox+width-8});}
...
export function seriesHit(chart,p){
  ...
  for(const g of panel.groups){if(s.brushGroup&&s.brushGroup!==String(g.key)||!g.cdf.n)continue;const proportion=cdfAt(g.cdf,value),estimate=r.cdfEstimates?.groups?.[g.key],display=estimate?.available?cdfDisplayAt(estimate,value,'smooth'):proportion,dy=Math.abs(p.y-(panel.b-display*(panel.b-panel.t)));
    if(dy<distance){const members=new Set(g.ids),ids=r.rows.filter(row=>members.has(row.id)&&Number.isFinite(row[s.x])&&row[s.x]<=value).map(row=>row.id);distance=dy;best={kind:'cdf',ids,text:`${g.label}\n${field(s.x).short} ≤ ${value.toFixed(2)}\n实际累计 ${ids.length} / ${g.cdf.n} 个记录 · ${(proportion*100).toFixed(1)}%\n点击查看本组对应记录`};}
  }return best;
}
export function drawMatrix(chart){
  ...
  for(let j=0;j<n;j++)for(let i=0;i<n;i++){
    ...
    ctx.fillStyle=diagonal?THEME.background:correlationColor(v);ctx.fillRect(p.l,p.t,p.r-p.l,p.b-p.t);
    ...
    if(s.matrixStyle==='scatter'&&!diagonal){
      ...
      ctx.fillStyle=THEME.background;ctx.fillRect(p.l,p.t,p.r-p.l,p.b-p.t);clipped(ctx,p,()=>{ctx.fillStyle=THEME.purple+'30';for(const row of pairs){const x=p.l+5+(row[cell.x]-xmin)/(xmax-xmin)*(size-14),y=p.b-18-(row[cell.y]-ymin)/(ymax-ymin)*(size-27);ctx.fillRect(x,y,1.4,1.4);}});
    }else{
